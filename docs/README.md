...
## Per-milestone records (M0–M50, all closed; tags v1.0.11–v1.2.0)
...
Later milestones (M38+) similarly vary in doc completeness: **M38/M39** (v1.0.39) were
ledger-backfilled from git history (DEC-0060) with only `m39-*`; **M48** (v1.1.5) has
planner + qa-signoff + `m48-openmsx-ab.md`; **M49** (v1.1.6) shipped on owner live-validation
with a planner package but no separate qa-signoff; **M50** (v1.2.0, external XML config) has
planner + `m50-qa-signoff.md`. The authoritative per-milestone status remains the
`agent-protocol/state/` ledger, not this index.
