...
- Status: In Progress (WIP checkpoint committed af44cb3; NOT tagged). Phase 1 (msx-playtest harness) committed d522804. **Phase 2 (DEC-0050) + Phase 3 (DEC-0051/0052 debug tooling) DONE + committed af44cb3**: FM-PAC peripheral cartridge (real-ROM integration test), disk-save persistence, speculative sram_ removed, R-M35-1 strengthened, DSKCHG read-and-clear fix; F12 comprehensive debug snapshot + F10 live stream-capture (per-frame snapshots + CPU-trace ring + FDC read log + HALT/SP-underflow crash auto-finalize). **QA CONDITIONAL PASS** on the committed portion (docs/m36-qa-signoff.md; 206/206, 15/15 focus targets, zero cpu/core, openMSX source-parity independently verified, 2/2 adversarial mutations non-vacuous). The mandatory item **Bug B (YS II building-interior black screen) is ROOT-CAUSED but OPEN (DEC-0053)**: a FAITHFUL nested VBLANK-interrupt storm (the game's minimal ISR 0xA5E3 runs the sound routine's unpatched EI 0xA373 and never acks S#0); our V9958 interrupt path was audited and is CORRECT vs openMSX; disk reads are byte-perfect (12/12 CRCs vs disk2.dsk); the true divergence is UPSTREAM of the captured window and needs a human-driven earlier F10 trace and/or an openMSX A/B of the scene (NOT headlessly reproducible -- no crashing input log; navigation stalls). NEW accuracy backlog item: WD2793 read rotational latency (a real fidelity defect, but audit-assessed NOT the Bug B cause). **v1.0.37 WITHHELD** (the milestone's own completion goal is unmet). Carrying residual R-M35-1.
