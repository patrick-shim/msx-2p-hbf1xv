...
- Status: **QA CONDITIONAL PASS (docs/m37-qa-signoff.md) — tag v1.0.38 GATED on human live-verify.** All 5 slices implemented + committed (A 861eec1, B 85b48aa, D+E cd459a3, C 85827e5). QA clean-rebuild **210/210, 0 failed**; cpu/core diff EMPTY across the cycle; Slice-C oracle proven live under adversarial mutation (5 targeted fails → byte-identical restore); NO oracle weakened; B byte-identity non-vacuous; D/E additive. QA cross-checked fMSX (WD1793 asserts DRQ immediately — references disagree; this emulator correctly follows the more-accurate openMSX/real-hardware rotational model). **Two human live-verify gates before the tag:** (1) **Slice C (Medium/blocker-if-regression)** — YS II two-disk boot + in-game interior/disk load + save with the new read-latency; (2) **Slice E (Low/non-blocking)** — SDL3 `--scale`/`--filter` visual + drag-resize + Alt+Enter fullscreen (both directions, no RETURN leak). On both passing → full PASS → coordinator release decision + git tag v1.0.38. Future backlog (noted by Slice C): write/read-address/read-track paths still use the fixed first-DRQ latency (rotational model not yet applied — higher risk, deferred).
