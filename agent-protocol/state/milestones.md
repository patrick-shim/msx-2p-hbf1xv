...
## M36 (Kickoff 2026-07-10, DEC-0049 — playtest/live-QA harness + 2 YS II live-playtest bugs; Phase 2A REFRAMED by DEC-0050; tag target v1.0.37)

- Milestone ID: M36
- Title: Autonomous Playtest/Live-QA Harness (Phase 1) + YS II Live-Playtest Bug Fixes (Phase 2 — building-entry black screen + save persistence via FM-PAC peripheral cartridge and/or disk saves)
- Spec Owner: MSX Planner Agent (opus)
- Developer Owner: MSX Developer Agent (opus)
- QA Owner: MSX QA Agent (opus)
- Objective: (Phase 1) Build a GENERAL, REUSABLE, AUTONOMOUS playtest/live-QA capability — a vision-driven observe->act->replay loop (headless --input-script drive + --dump-frame PNG capture read by a vision-capable opus agent; deterministic/cycle-stamped -> regression-able). (Phase 2) Apply it to the two bugs the human found in live YS II play, plus the DEC-0050 disk-write work item, toward the completion criterion: "YS II fully playable like real hardware — building interiors load, FM-PAC S-RAM saves work (peripheral cart) AND/OR disk saves persist — verified by msx-playtest."
- Scope: Phase 1 = additive tooling (.claude/, tools/, additive src/main.cpp/frontend flags — the M27 precedent; ZERO device/core edits). Phase 2 = device-level + machine-wiring + additive-CLI: S-a msx-playtest agent frontmatter enabler; S-b Bug B (black-screen-on-building-entry) repro->root-cause->universal fix; S-c disk-save persistence (host-file write-back opt-in + formatted-blank-720KB disk asset/tool); S-d FM-PAC peripheral cartridge (new CartridgeMapperType::FmPac — ROM + 8KB battery SRAM + 0x5FFE/0x5FFF magic unlock + 0x7FF6 enable + 0x7FF7 bank per references/openmsx-21.0/src/sound/MSXFmPac.cc, loadable via --cart roms/fmpac.rom); S-e reconcile/remove the speculative internal sram_ (a DEC-0050 inaccuracy); S-f strengthen the R-M35-1 multi_disk integration test. ZERO src/devices/cpu/ or src/core/ edits expected -> ZEXALL withheld; additive/default-off so single --disk / no-cart invocations stay byte-for-byte unchanged.
- DEC-0050 reframe: the HB-F1XV built-in FM is MSX-MUSIC (OPLL + BIOS, NO SRAM) — so "NO S-RAM AVAILABLE" on a bare machine is CORRECT hardware behavior; the FM-PAC's 8 KB battery SRAM is a PERIPHERAL cartridge, not internal. This supersedes the machine-level-SRAM framing of docs/m36-planner-package.md; the authoritative Phase-2 plan is docs/m36-phase2-planner-package.md.
- Acceptance Criteria: (C-build, MANDATORY) YS II building interiors load — Bug B fixed universally, msx-playtest-verified (vision-read frames). (C-save, AND/OR) at least one save-persistence path demonstrably persists — FM-PAC SRAM save (roms/fmpac.rom inserted via --cart) and/or disk save written back to a .dsk. Per-slice deterministic unit/integration/system tests; openMSX A/B where behavior-affecting (FM-PAC vs MSXFmPac); msx-playtest end-to-end verification of the completion criterion; zero regression; QA sign-off covering Phase-1 tooling too before closure.
- Status: **Done — CLOSED by coordinator release decision DEC-0054, tag v1.0.37 (2026-07-10); YS II FULLY PLAYABLE like real hardware (building interiors load + saves persist), LIVE human-validated on the real SDL3 build.** Phase 1 (msx-playtest harness) committed d522804. **Phase 2 (DEC-0050) + Phase 3 (DEC-0051/0052 debug tooling) DONE + committed af44cb3**: FM-PAC peripheral cartridge (real-ROM integration test), disk-save persistence, speculative sram_ removed, R-M35-1 strengthened, DSKCHG read-and-clear fix; F12 comprehensive debug snapshot + F10 live stream-capture (per-frame snapshots + CPU-trace ring + FDC read log + HALT/SP-underflow crash auto-finalize). **QA CONDITIONAL PASS** on the committed portion (docs/m36-qa-signoff.md; 206/206, 15/15 focus targets, zero cpu/core, openMSX source-parity independently verified, 2/2 adversarial mutations non-vacuous). The mandatory item **Bug B (YS II building-interior black screen) is FIXED (DEC-0054)**: it was a real V9958 interrupt-accuracy bug — `change_register` did not re-evaluate /INT on an IE1/IE0 register write, so YS II's building-load ISR clearing IE0 left our /INT asserted → the ISR's EI re-fired into a nested-VBLANK stack overflow. Fixed per openMSX VDP.cc:1177-1198 (SUPERSEDES DEC-0053's "V9958 interrupt path is correct" conclusion — that audit checked the S#0-read path but missed the register-write path); pinned by the human's F10 lightweight-watch-mode capture. Evidence: non-vacuous unit test + interrupt-seam regressions clean + openMSX VDP-IRQ runtime A/B (0→1→0→1 on the genuine Sony_HB-F1XV) + LIVE human validation. **SDL3 FM-PAC SRAM persistence** added (auto-save <cart>.sram, load-on-insert + flush-on-exit). **QA PASS** (docs/m36-qa-signoff-final.md; clean-rebuild 207/207 then 208/208, zero cpu/core, source parity line-for-line, non-vacuous mutation, independent openMSX VDP-IRQ A/B; R3 A/B waiver recorded). Residual non-blocking: R-A WD2793 read rotational latency (NEW accuracy backlog, audit-assessed NOT the Bug B cause), R-B commit a tools/ VDP-IRQ probe, R-C/D/E FM-PAC minor; R-M35-1 satisfied.

## M37 (Opened 2026-07-10, DEC-0055 + DEC-0056 — post-M36 hardening; tag target v1.0.38)

- Milestone ID: M37
- Title: Post-M36 hardening — clear the DEC-0054 residuals + two live-play feature requests
- Spec Owner: DEC-0055 (A/B/C) + DEC-0056 (D/E); MSX Master Agent (coordinator)
- Developer Owner: MSX Developer Agent (three dispatches: A/B, D/E, C)
- QA Owner: MSX QA Agent
- Scope: FIVE independent frontend / src/devices/fdc / tooling slices — ZERO src/devices/cpu & src/core edits (ZEXALL withheld, git-diff-proven empty across e845b1e..HEAD):
    - **A** (DEC-0055/R-B) — commit the openMSX VDP-IRQ A/B probe as a reusable `tools/openmsx-vdp-irq-parity.ps1` + `docs/openmsx-vdp-irq-parity.md` (PASS 0→1→0→1). Tooling/docs only.
    - **B** (DEC-0055/R-D) — mix an inserted external FM-PAC cartridge's YM2413 OPLL into the machine audio (per-bay `FmSources[2]`, additive; reuses the built-in MSX-MUSIC OPLL amplitude scale + advance cadence; byte-identical when no FM-PAC inserted).
    - **C** (DEC-0055/R-A / DEC-0053) — WD2793 Read-Sector rotational first-DRQ latency: replace the fixed 228-cycle first DRQ with an index-pulse-relative variable delay (`cycles_until_sector_id`). openMSX-corroborated (docs/openmsx-fdc-read-timing-ab.md). Documented sector-vs-track approximation. Highest regression risk — quarantined last.
    - **D** (DEC-0056) — `--speed <0..7>` CLI (headless + SDL3) sets the initial Sony Speed Controller level (Mb670836PauseController; a CPU-slowdown duty cycle, NOT a turbo).
    - **E** (DEC-0056) — SDL3 window scaling: resizable + aspect-correct letterbox (SDL_SetRenderLogicalPresentation 320×240) + `--filter nearest|linear` (default linear) + `--scale N` (default 2 = 640×480) + `--fullscreen` + Alt+Enter runtime toggle.
- Acceptance Criteria: each slice implemented + deterministic tests + evidence gates; no existing oracle weakened; behavior-affecting slices carry an openMSX A/B; QA sign-off before the v1.0.38 tag; the two human live-verify actions (C YS II, E visual) pass.
- Unit Tests Required: frontend_machine_audio_mixer_fmpac_unit_test (B); wd2793_type2_unit_test rotational cases (C); sdl3_cli_unit_test cases 10–14 (D/E).
- Integration Tests Required: frontend_sdl3_video_presenter_pixel_integration_test filter readback + frontend_sdl3_app_scaling_integration_test (E); the m16 FDC/boot + disk-save + multi-disk integration tests re-green (C).
- Regression Scope: full deterministic suite (ZEXALL #128 withheld); built-in audio path byte-identity (B); FDC/boot/disk-save/disk-read/multi-disk (C); parse_sdl3_cli additive discipline (D/E).
- Status: **Done — CLOSED by coordinator release decision DEC-0058, git tag v1.0.38 (2026-07-10); QA FINAL PASS (docs/m37-qa-signoff.md) + LIVE human validation (DEC-0057: "YS II loads and plays perfectly, SRAM works perfectly, screen looks good").** Six slices implemented + committed (A 861eec1, B 85b48aa, D+E cd459a3, C 85827e5, F 83a0888). QA pristine clean-rebuild **210/210, 0 failed**; cpu/core diff EMPTY across the whole cycle (e845b1e..HEAD); NO oracle weakened (Slice-C rotational oracle proven live under adversarial mutation 5 targeted fails → byte-identical restore; Slice-F F10-gate non-vacuous by two-sided injected-event differential); B byte-identity non-vacuous; D/E/F additive. QA cross-checked fMSX (WD1793 asserts DRQ immediately — references disagree; this emulator correctly follows the more-accurate openMSX/real-hardware rotational model). **Both human live-verify gates DISCHARGED (DEC-0057):** (1) Slice C — YS II two-disk boot + interior/disk load + save with the new read-latency: PASS (no regression); (2) Slice E — SDL3 scaling/fullscreen visual: PASS. **Slice F (DEC-0057):** --capture on|off (default off) gating the F10 hotkey so a mis-struck F10 is inert during play (F11/F12/Alt+Enter unchanged) + default --scale 3/--filter linear (960×720 smooth out-of-box; explicit overrides; E's 640×480 default re-derived to 960×720, oracle strengthened). Future backlog (non-blocking, noted by Slice C + QA): WD2793 write/read-address/read-track paths still use the fixed first-DRQ latency (rotational model not yet applied — higher risk, deferred); optional F12-snapshot gating under --capture if the human wants it.

## M38 (2026-07-11, backfilled by DEC-0060 — V9958 bitmap horizontal scroll fix; released in v1.0.39)

- Milestone ID: M38
- Title: V9958 bitmap-mode horizontal scroll correctness (R#27 fine + multi-page windowing)
- Spec Owner: MSX Master Agent (coordinator) — from the human's live-play report ("MSX2+ only games ... the 9958 VDP should support horizontal scroll properly but the screen gets all scrambled up"; F1 + Laydock screenshots)
- Developer Owner: MSX Developer Agent (commit `b0db02b`)
- QA Owner: consolidated into the M39/v1.0.39 sign-off (`docs/m39-qa-signoff.md` §2.2)
- Scope: universal register-level fix in `src/devices/video/vdp_frame_renderer.{cpp,h}` for two root causes of scrambled horizontal scroll in bitmap modes (re-derived from openMSX, understanding only — no code copied): (1) R#27 FINE scroll was wrong-signed + circular-wrapped (73-75% pixel divergence) — R#26 rotates the display LEFT in 8px steps (already correct) while R#27 shifts RIGHT 0..7 dots and exposes a left border strip (separate mechanism/sign; VDP.hh:340-342/629-631, SDLRasterizer.cc:464-465, PixelRenderer.cc:586-604); (2) multi-page split-page windowing missing (25% divergence) — the coarse-wrap tail must read the ADJACENT even/odd page (p2=p1^1) under isMultiPageScrolling (R#25 bit0 && R#2 bit5), not a self-wrap (SDLRasterizer.cc:479-538). Fix splits coarse-left / fine-right+border; `compose_bitmap_scroll()` reads the coarse-wrap tail from the adjacent page; two-page decode runs only when multi-page is active (single-page path byte-identical). Applied uniformly to G4/G5/G6/G7/YJK/YAE — NOT game-specific.
- Acceptance Criteria: game-free cartridge-ROM video-test harness with per-scenario openMSX A/B at baseline 0.00%; the three broken scroll scenarios flip MISMATCH→MATCH with no regression on the already-correct ones; live human validation on the reported games.
- Unit Tests Required: covered by the M38 A/B harness (`tools/gen-m38-vdp-scroll-cart.py`, `tools/m38-run.ps1`, `tools/m38-ab-diff.py`) — scroll frame dumps s01-s11.
- Integration Tests Required: full deterministic suite re-green (no renderer regression); the M38 scroll dumps sha256-stable.
- Regression Scope: `vdp_frame_renderer` single-page path byte-identity; all bitmap modes G4-G7 + YJK/YAE.
- Status: **Done — released in v1.0.39.** M38 A/B: s03_g4_fine / s04_g4_coarse_fine / s06_g4_multipage MISMATCH→MATCH (0.00%, baseline-level); s01-s08/s10/s11 MATCH; `s09_yjk_baseline` carried a pre-existing 6.25% YJK residual (NOT a scroll scenario) — later root-caused and fixed in **M41 as DEF-M41-YJKOFFSET** (`6044c19`). Human-validated LIVE: Laydock gameplay + F1 `.omr` replay ("the scroll is working!!"). Independently proven byte-identical pre/post the later M39 raster fix (M39 QA §2.2). No cpu/core touch.

## M39 (2026-07-11, backfilled by DEC-0060 — digitized voice + HUD/border framing; TAG v1.0.39)

- Milestone ID: M39
- Title: MSX2+ gameplay polish — digitized voice/speech, vertical/HUD framing, border default
- Spec Owner: `docs/m39-fix-plans.md`; MSX Master Agent (coordinator) — from the human's live-play reports (voice-over silent in Aleste 2/Laydock; HUD letters "chopped at the bottom a bit"; "i prefer non-border like Sony's original")
- Developer Owner: MSX Developer Agent (commits `2405a35`, `7fac03d`, `73968d7`, `53e4eaf`)
- QA Owner: MSX QA Agent (`docs/m39-qa-signoff.md`)
- Scope: three universal, register/raster-level fixes + the repo-maintenance range members. All cpu/core-free.
    - **Voice** (`2405a35`) — digitized voice/speech silent (Aleste 2 Compile voice ~1:15, Laydock): root cause = audio produced ONCE PER FRAME in a batch, collapsing all sub-frame PSG volume writes (software-PCM ~18.7 kHz). Fixed via **PSG `sync_to_cycle()` sync-before-change** (advance the M34 box-average integral to each write cycle BEFORE it mutates — audio analog of the M32 per-line VDP render-sync); default batch path byte-identical. Also routed the previously-unmixed **1-bit DAC** (PPI port-C bit7 keyclick, additive `ClickDac`) into the mix, idle-byte-identical. (sync-before-change is PSG-only so far — SCC/FM still batch, tracked follow-on.)
    - **Border** (`7fac03d` → reverted by `73968d7`) — briefly defaulted to the openMSX-matching framed canvas (to fix 212-line crop/squish), then REVERTED to bare edge-to-edge (Sony original) per the human's explicit preference; `--border` now opts INTO the framed canvas. Frontend/CLI only.
    - **HUD raster seam** (`53e4eaf`) — fixed-top-HUD-over-scrolling-playfield games (Aleste 2 "SC/AREA/ARMO/LEV") had the HUD's bottom ~1px row clipped at the seam: the M32 render-sync committed mid-frame VDP writes at `sync_to_line(line+1)`, one display line early; openMSX rounds with the active-display left margin (PixelRenderer.cc:566-567) → **`line+2`**. Universal raster constant (governs R#23 splits, R#1 BL bands, palette bars). `hbf1xv_machine.cpp` ONLY.
  Preceded by the same-day repo-maintenance sub-phase (unnumbered): `ed419cd` docs sync, `e825b9c` tree-wide comment tidy (ZERO logic), `71b71e1` untrack local-only dev folders from the remote, `09d12b5` README rewrite (de-AI). Tooling built: `tools/omr-to-input-script.py` (openMSX `.omr` replay → `--input-script`, enabling dynamic A/B from a human recording), `tools/m39-voice-ab-analyze.py`.
- Acceptance Criteria: each fix universal + openMSX-A/B-grounded (never per-game); no oracle weakened (re-derived ones strengthened toward openMSX); idle/default paths byte-identical; QA sign-off then human perceptual live-verify before declaring v1.0.39 released.
- Unit Tests Required: `click_dac`, `psg_sync_before_change`, `machine_audio_mixer(_click)` (voice); `sdl3_cli_unit_test` border-default cases (revert); M32 per-line-latch + M34 BL-latch oracles re-derived L+1→L+2 (HUD).
- Integration Tests Required: full deterministic suite (213/213, ZEXALL #128 excluded); M38 scroll dumps sha256-identical pre/post; Metal Gear headless boot→gameplay→transition-blank.
- Regression Scope: all pre-existing audio oracles (idle-byte-identity); the render-sync constant's blast radius (every R#23/R#1/palette raster effect); border presentation oracle; cpu/core behaviorally empty (only `e825b9c` comment-tidy touches z80a).
- Status: **Done — TAGGED v1.0.39 at `53e4eaf` (owner-run push per DEC-0047).** QA (`docs/m39-qa-signoff.md`) = **CONDITIONAL PASS**: clean rebuild both exes; **213/213** fast suite; cpu/core behaviorally empty (comment-only); M38 scroll byte-identical pre/post; M32 line-interrupt untouched + split within tolerance; M34 BL oracle re-derived L+1→L+2 with BL-gating PRESERVED; voice idle-byte-identity intact; border revert oracle correct; Metal Gear headless boots→gameplay (complete HUD)→full-screen transition blank. Conditioned on a perceptual human live-verify checklist (a HUD glyph bottoms in motion / b border default / c Metal Gear seam; voice already confirmed). **Live-verify PASSED** (human confirmed scroll+voice live, HUD+border verified, thumbs-up). Residual non-blocking follow-ons: extend sync-before-change to SCC/FM (PSG-only now); R#18 display-adjust unimplemented (neutral in tested games); the s09 YJK residual (closed by M41).

## M41 (Opened + closed 2026-07-11, DEC-0059 — final system-wide production-QA A/B sweep; release target v1.0.40)

- Milestone ID: M41
- Title: Final production QA — system-wide openMSX A/B parity sweep + inline fixes for the defects it found
- Spec Owner: docs/m41-production-qa-plan.md; MSX Master Agent (coordinator)
- Developer Owner: MSX Developer Agent (S1-S4 measurement + two inline fixes)
- QA Owner: MSX QA Agent (docs/m41-production-qa-signoff.md)
- Scope: The human's "final PRODUCTION QA before release" — a deterministic openMSX A/B sweep over 8 subsystems (keyboard, joystick, PSG, FM/OPLL, BASIC execution, screen modes, sprites, disk I/O), MATCH/KNOWN-DELTA/BLOCKED taxonomy. One additive src enabler (`--input-script JOY=` verb, `3976bc4`, KEY path byte-identity guarded). Two real device defects found during measurement + fixed inline (both src/devices, NOT cpu/core → per-edit ZEXALL withheld):
    - **DEF-M41-YJKOFFSET** (`6044c19`, src/devices/video/vdp_frame_renderer.cpp) — V9958 YJK/YAE (SCREEN 10/11/12) rendered 4px too far LEFT vs openMSX; fixed via content_lead=+4px on the YJK/YAE compose path (non-YJK modes untouched). +4 corroborated by fMSX 6.0 + V9958 fact-sheet §5; disagrees with openMSX-21.0-source → two-reference-disagreement rule applied.
    - **DEF-M41-PSGENV** (`6f6fb39`, src/devices/audio/psg_ym2149.cpp) — PSG YM2149 hardware envelope ran at HALF rate (missing openMSX `count+=duration*2` pairing with period=2*value); fixed to mirror openMSX exactly → envelope EP-exact (EP=2048 3.414 Hz, EP=1024 6.827 Hz == openMSX == datasheet).
  Ride-alongs already on main from the same session: `6ef652b` --fast-disk (WD2793 turbo + Alt+D), `73ce71c` Right-Ctrl `:/*` keymap. Local tool: format-blank-disk.ps1 optional -BootSectorFrom bootable-blank path.
- Acceptance Criteria: measure all 8 subsystems A/B; escalate + fix true device MISMATCHes universally (never per-game); every non-MATCH a legitimately irreducible KNOWN-DELTA or BLOCKED with justification; no oracle weakened; clean-rebuild gates + release-checkpoint ZEXALL sweep green; QA sign-off before the v1.0.40 tag.
- Unit Tests Required: video_vdp_frame_renderer_yjk_unit_test (YJK, non-tautology-proven); audio_psg_ym2149_unit_test envelope rate (first-principles, re-derived); input_script_unit_test JOY verb.
- Integration Tests Required: the full fast ctest subset re-green (215/215); disk SAVE/LOAD/BSAVE/seq/random on-disk byte round-trip vs openMSX write-back.
- Regression Scope: full deterministic suite + the ZEXALL/ZEXDOC full sweep run ONCE as the release checkpoint (PASS, 2069 s, 0 markers; debug/m41/zexall/m41-zexall-sweep.log); mixer oracles unaffected (fixed-volume, no envelope); tone/noise pitch unregressed.
- Status: **Done — QA CONDITIONAL PASS (msx-qa 2026-07-11), conditions C1-C3 discharged by coordinator 2026-07-11.** Four hard gates GREEN (clean rebuild both exes; fast ctest 215/215; ZEXALL/ZEXDOC full sweep PASS 0-marker; asset gate PASS). 3 M41 commits confirmed src-scoped (no cpu/core), no AI trailer. Non-tautology proven (adversarial YJK mutation → 11 unit fails). Final parity matrix: **30 true MATCH + 1 partial (H3) + 4 KNOWN-DELTA (FM sample-exact attack/rhythm = license-sensitive-scope ban, NukeYKT tables never transcribed, correctly not claimed) + 3 BLOCKED (joystick-port unscriptable on openMSX headless; decode proven A/B-equal via STICK(0) control) = 38 dispositioned** — every non-MATCH legitimately irreducible, no masked defect. Conditions: C1 (H3 disk row overstated openMSX on-disk parity — the committed omx.dsk was byte-identical to the blank start image = openMSX BSAVE keystroke dropped, wrote no file; re-dispositioned honestly as our-side-correct + openMSX-on-disk-BLOCKED; write parity proven by H1/H4/H5 SHA-equal), C2 (this ledger refresh + DEC-0059), C3 (DEF-M41-YJKOFFSET RESOLVED folded into debug/m41/ab/m41-ab-report.md, S2 m10/m12 rows MISMATCH→MATCH) — ALL discharged. Release target **v1.0.40** at HEAD `6f6fb39` (5 commits ahead of the pushed v1.0.39 tag 53e4eaf); tag+push owner-run per DEC-0047. NOTE ledger was stale since M37: v1.0.39 (MSX2+ gameplay fixes) shipped between M37 and M41 without its own milestone blocks (known documentation debt; fixes recorded in project memory + commit history).

## M42 (2026-07-12, DEC-0061 — opt-in `--ram 64|128|256|512`; released v1.0.41)

- Milestone ID: M42
- Title: Opt-in main-RAM sizing (`--ram <64|128|256|512>`), default 64 KB stock
- Spec Owner: DEC-0061; MSX Master Agent (coordinator) — from the human's request ("add option --ram 128, 256, 512"; "how far... up to 4M?")
- Developer Owner: MSX Developer Agent (commit `3f69786`)
- QA Owner: MSX QA Agent (`docs/m42-qa-signoff.md`, `docs/m42-openmsx-ram-ab.md`)
- Scope: opt-in `--ram <64|128|256|512>` CLI (headless + SDL3) sizing HB-F1XV main RAM; default 64 KB = stock spec (byte-identical when absent/64). `MemoryMapperRam` parameterized by the backing region (num_segments = ram.size()/16 KB, replacing the compile-time kSegments=4); `Hbf1xvMachine(dram_bytes=64*1024)` config-driven; shared `parse_ram_kb` enum validator on both exes. The authentic S1985 5-bit read-back mask (0x1F) is UNCHANGED for every size — RAM detection works via the fold-based segment mirroring. 512 KB = the 5-bit read-back ceiling (32 segments); >512 KB is NOT offered internally (needs an external RAM cartridge — separate future work). Frontend + machine + devices/memory only; ZERO src/core & src/devices/cpu.
- Acceptance Criteria: default-64 byte-identical; fold/mirroring correct at 128/256/512 (distinct within fitted range, mirror past it, all-distinct at the 512 ceiling); CLI enum accept + reject policy; a live end-to-end detection proof; clean rebuild + fast ctest; no cpu/core touch (ZEXALL withheld).
- Unit Tests Required: `devices_memory_mapper_ram_unit_test` parameterized fold cases (64/128/256/512); `frontend_sdl3_cli_unit_test` --ram accept/reject + shared parse_ram_kb.
- Integration Tests Required: `hbf1xv_ram_size_integration_test` (per-size dram_size + bus-fabric segment detection + --ram 512 boot smoke); `hbf1xv_ram_detect_boot_integration_test` (QA — real BIOS boot + mapper probe, openMSX A/B parity).
- Regression Scope: default-64 memory/mapper/machine oracles byte-identical; debug dump/snapshot follow dram_.size(); cpu/core behaviorally empty.
- Status: **Done — QA PASS (unconditional), released target v1.0.41 @ `3f69786` (owner-run tag+push per DEC-0047).** Clean rebuild both exes; fast ctest **216/216** (217/217 with the QA A/B test); default-64 byte-identity proven (memory-regions + mapper readback oracles unchanged; `& (4-1)==& 3`); commit src-scoped, zero cpu/core, no AI trailer; non-tautology proven (fold mutation → 34 case failures, restored byte-identical per DEC-0049). **End-to-end openMSX A/B PARITY** (`docs/m42-openmsx-ram-ab.md`): an internal-512 HBF1XV override (stock XML, Main-RAM MemoryMapper size 64→512, NOT an external ext) vs our engine after a real `bios/` boot + live Z80 → both report 32 distinct mapper segments at 512 KB and 4 at stock 64 KB, byte-for-byte (distinct counts + exact fold-aliasing sequence). CLI rejection verified both exes; --help/README label 128/256/512 as opt-in non-stock (512 KB = 5-bit ceiling). Residual (Low, non-blocking): non-stock A-5 power-on fill + larger debug dumps, both deterministic and A/B-neutral.

## M43 (2026-07-12, DEC-0062/0063/0064 — FM-PAC CALL/screen/SRAM accuracy; QA PASS; tag target v1.1.0)

- Milestone ID: M43
- Title: FM-PAC BASIC-extension accuracy — CALL dispatch (void), GRAPHIC2/3 AND-mask screen fix, SRAM bit-exact
- Spec Owner: docs/m43-planner-package.md; MSX Master Agent (coordinator)
- Developer Owner: MSX Developer Agent (Slice 1 diagnostic, Slice 2 `a733d65`, Slice 3 `9f6eb8c`)
- QA Owner: MSX QA Agent (docs/m43-qa-signoff.md)
- Scope: from the human's FM-PAC `CALL FMPAC` "Syntax error" report. Slice 1 (no code) DIAGNOSTIC: DEF-M43-CALLDISPATCH = VOID (dispatch works at openMSX parity; the report was a headless frame-capture staleness artifact + the human's prior non-canonical 16 KB ROM; slot fabric verified correct + untouched — the diagnostic-first gate avoided an unnecessary edit to the address fabric). Slice 2 (`a733d65`, src/devices/video): DEF-M43-FMPAC-SCREEN — V9958 GRAPHIC2/3 table addressing corrected additive→openMSX AND-mask (mode-aware; universal SCREEN 2/4 fix). Slice 3 (`9f6eb8c`, src/devices/cartridge): FM-PAC SRAM openMSX-exact format (8206-byte, 16-byte header + 8190 data) + lossless migration of legacy raw-8192 saves + register parity + M36 asset test reconciled to the canonical 64 KB ROM.
- Acceptance Criteria: FM-PAC manager renders matching openMSX; all screen modes still bit-exact; SRAM format byte-identical to openMSX + lossless migration of the human's real save + round-trip; no oracle weakened; non-tautology proven.
- Unit Tests Required: video_vdp_frame_renderer_tile_modes (GRAPHIC2 canonical R#4=0x03/R#3=0xFF + G3); cartridge_fmpac_sram_persistence (format/migration/round-trip/fail-safe).
- Integration Tests Required: m36_fmpac_cartridge (reconciled) + sdl3_app_fmpac_sram round-trip.
- Regression Scope: all 13 screen-mode openMSX A/B (0.000%); FM-PAC SRAM data-safety; ZEXALL WITHHELD-substituted (git-diff-proven zero cpu/core since v1.0.40's clean sweep).
- Status: **Done — QA FINAL PASS (docs/m43-qa-signoff.md), coordinator release decision tag v1.1.0 @ `9f6eb8c` (owner-run tag+push per DEC-0047; recommended live human re-verify first).** Clean rebuild both exes; fast ctest 218/218; asset gate PASS; commits src-scoped (video + cartridge), zero cpu/core, no AI trailer. No-regression: 13 modes 0.000% vs openMSX, changed renderer (m2_g2/m4_g3) byte-perfect. FM-PAC manager UI renders (VRAM byte-identical to openMSX; QA-corrected residual 0.979% = a golden mid-palette-load artifact, NOT a defect). SRAM: data-safety PASS on all 5 points, lossless migration verified on the human's real save, hardening exceeds openMSX, no data-loss path. Non-tautology proven on the mask + the migration. Residual all LOW/INFO (palette-load timing, dump staleness, write-mask latent, build-sdl3/ hygiene, v1.0.42/43/44 doc-debt).

## v1.0.42–v1.0.44 (2026-07-11/12 — coordinator UX patch releases, no milestone process)

These three tagged patch releases were small frontend/docs commits made directly by the coordinator (not planner→developer→QA milestones); recorded here for version-history completeness (the doc-debt QA flagged in DEC-0064). All frontend/docs only, zero cpu/core, no AI trailer.
- **v1.0.42 (`f6e745e`)** — SDL3 startup summary (loaded config: RAM/BIOS/disk[writable]/cart/video/fast-disk + the in-window hotkeys) replacing the internal-jargon launch banner; de-jargoned the `--help` footer.
- **v1.0.43 (`9d853d0`)** — the startup summary's BIOS line now shows the resolved absolute path + the actual `.rom` files present (enumerated, not hardcoded).
- **v1.0.44 (`36babf2`)** — novice-friendly `--help` rewrite (intro, GETTING STARTED copy-paste examples, options grouped by purpose, IN-WINDOW HOTKEYS, GOOD TO KNOW tips). Raw-string literal for readability.

## M44 (2026-07-12, DEC-0065/0066 — command-engine render-sync; PHASE 1 committed, PHASE 2 deferred)

- Milestone ID: M44
- Title: V9958 command-engine render-sync (DEF-M44-CMDSYNC) — Laydock 2 HUD flicker
- Spec Owner: docs/m44-planner-package.md + docs/m44-phase2-plan.md; MSX Master Agent
- Developer/QA Owner: MSX Developer Agent (Phase 1 `a27a1c4`); QA folded into the developer's A/B (219/219 + 13-mode no-regression + non-tautology)
- Scope: from the human's Laydock 2 flicker report + system-wide render-sync review. Confirmed root cause (`.omr` A/B vs openMSX): the V9958 command engine writes raster-visible VRAM outside the M32 render-sync seam. PHASED per DEC-0065.
- Status: **Phase 1 Done + committed (`a27a1c4`), NO REGRESSION (219/219, 13 modes byte-identical, non-tautology proven), but does NOT clear the Laydock flicker — that is the deferred Phase-2 CE-pacing item (instrumented: HUD commands run below the beam; the residual is the unpaced busy-wait blit loop from 0-cycle CE).** Phase 1 is the observation-timing FOUNDATION + is safe/inert by default. **Phase 2 DEFERRED by the human** — full resumable plan in `docs/m44-phase2-plan.md` (2a CE-duration/pacing = the actual fix; 2b full access-slot cycle timing = bit-parity). v1.1.1 held until Phase 2 lands. **UPDATE 2026-07-13 (DEC-0069 + DEC-0069-OUTCOME): Phase 2a DONE + committed (`572d9cc`) — the CE busy-duration pacing model (license-safe: `ticks_per_unit` composed ONLY from the already-blessed `VdpAccessDelta` values, NOT the banned VDPAccessSlots.cc table; CE-busy overlay at the S#2 read only, zero src/core). LIVE HUMAN-CONFIRMED: Laydock 2 severe HUD flicker GONE + cut-scenes now play at NORMAL speed (were fast-forwarded — the CE-pacing win). The residual label/line jitter is the per-raster slot-accuracy (Phase 2b) boundary — human-ACCEPTED ("i'm good with what we have"), Phase 2b DECLINED. New CE-duration oracle; fast subset 222/222; render byte-identical pre==post; all M22/M32/Phase-1/VR-HR oracles unmodified. M44 CLOSED.** Separate pre-existing sprite-mode delta (sp2_double/sp3_collision vs openMSX) noted for later, unrelated to M44.

## M45 (2026-07-12, DEC-0067/0068 — WD2793 write DRQ data-integrity fix; QA CONDITIONAL PASS; v1.1.1)

- Milestone ID: M45
- Title: WD2793 disk-WRITE corruption fix (DEF-M45-WRITEDRQ) — the YS II save-corruption bug
- Spec/Dev/QA Owner: coordinator investigation + MSX Developer Agent (`4a8f8e0`) + MSX QA Agent (`docs/m45-qa-signoff.md`)
- Scope: from the human's critical report (YS II in-game save corrupted disk2; openMSX crashes identically → corruption persisted in the .dsk). Root cause (confirmed, real-object harness + forensic diff vs pristine): the WD2793 accurate-mode (`!fast_disk_`) WriteSector path modeled DRQ as a rigid absolute-cumulative deadline and injected 0x00+LOST_DATA (consuming data slots → early finish + dropped tail) whenever the CPU trailed that deadline — corrupting normally-timed game writes (interrupts-enabled loop + mid-transfer stall). REVERSED the --fast-disk suspicion (fast mode was code-immune + byte-perfect). Fix: re-modeled to the openMSX/real-WD2793 edge DRQ handshake + one-byte pipeline + first-byte CHECK_WRITE gate; removed the `!fast_disk_` gate (identical with/without --fast-disk). `src/devices/fdc/wd2793.{cpp,h}` only, zero cpu/core.
- Status: **Done — QA CONDITIONAL PASS.** 206/206 headless (SDL3-gated delta only), all FDC/disk oracles green + new `wd2793_write_stall` test; non-tautology + forensic match definitive (revert reproduces the exact disk2 105-zero signature); BIOS-write byte-perfect + mode-independent; openMSX A/B BLOCKED (harness DI;HALT — acceptable given flat-.dsk architecture). Release v1.1.1 @ `4a8f8e0` GATED on the owner's live YS II re-save-and-load test (real-world closure). Residual (backlogged, non-blocking): the fix is CPU-paced (safer than openMSX's scheduled-tick LOST_DATA substitution, invisible to real software) — a full cycle-accurate write model is a future item. Human's corrupt save unrecoverable → restore pristine ys2-d2.dsk (pending OK). **UPDATE 2026-07-13: M45 LIVE-CONFIRMED CLOSED + follow-up M45b (DEC-0070, `0f648d7`).** The v1.1.1 `4a8f8e0` build's first-byte CHECK_WRITE window was itself too short (~1140 cyc) → aborted YS II's legitimate save WRITING NOTHING → game hang (worse than the original corruption; human live "save hangs" report + crash capture). Fixed as M45b (cherry-picked to main): first-byte DRQ now rotational-search-relative (same as the read path) + a full-revolution CHECK_WRITE window; the M45 edge-handshake corruption fix kept intact; NEW `wd2793_write_firstbyte` test + adversarial-revert proof (21 fails). Because the abort wrote nothing, the human's `ys2-d2.dsk` stayed byte-identical to pristine (the corrupt `disk2.dsk` leftover is the separate earlier-episode artifact). **LIVE HUMAN-CONFIRMED: YS II save → load → play → die → continue works WITH AND WITHOUT `--fast-disk`.** Combined main rebuild GREEN (FDC/disk 15/15, fast subset 222/222). **v1.1.1 READY** — owner-run tag+push.

## M46 (2026-07-13, DEC-0071 — convenience-first launch defaults + `--stock` + `--slotN` rename; QA CONDITIONAL PASS; v1.1.2)

- Milestone ID: M46
- Title: Convenience-first launch defaults + `--stock` escape hatch + `--cartN`→`--slotN` rename + enriched banner + FM-PAC slot-2 auto-load
- Spec/Dev/QA Owner: MSX Master Agent (coordinator, DEC-0071) + MSX Developer Agent (`70db39f`) + MSX QA Agent (`docs/m46-qa-signoff.md`)
- Scope: from the human's "system that works" convenience request. The out-of-box machine becomes a ready-to-game config (DEFAULT CONFIG ONLY — the hardware spec table + the CAPABILITY to run stock are UNCHANGED and one flag away): `--ram` default 512 KB, `--fast-disk` default ON, FM-PAC auto-loads into SLOT 2 (from `roms/fmpac.rom`, SRAM auto-persists), a one-shot `--stock` for the authentic bare machine, the `--cart1/2`→`--slot1/2` rename (`--cartN` kept as silent aliases), and an enriched startup banner. Frontend + FM-PAC-slot2 machine wiring only; ZERO cpu/core. CRITICAL anti-drift: the machine ctor + `Sdl3AppConfig` struct stay stock (direct-construction tests unaffected; the M42 `ram_size`/`ram_detect` byte-identity oracles preserved) via a CLI-resolver seam; the openMSX A/B harness + CLI-launching tests pass explicit stock flags.
- Status: **Done — QA CONDITIONAL PASS (DEC-0071 OUTCOME).** Implemented `70db39f`. Trimmed/scoped gate (DEC-0071 test-cadence addendum, ZEXALL withheld — empty cpu/core diff): 136/143, the 7 fails ALL confirmed pre-existing asset-absence from the pre-M46 asset reorg (roms/aleste.rom, roms/metalgear.rom, disks/msxdos22.dsk moved/removed), zero M46 code. Anti-drift verified 3 levels; resolver/FM-PAC/alias/banner pinned non-tautologically; FM-PAC graceful-skip paths preserve DEC-0050 "NO S-RAM" on non-load; 9-tool `--stock` migration applied. AC-10 (interactive `CALL FMPAC` from slot 2 + in-game SRAM) owner-gated → human live-checked + directed release. **v1.1.2 released** — README bumped + doc-synced (new flags/defaults, `--slotN`, `--stock`); commits `70db39f` (src) + `c00c7d1` (README v1.1.2 doc); tag v1.1.2 @ `c00c7d1`, owner-run push per DEC-0047. Deferred local-only hygiene: 7 stale asset-path test fixtures → skip-when-absent (folded into M47).

## M47 (2026-07-13→14, DEC-0072 — YS II disk-save crash investigation + device-fidelity hardening; autonomous)

- Milestone ID: M47
- Title: YS II 8-slot disk-save crash — root-cause + universal fix (DEF-M47-DISKWRITE), then FINAL VERDICT (inherent to the game/disk) + CPU/VDP/PSG timing-parity fixes
- Spec/Dev/QA Owner: MSX Master Agent (coordinator, DEC-0072, autonomous run) + MSX Developer Agent(s) + 4-specialist device-fidelity audit + MSX QA Agent (`docs/m47-qa-signoff.md`)
- Scope: from the human's report that YS II in-game disk saves to later slots (a full 8-slot save area) still crash on reload, with the authorization to run autonomously "whatever it takes until QA passes." System-wide-review-first + universal-fixes (never game-specific).
- Status: **CLOSED — DEC-0072 FINAL VERDICT (2026-07-14): the crash is INHERENT to the game/disk, NOT an emulator defect; the emulator is VINDICATED.** Trajectory: (1) an initial WD2793 write-model improvement landed (`a27be57`, the never-drop write model + H4 CHS-latch — a real, KEPT correctness improvement, QA CONDITIONAL PASS `docs/m47-qa-signoff.md` gates 71/71 + 144/144), but the owner's live 8-slot save STILL crashed on reload. (2) A decisive independent test settled the cause: the identical crash reproduces on the owner's LOCAL Windows openMSX (a separate accuracy-focused emulator) from a FRESH game on a VANILLA disk across MULTIPLE game sources → the fault is YS II's own save/load code mishandling a full 8-slot save area (buggy routine shared across the common crack lineage these `.dsk` images descend from). PROOF the FDC is clean: env-gated per-byte write logger = 0 substitutions + byte-identical-to-disk, corruption confined exactly to the 8 save slots (LBA 9-16, no FAT/dir spill); independent WD2793 datasheet audit ("no write-corruption defect"); openMSX itself crashes loading OUR produced disk. (3) An intermediate "our CPU/VDP timing" hypothesis (the crash is timing-*sensitive* per a `--vdp-cmd-bias` sweep) was CORRECTED — openMSX at correct timing also crashes, so timing was a distraction, not the cause. Verdict doc: `docs/m47-dec0072-timing-fidelity-verdict.md`. Practical guidance: 1-2 disk slots or FM-PAC SRAM saves (100% reliable) work; don't fill all 8 disk slots on these images.
- Device-fidelity hardening (DEC-0074 — independent correctness wins surfaced by the audit, NOT the YS II cause; grounded tier-1 hardware spec + openMSX-A/B tier-2 confirmation per the reference-precedence directive DEC-0073): **(1) CPU interrupt/NMI-acknowledge timing** — IM1 13T / IM2 19T / NMI 11T (were 14/20/12): the S1985 +1 M1 wait gates on the opcode-fetch /M1+/MREQ, not the ack's /M1+/IORQ; `tick_refresh_only()` split + NMI R-tick fix; 4 oracles updated; `references/fact-sheets/Zilog Z80A CPU.md` §5 corrected. **(2) V9958 command-engine** — LINE 88→112, per-line end-of-line overhead (HMMV+56/HMMM+64/LMMV+64/LMMM+64/YMMM+0), LMCM/LMMC/HMMC TR per-unit pacing. **(3) PSG** — Tone/Noise counter clamp-to-period-1 on a period-shrink write. The full VDP access-slot CONTENTION model (154/88/31 + CPU↔command stealing) stays DEFERRED — HIGH risk + license-sensitive ~340-entry tables (C1/D4 UNBUILDABLE-WITHOUT-FABRICATION ban). Verification: full suite **228/228** incl. the ZEXALL/ZEXDOC sweep after the CPU work; VDP suite green. Also this session: the reference-precedence directive (DEC-0073) authored + propagated (CLAUDE.md + all 5 agents + all 5 commands + project-baseline.md + guardrails.md), and the 7 stale-asset test fixtures → skip-when-absent (M46 residual). Version/tag/push are the owner's call per DEC-0047 (README reads v1.1.3).

## M48 (2026-07-14, DEC-0075 — V9958 command-engine ↔ CPU VRAM access-slot CONTENTION model, ban-respecting formula; DONE — released v1.1.5)

- Milestone ID: M48
- Title: V9958 command-engine ↔ CPU VRAM access-slot CONTENTION model (deferred backlog **D4/C1** remainder) — COMPACT FORMULA / parametric, NOT the openMSX per-slot POSITION table
- Spec Owner: MSX Planner Agent (`docs/m48-planner-package.md`)
- Developer Owner: MSX Developer Agent (TBD at kickoff)
- QA Owner: MSX QA Agent (TBD at kickoff)
- Scope: model the remaining CPU↔command-engine VRAM access-slot CONTENTION that v1.1.4/DEC-0074 left DEFERRED. v1.1.4 landed only the CONTAINED per-command cost corrections (LINE 88→112, per-line end-of-line overhead HMMV+56/HMMM+64/LMMV+64/LMMM+64/YMMM+0, LMCM/LMMC/HMMC TR per-unit pacing) in `src/devices/video/vdp_command_engine.*`+`v9958_vdp.*`. M48 adds, as a pure timing/STATUS overlay on the existing S#2 CE/TR busy windows (VRAM mutation stays ATOMIC, M44 render-sync untouched): **Slice 1** — a per-line command-throughput cap derived from the three tier-1 fact-sheet §8 slot COUNTS (154 display-off / 88 sprites-off / 31 sprites-on), sprite-aware, superseding the DEC-0069 empirical `kActiveDisplaySlotFactorPercent` placeholder; **Slice 2** — CPU-concurrent-access command slowdown (CPU-priority rule: each CPU `#98` VRAM access during a busy command steals one slot, extending the command's CE window; the CPU access itself is NEVER stalled/delayed/dropped → CPU timing byte-identical). **HARD BAN (non-negotiable, `feedback_license_sensitive_scope.md`/guardrails/CLAUDE.md C1/D4):** the openMSX `VDPAccessSlots.cc` ~340-entry per-slot POSITION tables are BANNED — read for EFFECT only, NEVER transcribed/re-derived into `src/`. The ONLY legal numeric inputs are 154/88/31, 1368, 6. `/WAIT` not wired on HB-F1XV → the command→CPU stall direction is out of scope on tier-1 hardware grounds. Cycle-exact per-slot phase / dropped-request / raster-tearing remain UNBUILDABLE-WITHOUT-FABRICATION (C1/D4 remainder).
- Acceptance Criteria: see `docs/m48-planner-package.md` §4 (AC-1..AC-6 slice-level + EG-1..EG-9 evidence gates). Highlights: AC-4 (CPU timing byte-identical, `git diff v1.1.4 -- src/devices/cpu src/core` EMPTY + M21/M22 fixtures unchanged); EG-4 full ctest green (228/228 + new oracles); **EG-5 full ZEXALL/ZEXDOC sweep MANDATORY at QA** (the withhold-substitute shortcut is NOT taken — CPU↔VRAM path touched); EG-6 all 13 M41 screen modes still 0.000% vs openMSX; EG-7 command-engine timing A/B (3 slot states + concurrent-`OUT` scenario); EG-8 non-tautology (oracles re-derived from tier-1 constants, adversarial mutation fails, no existing oracle weakened); EG-9 ban attestation (QA greps `src/` for any smuggled slot table).
- Unit Tests Required: Slice-1 slot-availability factor oracle (154/88/31 → factor, tier-1-re-derived); Slice-1 display-off byte-identity regression (AC-2); Slice-2 CPU-`#98`-steal oracle (N accesses → +N·slot_cost) + adversarial (access outside window has no effect); determinism + `--vdp-cmd-bias` default-0 regression.
- Integration Tests Required: CPU-T-state-invariance + M21/M22 back-to-back-`OUT` fixture byte-identity (extend the M23 non-gating pattern); a command + concurrent-CPU-`#98` scenario proving the CE window extends while CPU timing does not.
- Regression Scope: all prior suites green (228/228 baseline); full ZEXALL/ZEXDOC sweep PASS at QA; no oracle weakened; QA sign-off before closure; normal human-release-decision gate (HIGH blast radius; no auto-close). DEC-0069 live-validated titles (Laydock 2/F1/YS II) non-regressed headlessly or owner spot-checked.
- Status: **Done (2026-07-14) — released v1.1.5.** Both slices shipped (Slice 1 per-line throughput cap `slot_factor = 154 / S_effective` from the 154/88/31 counts; Slice 2 CPU-`#98` concurrent one-directional slot-steal). QA **PASS** (`docs/m48-qa-signoff.md`): full ctest **231/231**, **full ZEXALL/ZEXDOC sweep PASS** (1662 s, 0 markers — the CPU↔VRAM path was touched, so the mandatory sweep ran), 13-mode screen A/B + command-timing A/B PASS, ban-attestation clean (only 154/88/31/1368/6 in `src/`; no per-slot POSITION value transcribed), CPU byte-identical (empty `src/core`+`src/devices/cpu` diff). Tag **v1.1.5**. The cycle-exact per-slot remainder stays the C1/D4 UNBUILDABLE-WITHOUT-FABRICATION ceiling (banned openMSX position table).

## M49 (2026-07-14, DEC-0076 — per-line LIVE sprite attribute/register fetch; closes backlog D9; DONE — released v1.1.6)

- Milestone ID: M49
- Title: Per-line LIVE sprite attribute/register fetch — progressive `SpriteEngine::check_until()` replacing the frame-atomic `recompute_frame()` (closes backlog **D9**)
- Spec Owner: MSX Planner Agent (`docs/m49-planner-package.md`)
- Developer Owner: MSX Developer Agent (TBD at kickoff)
- QA Owner: MSX QA Agent (TBD at kickoff)
- Scope: fix the ONE architectural defect (backlog D9). The sprite subsystem is FRAME-ATOMIC — `SpriteEngine::recompute_frame()` (`src/devices/video/sprite_engine.cpp:74-301`) runs ONLY from `V9958Vdp::on_vsync()` (`src/devices/video/v9958_vdp.cpp:685-705`), snapshotting the whole `control_regs` file + all VRAM at end-of-frame and computing every output line's visibility / 5th-sprite / position at once (registers baked frame-final: R#23 vscroll `:138`/`:183`, R#8 SPD+TP `:103-105`/`:247`, R#1 BL/SI/MAG `:102`/`:108-110`, R#5/R#11 attr base `:116-117`/`:132`, R#6 pattern base `:136`). The BACKGROUND is already per-line-LIVE via the M32 seam `Hbf1xvMachine::VdpRenderSyncAdapter::on_before_state_change()` (`src/machine/hbf1xv_machine.cpp:53-124`, +2-line margin `:101`). Space Manbow + Laydock 2 do CYCLE-TIMED mid-frame VDP reprogramming (NOT line interrupts: R#0 IE1=0, R#19=0xDB) to split a fixed HUD over a scrolling playfield → the end-of-frame snapshot captures the bottom/playfield state → top-region sprites culled/mis-positioned + flicker. Tier-1 `references/fact-sheets/Yamaha V9958 VDP.md` §6 line 120 ("sprite data for line N is fetched during line N−1"), §7 line 126 / §10 line 178 (R#23 live per-line offset). Tier-2 corroboration (effect only, NEVER copy code): openMSX `SpriteChecker::sync()/checkUntil()` incremental check + per-write sync-before-change in `VDP.cc` — the SAME protocol our background M32 seam already has. **Four slices:** **S1** — a stateful `SpriteEngine::check_until(line)` watermark pass (mirror `VdpScanlineAccumulator`, `src/devices/video/vdp_scanline_accumulator.h:61-145`) reading `control_regs`+VRAM AT the raster point, replacing the single sweep; per-line formulas + mode-2 base-mask + planar addressing preserved VERBATIM; R#1 BL / R#8 SPD / mode-0 gates become per-line-live. **S2** — drive `check_until(line + 2)` from the EXISTING M32 seam on a sprite-relevant write (R#1/R#5/R#6/R#8/R#11/R#23 or sprite-attr VRAM) with the SAME +2 boundary + M44 watermark-direction discipline (`hbf1xv_machine.cpp:118-123`) so the sprite split coincides with the background split; finalize + roll at `on_vsync()`. **S3** — feed the DEC-0031 line-granular collision / 5th-sprite / S#0 / S#3-S#6 machinery (`sprite_engine.cpp:238-325`) from the progressive pass, BYTE-IDENTICAL. **S4** — keep R#23 in the sprite Y formula, per-line-LIVE (NOT removed); confirm direction via openMSX A/B on Space Manbow. ALGORITHM change (D4/C1 license-sensitive-TABLE ban does NOT apply; the "never copy openMSX `SpriteChecker` code" rule DOES). Edits confined to `src/devices/video/{sprite_engine,v9958_vdp}.*` + `src/machine/hbf1xv_machine.cpp`; ZERO `src/core`, ZERO `src/devices/cpu`.
- Acceptance Criteria: see `docs/m49-planner-package.md` §4 (AC-S1..AC-S4 + EG-1..EG-9). Highlights: AC-S1 no-mid-write byte-identity vs today's `recompute_frame()` + per-line BL/SPD gate; AC-S2 sprite split == background split at L+2 (M44 watermark discipline held); AC-S3 DEC-0031 collision/5th-sprite/S#0/S#3-S#6 byte-identical; AC-S4 R#23 per-line-live A/B. EG-2 FULL ctest (every existing sprite/M22/M32 oracle UNMODIFIED + new); **EG-3 FULL 13-mode 0.000% screen A/B vs openMSX (MANDATORY, HIGH blast radius)**; **EG-4 Space Manbow + Laydock 2 headless before/after frame captures under `debug/m49/` (MANDATORY)** + Space Manbow R#23-direction openMSX A/B; **EG-5 DEC-0031 collision-byte-identity gate (MANDATORY)**; EG-6 determinism frame-hash equality; EG-7 asset/checksum; EG-8 ZEXALL NOT required (zero cpu/core diff); EG-9 ban-attestation grep of `src/`.
- Unit Tests Required: `check_until` no-mid-write byte-identity vs `recompute_frame` (new); per-line BL/SPD/mode-0 gate (mid-frame toggle changes only lines after the commit boundary, new); progressive-vs-atomic collision-event equality (new, adversarial); existing `video_sprite_engine_mode1/mode2/mode2_attribute_masking/collision_relatch` + `video_vdp_frame_renderer_sprites`/`_sprite_edge_clip` green UNMODIFIED; reuse `tests/parity/m22_sprite_*` probes.
- Integration Tests Required: split-coincidence system oracle (sprite visibility split at L+2 equals the background accumulator split); M44 command-row-sink + sprite watermark coexistence; existing `hbf1xv_m22_sprites_integration`, `hbf1xv_m22_sprites_command_engine_system`, `hbf1xv_m32_split_screen_system`, `hbf1xv_m32_line_interrupt_integration`, `hbf1xv_m32_per_line_latch_integration` green UNMODIFIED.
- Regression Scope: all prior sprite/M22/M32/screen suites green; full 13-mode 0.000% screen A/B; DEC-0031 collision byte-identity; determinism; NO oracle weakened (adversarial mutation must fail ≥1 oracle). QA sign-off before closure; NORMAL human-release-decision gate (HIGH blast radius; no auto-close). Owner live spot-check of Space Manbow + Laydock 2 + a sprite-heavy non-split shmup recommended. Open sub-question flagged: whether Laydock 2 scoreboard noise is purely sprite or ALSO a background facet (Laydock 2 repro required in dev; a background facet → SEPARATE new backlog row, NOT M49 expansion). Residual ceiling (honest): line-granular +2 rounding kept; cycle-exact intra-line splits (Aleste 2) remain LINE-quantized (M44 Phase 2b human-accepted boundary; cycle-exact needs the banned access-slot table).
- Status: **Done (2026-07-14) — released v1.1.6.** Per-line-live `SpriteEngine::check_until(line)` watermark pass (driven from the existing M32 seam with the +2 boundary) replaced the frame-atomic `recompute_frame()`; rendered sprite plane now splits per-line, while collision / 5th-sprite / S#0 / S#3-S#6 were kept FRAME-ATOMIC (DEC-0031 byte-identical — an early wiring of collision through the per-line pass diverged Laydock 2 and was corrected). Closes backlog **D9**. Space Manbow + Laydock 2 top-region sprites no longer culled; owner LIVE-validated ("works perfectly, this must be the release version"). Tag **v1.1.6**. Closure evidence = **DEC-0076** + owner live-validation + tag + README v1.1.6 notes (no separate `m49-qa-signoff.md` — this shipped on owner live-sign-off, honestly recorded). Residual: line-granular +2 rounding; cycle-exact intra-line splits remain LINE-quantized (needs the banned access-slot table). A later production-hardening increment added the tunable enemy-flicker knob (v1.1.7 shipped the phosphor-persistence softener; the flicker itself is genuine 8-sprites-per-line hardware behavior, knob shipped as-is per owner).

## M50 (2026-07-14, DEC-0077 — externalized strict-XML configuration; production hardening; DONE — released v1.2.0)

- Milestone ID: M50
- Title: Externalized strict-XML configuration — move DEFAULTS/KNOBS out of hardcoded C++ into a strict `sony_msx_hbf1xv.xml` (configurable without recompiling)
- Spec Owner: MSX Planner Agent (`docs/m50-planner-package.md`)
- Developer Owner: MSX Developer Agent (TBD at kickoff)
- QA Owner: MSX QA Agent (TBD at kickoff)
- Scope: from the owner's production-hardening request — a strict `sony_msx_hbf1xv.xml` so the machine is configurable WITHOUT recompiling. Config FOUND → parse+validate+use; NOT found → ONE WARNING line + built-in defaults, continue (runs standalone). Confirmed scope "Settings + machine config". **EXTERNALIZE:** (A) CLI-option/session defaults — main RAM KB (`--ram`), fast-disk, FM-PAC auto-load + slot, border, disk-writable, speed (0..7), and the SDL3-presentation knobs scale/filter/persistence/persistence-mode/fullscreen/capture (each an existing knob hardcoded in the M46 `resolve_session_defaults()` layer / `Sdl3AppConfig` struct — audited in `src/frontend/{sdl3_cli.h,sdl3_app.h,sdl3_main.cpp}` + `src/main.cpp`); (B) machine sizing/paths — main RAM KB, VRAM KB, ROM/BIOS dir + the 7 BIOS filenames (`f1xvbios/ext/kdr/disk/mus/kfn/firm.rom`, `hbf1xv_machine.cpp:513-520`), FM-PAC ROM + SRAM path, softwaredb path (`kDefaultSoftwareDbPath`, `cartridge_identifier.h:39`); slot layout as an OPTIONAL validated-to-builtin echo. **HARD-EXCLUDE (stay hardcoded, the immutable silicon spec, reference-precedence tier 1 — a bad XML edit must NEVER be able to break accuracy):** Z80A 3.58 MHz + frame-cycle constants, IM1=13T/IM2=19T/NMI=11T (DEC-0074), S1985 +1 M1 wait, V9958 154/88/31 slots + 1368 cyc/line + M48/M49 parity values, WD2793 FDC constants — empty-diff gate over `src/core`, `src/devices/cpu`, `src/devices/video/vdp_access_timing.h`, `src/devices/video/{v9958_vdp,sprite_engine}.*`, `src/devices/fdc/wd2793.cpp`, `src/devices/chipset/s1985_engine.*`. **Precedence:** explicit CLI arg > XML value > built-in default, resolved in the M46 anti-drift Layer-B seam (ctor `kDramBytes` + `Sdl3AppConfig`/`ParsedSdl3Cli` stock defaults stay UNCHANGED). **Graceful fallback:** file-not-found → WARNING + defaults; bad VALUE (unknown key / wrong type / out-of-range) → per-key WARNING naming the key + that key's default (rest of file still applies); structurally-broken file → whole-file fallback; never crash. **Determinism guard (CRITICAL — keeps the ~235-test suite byte-identical):** headless NEVER auto-loads (default scaffold, `--debug-session`, all `*-parity` modes); SDL3 `--hidden-window` NEVER auto-loads; auto-load ONLY in a genuinely interactive SDL3 launch; `--config <path>` forces load in any mode (no test passes it); search order (interactive) `--config` → exe-dir → CWD, first-found-wins. **Reuse the existing XML machinery:** extract the tolerant `TagScanner` tokenizer from `src/machine/software_db.cpp:37-136` into a shared `src/machine/xml_tokenizer.*` (behavior-preserving; software-db tests are the guard) + a new `src/machine/emulator_config.*` model/validator; NO new third-party XML dependency. **Shipped reference:** a TRACKED, fully-commented `sony_msx_hbf1xv.xml` at repo ROOT (verified NOT gitignored) with every knob + default + allowed range; shipped values == built-in defaults (round-trip identity). **Four slices:** S1 config model + strict parser/validator (reuse the XML reader) · S2 CLI-default resolution wiring (CLI>XML>default precedence + the determinism rule + `--config`) · S3 machine-config sizing/paths (BIOS dir + 7 filenames + FM-PAC/softwaredb paths + RAM KB; VRAM validated-to-128; slots validated-to-builtin) · S4 shipped reference config + docs + round-trip test. Edits confined to `src/machine/{emulator_config,xml_tokenizer,software_db}.*`, `src/machine/hbf1xv_machine.cpp`, `src/frontend/{sdl3_cli,sdl3_main}.*`, `src/main.cpp`, new tests, root `sony_msx_hbf1xv.xml`; ZERO `src/core`, ZERO `src/devices/cpu`, ZERO timing constants.
- Acceptance Criteria: see `docs/m50-planner-package.md` §7 (milestone AC-1..AC-8 + per-slice AC-S1..AC-S4). Highlights: AC-1 byte-identical no-config fallback (full suite GREEN + UNCHANGED, count == pre-M50 baseline); AC-2 shipped-config == no-config (round-trip identity); AC-3 a changed value takes effect; AC-4 precedence CLI > XML > default proven per knob; AC-5 graceful malformed handling (per-key + whole-file fallback, no crash); AC-6 empty-diff over the §3 HARD-EXCLUDE timing set (accuracy protection); AC-7 determinism rule (headless + hidden-window never auto-load; `--config` forces; no test loads config); AC-8 parser reuse, no new dependency, software-db refactor behavior-preserving.
- Unit Tests Required: config parse/validate over the full schema (S1); per-key type/range/enum validation with the exact WARNING + default fallback (non-tautological — assert the WARNING names the key AND the effective value is the default); structural whole-file fallback (no throw); precedence CLI > XML > default per knob (S2); the determinism-rule test (headless/hidden-window no auto-load, `--config` forces, S2); the shipped-config round-trip test (S4); software-db tests byte-identical after the tokenizer extraction (S1).
- Integration Tests Required: `--config <path>` end-to-end on both executables resolving a changed value; a stray root `sony_msx_hbf1xv.xml` leaves every headless/`--debug-session`/parity + SDL3 `--hidden-window` run byte-identical (determinism guard); BIOS-dir/filename + FM-PAC/softwaredb path resolution from config with built-in defaults on omission (S3).
- Regression Scope: full deterministic suite GREEN, count == pre-M50 baseline (~235, developer-pinned) + the new config tests; pre-M50 tests byte-identical; the §3 empty-diff accuracy gate; asset/checksum gates. openMSX A/B NOT required (pure config plumbing — every settable value was already A/B-validated in its origin milestone: RAM M42, FM-PAC/fast-disk M46, video/persistence M37/M39; the timing-constant diff is empty). ZEXALL NOT run (zero cpu/core diff, `feedback_slow_test_cadence.md`). QA sign-off before closure; NORMAL human-release-decision gate (MED blast radius; no auto-close). Production hardening — no backlog row (not a deferred-scope item).
- Status: **Done (2026-07-14)** — all 4 slices implemented (S1 `emulator_config`/`xml_tokenizer` model+strict parser, S2 `config_runtime` CLI>XML>default wiring + determinism gate + `--config`, S3 machine sizing/paths + `set_bios_filenames`, S4 shipped commented `sony_msx_hbf1xv.xml` at repo root + round-trip anti-drift guard + README). **QA PASS** (`docs/m50-qa-signoff.md`): 242/242 green (235 pre-M50 byte-identical + 7 new), empty §3 timing/cpu diff re-attested, round-trip guard proven genuine by adversarial mutation, determinism/precedence/robustness verdicts all PASS, config publishable. **Released v1.2.0** (owner-approved; assistant-pushed per DEC-0047-AMENDMENT-A).

## M51 (2026-07-15, DEC-0078 — sprite disappearance/flicker during scroll; post-M49 sprite-subsystem defect; CRITICAL, gameplay-breaking)

- Milestone ID: M51
- Title: Root-cause and fix the sprite disappearance/flicker-during-scroll defect (Aleste 2 plane vanishes/re-appears; Firebird player + flying enemies flicker constantly; Laydock 2 corroborates)
- Spec Owner: MSX Planner Agent (`docs/m51-planner-package.md`, to be produced)
- Developer Owner: MSX Developer Agent
- QA Owner: MSX QA Agent
- Scope: From the owner's 2026-07-15 CRITICAL live-play reports (fully-autonomous run pre-authorized). Three independent repros, all KonamiSCC carts: **Aleste 2** — player plane sprite completely disappears/re-appears correlated with game scrolling; **Firebird (Hi no Tori Hououhen)** — player character flickers constantly + flying enemy sets flicker severely while ground-locked (background-layer) objects are clean, isolating the defect to the SPRITE subsystem; **Laydock 2** — same disappearance symptom (owner). Timeline: post-M48 (v1.1.5 access-slot contention) and post-M49 (v1.1.6 per-line-live sprite fetch). Candidate attributions (hypotheses, NOT pre-decided): (a) M49 per-line-live watermark/segment regression (per-line visible-buffer lifecycle across mid-frame `check_until` segments, 5th/8-sprite-limit misfire, frame-to-frame segment-boundary wander), (b) M48 timing-overlay interaction shifting where game ISR writes land relative to sprite segments, (c) the documented M49 residual (1-frame lag + cycle-timed intra-line ceiling) proving gameplay-breaking, (d) a distinct latent sprite/R#23 defect. Coordinator recon: the static sprite-line formula (`sprite_engine.cpp:263-267`) is at openMSX parity (incl. R#23 + sentinel-216) — the dynamic path is the suspect. Diagnostic-first discipline (M43/M47 precedent): deterministic repro + milestone-pinning bisect (v1.1.4 → v1.1.5 → v1.1.6 → v1.2.0) BEFORE any fix. Universal spec-modeling fix only; reference precedence DEC-0073 (V9958 fact-sheet tier 1 > openMSX SpriteChecker effect-only tier 2 > fMSX tier 3; never copy reference code).
- Acceptance Criteria: to be detailed by the planner package. Non-negotiable outline: AC-repro (deterministic `--input-script`/frame-dump repro demonstrating the defect pre-fix), AC-attribution (root cause pinned to a concrete mechanism with evidence, incl. bisect verdict), AC-fix (player sprite stays visible across scroll in Aleste 2; Firebird player/flying-enemy flicker resolved to hardware-authentic behavior — genuine >8-sprites-per-line hardware flicker MUST NOT be "fixed" away), AC-regression (all existing sprite/M22/M32/M49 oracles green unweakened; DEC-0031 collision byte-identity; 13-mode 0.000% screen A/B), AC-evidence (before/after captures under `debug/m51/`).
- Unit Tests Required: deterministic sprite-engine tests reproducing the defect mechanism pre-fix and locking the fix (planner to specify; must be non-tautological with adversarial-revert proof).
- Integration Tests Required: deterministic machine-level scroll+sprite scenario (mid-frame R#23/attribute writes across frames) asserting stable sprite visibility; frame-hash determinism preserved.
- Regression Scope: full fast-subset ctest green; every existing sprite/collision oracle unmodified-or-honestly-re-derived; 13-mode screen A/B 0.000%; ZEXALL only if cpu/core touched (not expected). QA sign-off required; NORMAL human-release gate (HIGH blast radius; no auto-close).
- Status: **Done (QA PASS 2026-07-15 `docs/m51-qa-signoff.md`; owner live-confirmed "near perfect" 2026-07-15 → RELEASED v1.2.1: fix `278af86` + README sync `b97a048`, tag + main pushed, remote-verified)** — root cause H1 CONFIRMED (first-bad v1.1.6/M49: the M44 command-row sink sealed command-blitted rows with empty sprite buffers past the render-only watermark; M48 eliminated by bisect). Fix = consumer-side pacing contract (`V9958Vdp::commit_sprite_rows()` + sink/memoization guards), commit `278af86` local-only, 3 files +228/−5, zero core/cpu. Gates: fast ctest 244/244 (worktree AND canonical, QA-reproduced), 12/12 runnable modes 0.000% openMSX A/B (+ QA live canonical re-run), DEC-0031 collision byte-identity, post-fix oracles Aleste 2 + Firebird both 40/40 present / 0 transitions (== openMSX reference rate), determinism + trace-neutrality byte-identical, adversarial-revert proofs recorded. Incident: concurrent foreign session's unverified variant quarantined (`debug/m51/quarantine-rogue-session/`); coordinator serialized + landed the sha-verified set (report §8). Residuals (non-blocking): Laydock 2 probe-stage repro → owner live-check; M49 +2-line ceiling; 1-frame-stale fallback contract.

## M52 (Kickoff 2026-07-15)

- Milestone ID: M52
- Title: SDL3 Master Volume Control + Hotkey Reshuffle + Disk-Writable Default ON
- Spec Owner: MSX Planner Agent
- Developer Owner: MSX Developer Agent
- QA Owner: MSX QA Agent
- Scope: Per DEC-0079 (owner-requested usability slice). (1) Master volume: presentation-stage master gain applied AFTER the DEC-0039 mix (PSG:FM:keyclick ratios byte-identical at full volume); hotkeys Alt+D down / Alt+U up (host-hotkey discipline); CLI `--volume <0..100>` (--persistence pattern); XML `<volume>` via the M50 `emulator_config.*` seam, CLI > XML > built-in default. (2) Fast-disk toggle hotkey Alt+D → Alt+F. (3) `--disk-writable` default ON (CLI + XML), `--no-disk-writable` negative flag, Alt+S runtime toggle with planner-defined flush semantics; M45/M45b/M47 write-integrity preserved; determinism guard — headless/test paths byte-identical.
- Acceptance Criteria:
	- Planner package `docs/m52-planner-package.md` defines slices, per-slice acceptance, flush semantics, volume default/step, and headless parity decision.
	- Master gain provably post-mix (full-volume output byte-identical to pre-M52; DEC-0039 ratios untouched).
	- Hotkeys Alt+D/Alt+U/Alt+F/Alt+S live under the established host-hotkey discipline with stderr feedback; no collision (audit in DEC-0079).
	- CLI > XML > built-in default precedence verified by deterministic tests for `--volume` and disk-writable.
	- Pre-M52 deterministic suite remains byte-identical; new tests cover volume precedence, hotkey dispatch, disk-writable default + toggle + flush semantics.
	- Evidence gates: validate-assets, checksum, build both exes, full fast-subset ctest, empty-diff accuracy gate over the HARD-EXCLUDE timing set. openMSX A/B + ZEXALL not required (frontend/config-only; re-armed on any cpu/core edit).
- Unit Tests Required: volume precedence chain (CLI/XML/default), gain clamping/step, emulator_config `<volume>` + disk-writable default parsing, CLI `--volume` validation + `--no-disk-writable`.
- Integration Tests Required: SDL3 config-resolution round-trip (volume + disk-writable); disk-writable toggle flush-semantics test; full-volume byte-identity audio test.
- Regression Scope: All prior suites remain passing byte-identical; QA sign-off required before closure; NORMAL human-release gate (no auto-close).
- Status: Done (QA PASS + owner live-confirmed; released v1.2.2, 2026-07-15; DEC-0079-CLOSURE. Post-QA owner amendment DEC-0079-AMENDMENT-A: volume-up hotkey Alt+P → Alt+U, QA re-run waived by owner, rebuild + 10/10 related tests verified)
- Details: Planner package `docs/m52-planner-package.md` (RESP-M52-001; default volume 100 unity, linear s*vol/100 clamp-not-wrap step 10, key-repeat on volume keys only, Alt+S = DiskImage host-path bind/unbind, headless disk-writable stays OFF). Implementation `docs/m52-implementation-report.md` (RESP-M52-002): 13 tracked files +313/−40 + new `src/frontend/master_volume.h`; tests 245→248 executables; full ctest 247/247; §4.5 flips = exactly 3, DEC-0079-commented. QA `docs/m52-qa-signoff.md` (RESP-M52-003) = **PASS**: QA-reproduced 247/247, byte-identity test #183 non-tautological, anti-drift verified, Alt+S semantics proven (#184), FDC diff empty + write-integrity green, HARD-EXCLUDE empty-diff twice, shipped-XML round-trip green, headless smoke (Volume: 50% banner, disk-writable ON, boot-only disk SHA unchanged). ZEXALL + openMSX A/B correctly not required (frontend/config-only, empty cpu/core diff). UNCOMMITTED pending the owner live check (Alt+D/Alt+U audible, Alt+F, Alt+S, default disk-save persists, --no-disk-writable) → owner-directed commit/tag/push target v1.2.2.

## M53 (Kickoff 2026-07-15)

- Milestone ID: M53
- Title: Standalone MSX Disk Utility (`msx-disk.exe` — create / read / format)
- Spec Owner: MSX Planner Agent
- Developer Owner: MSX Developer Agent
- QA Owner: MSX QA Agent
- Scope: Per DEC-0080 (owner request + scope-area ratification). Single executable with `--create` (new fully-formatted blank 720 KB .dsk), `--read` (hex dump), `--format` (re-format existing image) — byte-exact HB-F1XV-compatible 720 KB 3.5" DD MSX-DOS FAT12 (80×2×9×512, MSX boot sector). STRUCTURE (owner directive): source in NEW `src/diskutils/`; compiled binary copied post-build to `diskutils/` from the ONE `build/` tree (DEC-0041). Dependency direction: tool may reuse repo disk-image knowledge; emulator NEVER depends on the tool. Binary gitignored by default (owner may override).
- Acceptance Criteria:
	- Planner package `docs/m53-planner-package.md` with CLI grammar, byte-exact FAT12/boot-sector spec (grounded + cited), slices, tests, gates, risks.
	- All three modes work and produce/accept images byte-compatible with the machine; a tool-created image boots/mounts in the emulator's own FDC headlessly.
	- Emulator suite byte-identical (247/247 pre-M53 tests untouched); new tool tests green.
	- `src/CLAUDE.md` gains the `src/diskutils/` responsibility line; `.gitignore` covers the output binary.
	- Evidence gates: build all targets, full fast ctest, validate-assets + checksums unchanged. openMSX A/B + ZEXALL N/A (DEC-0080).
- Unit Tests Required: boot-sector/BPB/FAT/root-dir byte oracles (vs golden + real msxdos23.dsk-derived expectations), create/format round-trip, hex-dump determinism, CLI validation.
- Integration Tests Required: emulator FDC headless boot/mount of a tool-created and a tool-formatted image.
- Regression Scope: All prior suites remain passing byte-identical; QA sign-off required; NORMAL human-release gate (owner user-test), no auto-close.
- Status: Done (QA PASS + owner user-test "works like a charm"; committed + pushed 2026-07-15; DEC-0080-CLOSURE. Owner-directed doc sweep: README section + bullet, root/src CLAUDE.md, project-baseline.md, all 5 .claude/agents now describe the diskutil + isolation rule. No version tag — tool-only addition, v1.2.2 remains current)
- Details: Planner `docs/m53-planner-package.md` (RESP-M53-001; layout source of truth tools/format-blank-disk.ps1:82-133, all three DEC-0073 tiers agree; self-contained tool, no emulator link; CLI --create/--format [--force, 737280-byte guard, exit 3]/--read [--sector|--range, xxd-style]; no timestamps/serial — deterministic golden SHA). Implementation `docs/m53-implementation-report.md` (RESP-M53-002): new src/diskutils/{msx_disk_format,hex_dump,cli}.{h,cpp}+main.cpp, additive CMake msx_diskutil+msx_disk (OUTPUT_NAME msx-disk) + POST_BUILD copy → diskutils/msx-disk.exe (gitignored; .gitkeep tracked); tracked diff exactly .gitignore+CMakeLists.txt; tests 247→253. QA `docs/m53-qa-signoff.md` (RESP-M53-003) = **PASS**: 253/253 QA-reproduced, baseline name-for-name intact, AC-2 golden SHA 6f1a7983…b0ce5188 three-way reproduced (tool/PS1/golden), isolation verified (msx_disk links only msx_diskutil; emulator-src diff EMPTY), CLI matrix 8/8, non-tautology confirmed (BPB-vs-synthesize + real-WD2793-datapath mount), POST_BUILD re-land verified. ZEXALL + openMSX A/B N/A (DEC-0080). UNCOMMITTED pending owner user-test (create → read → format scratch copy → emulator Disk-BASIC recognition) → owner-directed commit/push.

## M54 (Kickoff 2026-07-15)

- Milestone ID: M54
- Title: macOS Support — Single Codebase, Dual Toolchain (MSVC + AppleClang), Build-Time Auto-Detect
- Spec Owner: MSX Planner Agent
- Developer Owner: MSX Developer Agent (DEFERRED — executes on the owner's macOS machine; real-hardware evidence required)
- QA Owner: MSX QA Agent (DEFERRED — follows Mac-side developer evidence)
- Scope: Per DEC-0081. One repo/one codebase compiling on Windows (MSVC) and macOS (AppleClang) with CMake platform auto-detection; no fork. THIS CYCLE = PLANNING ONLY (owner directive): thorough package incl. portability audit (file:line), CMake platform matrix, .gitattributes/line-ending policy, filename-case audit, tooling strategy (pwsh/openMSX-native), macOS runbook for the owner, acceptance criteria both platforms, deferral protocol.
- Acceptance Criteria (planning cycle): `docs/m54-planner-package.md` delivered covering DEC-0081 items (1)-(10); deferral note recorded in responses; M54 remains OPEN in PLANNING.
- Acceptance Criteria (milestone, Mac-side continuation): AppleClang clean build of all 3 targets (headless, sdl3, msx-disk); fast ctest parity vs the Windows 253 baseline modulo enumerated asset-gated skips; determinism byte-identity of golden oracles; Windows rebuild after portability edits stays 253/253 byte-identical; QA sign-off; NORMAL human-release gate.
- Unit Tests Required: none new this cycle (planning); Mac-side per package.
- Integration Tests Required: none new this cycle (planning); Mac-side per package.
- Regression Scope: zero code change this cycle. Mac-side: Windows suite byte-identity is the hard regression bar.
- Status: In Progress (PLANNING — developer/QA deferred to owner's macOS continuation)

## M54 status update (2026-07-16)

- Status: Done (DEC-0081-CLOSURE). Mac-side delivery `b669fea` (CMake dual-toolchain branch, .gitattributes binary pins, help-string neutralization, README dual-platform docs; Mac evidence 253/253 fast + ZEXALL 254/254 on AppleClang + golden SHA match). Windows re-gate coordinator-discharged 2026-07-16: AC-W1 clean rebuild + 253/253, AC-W2 goldens byte-identical (zero tests/ edits + direct SHA re-verify 6f1a7983…ce5188 == Mac arm64 == M53 golden), AC-W3 zero .gitattributes churn. Mac QA CONDITIONAL PASS condition discharged → Done. Residuals (LOW): 35 benign AppleClang warnings (no -Werror by design), .app/.icns deferred non-goal, Linux/GCC inherits the else() branch (future).
