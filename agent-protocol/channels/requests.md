...
- Request ID: REQ-M32-001
- From: MSX Master Agent (coordinator)
- To: MSX Planner Agent
- Milestone ID: M32 (RC-playtest defect pair: per-line raster rendering + FM mix calibration; tag target v1.0.33)
- Type: Milestone package
- Scope: Produce docs/m32-planner-package.md for the two DEC-0039 defects. (A) Raster-accurate per-line rendering: replace the frame-at-once register-snapshot render with scanline-accumulation driven by the machine's raster position, reading live VDP registers per line, so mid-frame register writes (R#23 screen splits first among them, but ALL control registers) take effect from the next line boundary — grounded in references/openmsx-21.0/src/video/PixelRenderer.cc semantics and the existing VdpRasterClock/DEC-0031 line-granular infrastructure. Must define: where line rendering hooks into the machine advance loop; the register-latch granularity contract (line boundary); interaction with sprite_engine's existing per-line R#23 use; the byte-identity regression oracle for register-static frames (all committed frame evidence must reproduce); a deterministic split-screen integration test (synthetic CPU program rewriting R#23 via line interrupt mid-frame — the Aleste 2 shape, no game asset dependency); and the SDL3/headless dump paths both consuming the accumulated buffer. (B) FM mix calibration: an honestly-derived replacement for kFmAmplitudeScale=5, grounded in the openMSX Sony_HB-F1XV.xml PSG:MSX-MUSIC volume ratio (21000:9000) and the fact-sheet §7 measured peaks, with a deterministic loudness-ratio test and a re-capture of the fmON/fmOFF evidence pair demonstrating audibility; the zero-YM2413 byte-identity oracle must remain green (scale change only affects non-silent FM).
- Acceptance Criteria: Package with slices, acceptance criteria per slice, regression oracles (esp. the register-static byte-identity oracle), A/B plan (frame-level vs openMSX for the split test; loudness ratio for FM), test matrix additions, and an explicit no-cpu/core-touch statement (ZEXALL cadence: fast subset only).
- Dependencies: DEC-0039; docs/m31-qa-signoff.md (RC baseline); references/openmsx-21.0/src/video/PixelRenderer.cc; references/fact-sheets/Yamaha V9958 VDP.md; references/fact-sheets/Yamaha YM2413 FM Chip.md.
- Requested At: 2026-07-09T10:30:00+09:00

---

- Request ID: REQ-M32-003
- From: MSX Master Agent (coordinator)
- To: MSX QA Agent
- Milestone ID: M32 (tag target v1.0.33)
- Type: Regression assessment + sign-off recommendation
- Scope: Full QA of the M32 implementation (`docs/m32-implementation-report.md` is the handoff; `docs/m32-planner-package.md` + DEC-0039 + RESP-M32-001 are the charter). COORDINATOR ARBITRATION OF THE AC-6 ESCALATION (recorded here, PROVISIONAL pending QA): the four committed-evidence divergences (boot-logo-after-f0150 wobble tearing; m31-rc-metalgear-f1100 + local metalgear-f1400/mg2-f1700 D9-class sprite bands) are provisionally ACCEPTED as the intended Defect-A raster-truth direction — QA must independently root-cause each one, confirm the developer's A-M32-2 claim (no IE1/CPU-trajectory shift anywhere in the evidence matrix), and confirm the divergence direction is corroborated by the openMSX LINE-accuracy model (the DEC-0031 committed openMSX reference PNGs are available for directional comparison). If QA confirms all four, the affected evidence is regenerated at closure with supersession notes (the established superseded-figure pattern); if ANY divergence fails root-cause verification, the provisional acceptance is VOID and the finding blocks closure.
- Acceptance Criteria: independent fresh-build reproduction of both suites (headless 177/177, SDL3-ON 186/186 claimed); adversarial verification that the new tests genuinely discriminate (mutation checks at QA's discretion); faithfulness of the line-interrupt relation and L+1 latch to the cited references; IE1-off zero-change proof; the finalize-AFTER-on_vsync ordering deviation judged; FM k=21 derivation independently re-derived from the cited openMSX sources; loudness ratio re-measured; zero-YM2413 oracle confirmed green and unmodified; hard-constraint audits re-run (zero cpu/core diffs → ZEXALL correctly not run per the standing cadence); license isolation confirmed; evidence gates re-run; `docs/m32-qa-signoff.md` with Pass / Conditional Pass / Fail.
- Dependencies: docs/m32-implementation-report.md; docs/m32-parity-trace-diff.md; docs/m32-planner-package.md; DEC-0039; RESP-M32-001/002.
- Requested At: 2026-07-09T14:20:00+09:00

---

- Request ID: REQ-M33-001 / RESP-M33-001 (combined docs-sweep dispatch + outcome + coordinator arbitrations, M33 housekeeping per DEC-0041)
- From/To: MSX Master Agent (coordinator) <-> MSX Developer Agent (documentation slice)
- Milestone ID: M33 (repository housekeeping; tag target v1.0.34)
- Outcome: Docs truth sweep COMPLETE, every claim tree-verified this run. (1) Root README.md REWRITTEN: v1.0.33 status snapshot, single-build flow via tools/bootstrap-build.ps1 + manual fallback, run examples with every CLI flag verified against sdl3_main.cpp/sdl3_cli.cpp/main.cpp, layout, asset policy, agent-protocol/state/ named authoritative. (2) tools/README.md REWRITTEN (was M0/M10-era, ~30 scripts missing): all 43 tool files indexed one line each, purposes verified from headers. (3) docs/README.md REWRITTEN (was M0-M8-only): frozen mNN-triad taxonomy, adjuncts, six investigation/fix report sets mapped to DECs, never-edit-retroactively rule stated. (4) debug/README.md NEW: runtime layout verified against hbf1xv_machine.h:333-377, gitignore patterns, complete committed-evidence exception inventory, capture/convert tooling. (5) bios/README.md UPGRADED: all seven ROM filenames/sizes/slot roles verified three ways (disk, loader specs hbf1xv_machine.cpp:385-392, validate-assets.ps1), 0xFF-fill-on-missing behavior documented. Left unedited as verified-accurate: src/CLAUDE.md, tests/CLAUDE.md (spot-checked), roms/README.md, src/devices/memory/README.md, tests/parity/z80_parity_checkpoint.md.
- Coordinator arbitrations on the sweep's escalated findings (all APPLIED by coordinator, not the agent): (a) sdl3_main.cpp print_usage() stale M17-era "YM2413 intentionally SILENT (E1 still open)" paragraph REPLACED with the accurate post-M31/M32 audio-mix statement (no test asserts usage text; grep-verified). (b) src/devices/chipset/README.md file map: four missing rows ADDED (ppi_8255 M15, mb670836_pause M25, reset_status_register DEC-0031, system_control), ppi_slot_select row re-scoped to "legacy M11 minimal, superseded in machine wiring by ppi_8255" — and the matching stale roadmap comment in ppi_slot_select.h:16-17 corrected. (c) agent-protocol/README.md Files list: state/deferred-backlog.md row ADDED (it is a DEC-0041-protected file and was undocumented). (d) Removal candidate src/peripherals/README.md (M0-era junk stub, typos, wrong content): REWRITTEN as an accurate six-device file map (matches the chipset/memory README pattern). (e) Removal candidate tools/__pycache__/mem-to-png.cpython-314.pyc (tracked bytecode): git rm'd; __pycache__/ + *.pyc added to .gitignore. (f) tests/CLAUDE.md: one-line tests/parity/ description added (fixture dir, not a test level).
- Also this cycle (coordinator, before the sweep): ZEXALL-cadence guard added to tools/bootstrap-build.ps1 — plain -RunTests now runs the fast subset (-E hbf1xv_m24_zexall_system_test); the full sweep requires the explicit -Slow switch (RC-gate cadence). The first bootstrap invocation had started the sweep un-filtered and was STOPPED mid-ZEXALL by the coordinator per the standing cadence memory; no sweep result is claimed.
- Requested/Responded At: 2026-07-09T18:30:00+09:00

---

- Request ID: REQ-M34-003
- From: MSX Master Agent (coordinator)
- To: MSX QA Agent
- Milestone ID: M34 (tag target v1.0.35)
- Type: Regression assessment + sign-off recommendation
- Scope: Full QA of the M34 implementation (`docs/m34-implementation-report.md` is the handoff; `docs/m34-planner-package.md` + DEC-0043 + RESP-M34-001 are the charter; `docs/m34-audio-ab.md` is the audio A/B record). The working tree is UNCOMMITTED M34 work on v1.0.34 (`git diff v1.0.34` + untracked new files -- NOTE per DEC-0044 that tests/tools/docs/debug are .gitignored, so use `git status --untracked-files=all` and the report §11 file inventory, not a bare `git status`). Two defects: (A) band-limited PSG/SCC box-average integration (kills the ultrasonic point-sampling alias -- the Aleste transition beep); (B) a BL (R#1 bit6) display-enable gate in VdpFrameRenderer::render_line() (the Metal Gear room-transition "slow fill").
- COORDINATOR PROVISIONAL ARBITRATION of the two report-§5 escalations (the four committed m32-aleste-play PNGs), PENDING your independent root-cause: PROVISIONALLY ACCEPTED for regeneration-with-supersession (M32 AC-6 precedent), conditioned on QA CONFIRMING BOTH developer claims: (1) rows 14-15 change because Aleste LEGITIMATELY drops BL=0 at display lines 13->15 every gameplay frame (the BL fix rendering correct hardware behavior -- verify the 915-transition measurement independently); (2) f3000/3200/3600's rows 99-117 differ for an M34-INDEPENDENT pre-existing reason (the developer's gate-stashed discriminator claim -- independently reproduce: with the BL gate removed, do rows 99-117 still differ from the committed PNG? if yes, M34-independent confirmed). If EITHER claim fails your verification, the provisional acceptance is VOID and that finding BLOCKS closure.
...
- Request ID: REQ-M38..M47 (channel-continuity note — NOT a back-fill)
- From: MSX Master Agent (coordinator)
- Milestone ID: M38 through M47 (v1.0.38 → v1.1.3)
- Type: Bookkeeping note
- Scope: This request/response channel pair has NOT been maintained per-cycle since M37 (v1.0.38). From M38 onward the LIVING coordination record is `agent-protocol/state/current-phase.md` (per-milestone active/prior-phase blocks) + `agent-protocol/channels/decisions.md` (DEC-0055…DEC-0074) + the per-milestone `docs/m<NN>-*` packages/reports/sign-offs. Milestones M38-M47 are fully recorded there and in `agent-protocol/state/milestones.md`. A full REQ/RESP back-fill for M38-M46 was NOT performed (out of scope for this doc-hygiene pass); the single M47 closing entry below bookmarks the most recent (autonomous) run.
- Requested At: 2026-07-14

---

- Request ID: REQ-M47-001 (consolidated; autonomous run, coordinator-driven)
- From: MSX Master Agent (coordinator) -> MSX Developer Agent(s) + a 4-specialist device-fidelity audit (opus)
- Milestone ID: M47 (DEC-0072, autonomous; tag target v1.1.3, owner's call)
- Type: Investigation + implementation + verification (dispatched live; back-filled to channel)
- Scope: Root-cause the YS II 8-slot disk-save crash the owner still saw after the `a27be57` write-fix. Built an env-gated per-byte FDC write logger + a deterministic `--record-input`→replay harness; ran a 4-specialist audit (WD2793 / V9958 / Z80A / RTC-PSG-mapper) grounded in datasheets + openMSX/fMSX + web; ran the `--vdp-cmd-bias` timing sweep; ran the decisive independent openMSX cross-check (fresh game, vanilla disk, multiple sources). Fixed the three real device-fidelity defects the audit surfaced (CPU int/NMI-ack timing, VDP command-engine timing, PSG counter clamp) universally; authored + propagated the reference-precedence directive. Zero cpu/core was NOT a constraint here — the CPU-core edit triggered the ZEXALL sweep per `feedback_slow_test_cadence.md`.
- Requested At: 2026-07-13 → 2026-07-14 (autonomous, multiple dispatches)

---

- Request ID: REQ-M51-001
- From: MSX Master Agent (coordinator)
- To: MSX Planner Agent
- Milestone ID: M51 (DEC-0078; autonomous run pre-authorized by the owner 2026-07-15)
- Type: Planning — milestone package (planner-first)
- Scope: Produce `docs/m51-planner-package.md` for the CRITICAL sprite disappearance/flicker-during-scroll defect. Three live repros (all KonamiSCC carts): Aleste 2 (player plane disappears/re-appears with scroll), Firebird / Hi no Tori Hououhen (player + flying enemies flicker constantly; ground-locked background objects clean — sprite-vs-background discriminator), Laydock 2 (same disappearance, owner report). Structure DIAGNOSTIC-FIRST (M43/M47 precedent): (1) deterministic repro slice — `--input-script`/`--dump-frame` frame-sequence captures demonstrating sprite present/absent alternation; (2) attribution slice — git bisect across tagged anchors v1.1.4 `2e09040` (pre-M48) → v1.1.5 `e7a7a41` (M48 contention) → v1.1.6 `0d84451` (M49 per-line sprites) → v1.2.0 `0bf3285`, same script + frame-hash oracle at each anchor; (3) fix slice(s) — universal spec-modeling fix per attribution; (4) regression/evidence slice — 13-mode 0.000% openMSX A/B, DEC-0031 collision byte-identity, existing sprite/M22/M32/M49 oracles unweakened, before/after captures under `debug/m51/`. Ground in tier-1 `references/fact-sheets/Yamaha V9958 VDP.md` (§6 sprite fetch line N−1, §7/§10 R#23 live per-line) with openMSX `SpriteChecker.{cc,hh}` effect-only tier-2 and fMSX `V9938.c` tier-3 triangulation. Coordinator recon to incorporate (hypothesis-grade): static sprite-line formula `sprite_engine.cpp:263-267` matches openMSX incl. R#23 + sentinel-216; suspects are the DYNAMIC path — M49 `check_until` watermark/per-line buffer lifecycle across mid-frame segments, 5th/8-sprite-limit misfire, M48 timing shift of ISR-write landing lines, or the documented M49 residual. Hypothesis discipline: none pre-decided; genuine >8-sprites/line hardware flicker must NOT be "fixed" away. NO code; package + per-slice acceptance + evidence gates + risks only.
- Constraints: planner produces specs only; expected fix locus `src/devices/video/{sprite_engine,v9958_vdp}.*` (+ `src/machine/hbf1xv_machine.cpp` seam if needed); ZERO `src/core`/`src/devices/cpu` expected; HIGH blast radius → NORMAL human-release gate, no auto-close.
- Requested At: 2026-07-15T00:45:00+09:00

---

- Request ID: REQ-M51-002
- From: MSX Master Agent (coordinator)
- To: MSX Developer Agent
- Milestone ID: M51 (DEC-0078; autonomous run)
- Type: Implementation + verification — diagnostic-first per `docs/m51-planner-package.md`
- Scope: Execute the M51 package end-to-end: Task 0 deterministic repro + falsifiable sprite-presence oracle (+ openMSX discriminator leg); Task 1 bisect v1.1.4 `2e09040` → v1.1.5 `e7a7a41` → v1.1.6 `0d84451` → v1.2.0 `0bf3285` (sequential rebuilds of the ONE canonical build/ via tools/bootstrap-build.ps1 — DEC-0041); Task 2 env-gated trace naming the mechanism (H1 command-sink sprite-pacing gap is the highest-prior suspect: `hbf1xv_machine.cpp:138-158` on_commit_up_to advances background past the sprite watermark after `sprite_engine.cpp:93` begin_frame clear — verify, don't assume); then the conditional fix branch the attribution selects, new non-tautological unit+integration tests (adversarial-revert proven), and evidence gates EG-1..EG-11 (build both exes, fast ctest, 13-mode 0.000% openMSX A/B, DEC-0031 collision byte-identity, before/after captures under debug/m51/, scope attestation). Guard: authentic >8-sprites-per-line multiplex flicker must NOT be fixed away (§1.6 3-prong guard). Fix locus bound: `src/devices/video/{sprite_engine,v9958_vdp}.*` + `src/machine/hbf1xv_machine.cpp`; anything wider STOPS and escalates. Zero core/cpu expected → ZEXALL not triggered. Produce `docs/m51-implementation-report.md`. Do NOT commit (coordinator owns closure).
- Constraints: reference precedence DEC-0073 (fact-sheet tier 1 > openMSX effect-only tier 2 > fMSX tier 3; never copy reference code); single build tree; deterministic tests only; no oracle weakened.
- Requested At: 2026-07-15T01:10:00+09:00

---

- Request ID: REQ-M51-003
- From: MSX Master Agent (coordinator)
- To: MSX QA Agent
- Milestone ID: M51 (DEC-0078; autonomous run)
- Type: Regression assessment + sign-off recommendation
- Scope: Full QA of the landed M51 fix (commit `278af86` on main, NOT pushed). Handoffs: `docs/m51-implementation-report.md` (incl. §8 concurrent-session incident + §9 QA handoff), `docs/m51-planner-package.md` (charter), DEC-0078. Coordinator landing record: verified 3-file set landed sha-exact from the developer's worktree deliverables (m51-src.diff sha256 d54b1525…), foreign session's unverified variant quarantined at `debug/m51/quarantine-rogue-session/canonical-landing-quarantine/`, canonical attestation **244/244** fast ctest GREEN after serializing against the foreign MSBuild fleet (stale-object hazard cleared via mtime bump). QA must: (a) reproduce EG-1/EG-2 from the canonical build/ (fresh fast ctest); (b) independently run the post-fix title oracles from the canonical binary (Aleste 2 + Firebird per report §1/§6 recipes, tools/m51-*); (c) verify EG-3..EG-11 evidence artifacts under `debug/m51/` are genuine and support the report's claims (A/B 12/12 0.000%, collision byte-identity, determinism, scope attestation vs `git show 278af86 --stat`); (d) verify the §1.6 authentic-multiplex guard was honored; (e) verify integrity: the 3 src files at HEAD == the sha-stamped deliverables (hash check) with NO foreign re-modification; (f) issue `docs/m51-qa-signoff.md` with Pass / Conditional Pass / Fail. NON-DESTRUCTIVE discipline (M35 rule): NO git checkout/reset/revert in the shared tree, NO src edits, NO parallel build trees; the adversarial-revert proof is already recorded worktree-side (report §3/§4) — assess it as evidence, do not re-execute it destructively. If a foreign writer reappears mid-QA (compiler procs you did not start, files changing under you), STOP and report.
- Requested At: 2026-07-15T03:40:00+09:00

---

- Request ID: REQ-M52-001
- From: MSX Master Agent (coordinator)
- To: MSX Planner Agent
- Milestone ID: M52 (DEC-0079)
- Type: Planning — milestone package (planner-first)
- Scope: Produce `docs/m52-planner-package.md` for the owner-requested SDL3 usability slice (DEC-0079): (1) master volume — presentation-stage master gain strictly AFTER the DEC-0039 mix (post-mix scalar; PSG:FM:keyclick ratios in `src/frontend/machine_audio_mixer.*` byte-identical at full volume), hotkeys Alt+D down / Alt+P up per the host-hotkey discipline (`src/frontend/sdl3_app.cpp:496-552` Alt+Enter/Alt+D/Alt+B/Alt+M precedent), CLI `--volume <0..100>` per the `--persistence` pattern (`src/frontend/sdl3_cli.h:136-151`), XML `<volume>` via the M50 seam (`src/machine/emulator_config.*`, `src/frontend/config_runtime.cpp:103` precedence CLI > XML > built-in default); planner decides the built-in default (owner asked for "cli with default volume value") and the hotkey step size + whether key-repeat is honored; (2) fast-disk hotkey Alt+D → Alt+F (message/behavior otherwise unchanged); (3) disk-writable default ON in CLI seam + XML default (`src/machine/emulator_config.h:64`), `--no-disk-writable` negative flag (`--fast-disk`/`--no-fast-disk` pattern), Alt+S runtime toggle with PRECISELY defined flush semantics (writes made while ON then toggled OFF before swap/exit — flushed or discarded, and how reported; interaction with the existing swap/exit flush points `src/frontend/sdl3_app.cpp:202,726-737,810`); M45/M45b/M47 write-integrity must not regress; headless `src/main.cpp` disk-writable-default parity is the planner's decision under the CRITICAL determinism constraint (headless/test paths must stay byte-identical; no test may start flushing host files). Deliver: slices with per-slice acceptance, milestone AC, deterministic test obligations, evidence gates (incl. the M50-style empty-diff HARD-EXCLUDE accuracy gate; openMSX A/B + ZEXALL not required, frontend/config-only), risks. NO code.
- Constraints: planner produces specs only; expected edit locus `src/frontend/*` + `src/machine/emulator_config.*` + possibly `src/main.cpp` + root `sony_msx_hbf1xv.xml`; ZERO `src/core`/`src/devices/cpu`/device-timing; MED blast radius; NORMAL human-release gate.
- Requested At: 2026-07-15T13:50:00+09:00

---

- Request ID: REQ-M52-002
- From: MSX Master Agent (coordinator)
- To: MSX Developer Agent
- Milestone ID: M52 (DEC-0079)
- Type: Implementation + deterministic tests + evidence gates
- Scope: Implement `docs/m52-planner-package.md` slices S1→S4 in order: S1 master-volume core (`master_volume.h` pure helper + Sdl3AudioPresenter post-mix scalar + `--volume <0..100>` CLI + `<volume>` XML via emulator_config + config_runtime CLI>XML>default resolution + sdl3_main wiring; default 100, linear s*vol/100, clamp); S2 hotkeys (Alt+D vol-down / Alt+P vol-up with key-repeat honored; fast-disk Alt+D → Alt+F fresh-keydown; host-hotkey discipline — only the letter keydown swallowed; stderr feedback lines); S3 disk-writable default ON in the M50 resolver seam + shipped XML (Sdl3AppConfig struct default stays false — anti-drift), `--no-disk-writable` negative flag, Alt+S toggle via DiskImage::set_host_path bind/unbind (ON binds + late-ON flushes whole image; OFF unbinds — in-memory kept, discarded at exit; ZERO src/devices/fdc edits); S4 shipped `sony_msx_hbf1xv.xml` + README/banner doc updates + round-trip test. Test obligations per package §5 (2 new test executables + full-volume byte-identity oracle + extended suites; the 3 intentionally-flipped policy assertions per §4.5 are expected — flip them with a comment citing DEC-0079, never weaken others). Evidence gates per package §6: tools/validate-assets.ps1, tools/checksum-assets.ps1 -OutFile docs/asset-checksums.txt, clean build both exes from the ONE build/ tree (DEC-0041), full fast-subset ctest (pre-M52 emulation fixtures byte-identical), and the M50-style EMPTY `git diff` over the HARD-EXCLUDE timing set (src/core, src/devices/cpu, src/devices/video/vdp_access_timing.h, src/devices/video/{v9958_vdp,sprite_engine}.*, src/devices/fdc/wd2793.cpp, src/devices/chipset/s1985_engine.*). openMSX A/B + ZEXALL not required (frontend/config-only) — STOP and escalate if any cpu/core/device-timing edit becomes necessary. Produce `docs/m52-implementation-report.md`. Do NOT commit (coordinator owns landing).
- Constraints: edit locus bound to src/frontend/* + src/machine/emulator_config.* + src/main.cpp (headless --volume rejection only if planner specified; headless disk-writable default stays OFF) + root sony_msx_hbf1xv.xml + tests/ + docs/; single build tree; deterministic tests only; no oracle weakened beyond the §4.5 enumerated flips; reference SDL3 claims against references/sdl3/ paths; never copy reference code.
- Requested At: 2026-07-15T14:06:00+09:00

---

- Request ID: REQ-M52-003
- From: MSX Master Agent (coordinator)
- To: MSX QA Agent
- Milestone ID: M52 (DEC-0079)
- Type: Regression assessment + sign-off recommendation
- Scope: Full QA of the uncommitted M52 working-tree changes (13 tracked files +313/−40 + new `src/frontend/master_volume.h` + gitignored test edits). Handoffs: `docs/m52-planner-package.md` (charter incl. §4.5 enumerated intended test flips + §6 evidence gates), `docs/m52-implementation-report.md` (persisted developer report), DEC-0079, RESP-M52-002. QA must: (a) reproduce the build + full fast-subset ctest from the ONE canonical `build/` tree (DEC-0041 — no parallel trees) and report the real counts; (b) verify the 3 §4.5 test-expectation flips are EXACTLY the enumerated set and no other oracle was weakened (diff-level review of the gitignored tests/ edits); (c) verify the M-1 full-volume byte-identity claim via test #183 and the anti-drift claim (Sdl3AppConfig/ParsedSdl3Cli struct defaults still false; flip confined to emulator_config + shipped XML); (d) verify Alt+S flush semantics via test #184 and confirm ZERO src/devices/fdc diff + FDC write-integrity suite green (M45/M45b/M47 preserved); (e) re-run the HARD-EXCLUDE empty-diff accuracy gate; (f) verify the shipped sony_msx_hbf1xv.xml round-trip (values == compiled defaults, zero warnings) and README/usage doc accuracy (Alt+F/Alt+D/Alt+P/Alt+S, --volume, --no-disk-writable); (g) manual-smoke the SDL3 hotkey wiring IF feasible headlessly via --hidden-window + input-script, otherwise disposition it honestly as the established manual-smoke boundary for owner live check; (h) ZEXALL optional — empty-diff-proven unchanged; run only if you judge the release baseline needs it; (i) issue `docs/m52-qa-signoff.md` with Pass / Conditional Pass / Fail + residual risks. NON-DESTRUCTIVE discipline (M35 rule): NO git checkout/reset/revert in the shared tree, NO src edits, NO parallel build trees. If a foreign writer appears mid-QA, STOP and report.
- Constraints: MED blast radius; NORMAL human-release gate (no auto-close); the owner live check (volume audible steps, hotkeys, disk-writable toggle) follows QA and precedes any release.
- Requested At: 2026-07-15T15:07:00+09:00

---

- Request ID: REQ-M53-001
- From: MSX Master Agent (coordinator)
- To: MSX Planner Agent
- Milestone ID: M53 (DEC-0080)
- Type: Planning — milestone package (planner-first)
- Scope: Produce `docs/m53-planner-package.md` for the standalone MSX disk utility per DEC-0080. Owner contract: single executable `msx-disk.exe`; modes `--create` (new fully-formatted blank 720 KB .dsk), `--read` (hex dump to stdout), `--format` (re-format existing image, fully HB-F1XV-compatible). STRUCTURE DIRECTIVE (owner, hard): source under `src/` (new `src/diskutils/`), compiled binary lands in `diskutils/` (post-build copy from the ONE build/ tree). Planner must define: exact CLI grammar (file arguments, in-place vs -o output, overwrite/confirm semantics for --format, hex-dump range/paging options if any — keep it simple), the byte-exact 720KB FAT12 layout spec (boot sector fields incl. media descriptor 0xF9 + OEM/BPB values, 2 FATs, root dir entries, cluster size — ground in `tools/format-blank-disk.ps1`, the repo DiskImage code (`src/machine/` disk_image), real `disks/msxdos23.dsk` as oracle, and DEC-0073-ranked references, citing concrete paths), reuse-vs-copy decision for disk-image knowledge (dependency direction: tool may reuse, emulator NEVER depends on the tool), CMake target + output-copy mechanics, gitignore handling for the binary (default: ignored), `src/CLAUDE.md` responsibility line text, slice decomposition, per-slice + milestone acceptance criteria, deterministic test obligations (unit: boot-sector/FAT/dir-structure oracles vs golden bytes, create/format round-trip, hex-dump determinism; integration: emulator FDC headlessly boots/mounts a tool-created image — reuse the existing --input-script/--dump-frame or debug-session harness), evidence gates (build all targets, full fast ctest with pre-M53 247/247 byte-identical, validate/checksum unchanged; openMSX A/B + ZEXALL N/A per DEC-0080), risks. NO code.
- Constraints: planner produces specs only; LOW blast radius; NORMAL human-release gate (owner user-test); additive-only — zero behavior change to emulator sources.
- Requested At: 2026-07-15T16:20:00+09:00

---

- Request ID: REQ-M53-002
- From: MSX Master Agent (coordinator)
- To: MSX Developer Agent
- Milestone ID: M53 (DEC-0080)
- Type: Implementation + deterministic tests + evidence gates
- Scope: Implement `docs/m53-planner-package.md` slices S1→S5 in order. S1 scaffold FIRST and prove the pre-M53 suite byte-identical (247/247 + the additive CMake targets building) before any tool logic. Tool: self-contained `src/diskutils/` (NO sony_msx_core link, NO emulator headers in the shipped binary; the one BPB-equality unit test may link both), `msx_disk` exe OUTPUT_NAME msx-disk + POST_BUILD copy_if_different → `diskutils/`, binary gitignored + `diskutils/.gitkeep`, `src/CLAUDE.md` responsibility line per package. Layout per package §spec (PS1:82-133 source of truth; FAT F9 FF FF; empty root; zero data; NO timestamps/serial — SHA256 of --create output must equal the tools/format-blank-disk.ps1 blank). CLI per package (--create/--format [--force, size guard 737280, exit 3 refusal]/--read [--sector|--range, xxd-style]/--help; exit codes 0/1/2/3). Tests per package §5: 5 unit + 1 integration (emulator FDC mounts a tool-created image + FAT12 readback; msxdos23.dsk-derived golden skip-when-absent). Evidence gates: validate-assets, checksum-assets, `cmake --build build --config Debug` (all targets incl. msx_disk; verify diskutils/msx-disk.exe lands), full fast ctest (pre-M53 byte-identical + new tests), git-diff attestation that ZERO existing src/ files changed except `src/CLAUDE.md` (+ CMakeLists wiring + .gitignore). openMSX A/B + ZEXALL N/A (DEC-0080). Produce `docs/m53-implementation-report.md` content (return inline if your guardrail requires; coordinator persists). Do NOT commit.
- Constraints: single build tree; deterministic tests only; additive-only; STOP and escalate if emulator source edits (beyond the enumerated wiring files) become necessary.
- Requested At: 2026-07-15T16:36:00+09:00

---

- Request ID: REQ-M53-003
- From: MSX Master Agent (coordinator)
- To: MSX QA Agent
- Milestone ID: M53 (DEC-0080)
- Type: Regression assessment + sign-off recommendation
- Scope: Full QA of the uncommitted M53 working-tree state (tracked: .gitignore + CMakeLists.txt; untracked new: src/diskutils/*, diskutils/ incl. built msx-disk.exe + .gitkeep; gitignored: tests additions, src/CLAUDE.md line, docs reports). Handoffs: `docs/m53-planner-package.md` (charter), `docs/m53-implementation-report.md` (persisted developer report incl. coordinator spot-check note), DEC-0080, RESP-M53-002. QA must: (a) rebuild from the ONE build/ tree + full fast ctest, report real counts (expect 253/253; pre-M53 247 name-for-name unchanged); (b) verify build isolation — shipped msx_disk binary links no emulator lib (inspect CMake target links), emulator-src diff EMPTY over src/core|devices|machine|frontend, and the ONLY tracked edits are .gitignore + CMakeLists.txt; (c) independently reproduce AC-2 (fresh --create SHA256 == 6f1a79835e0421178b01207b196fa245c127c976fa0c6abc3aaa57e6b0ce5188 == a fresh tools/format-blank-disk.ps1 blank, -ExecutionPolicy Bypass if needed); (d) exercise the CLI exit-code matrix live (usage 1 / IO 2 / safety-refusal 3 / --force 0) + --read --sector/--range correctness vs known layout bytes; (e) read the 6 new tests for non-tautology (esp. bpb_matches_machine — confirm it would FAIL on a wrong BPB, and the FDC-mount integration reads through the real WD2793 datapath, not a direct buffer compare); (f) verify the POST_BUILD copy re-lands a deleted diskutils/msx-disk.exe; (g) confirm .gitignore covers the binary and .gitkeep keeps the folder; (h) ZEXALL optional (empty-diff-proven); (i) issue `docs/m53-qa-signoff.md` with Pass / Conditional Pass / Fail + residual risks + the owner user-test recipe. NON-DESTRUCTIVE discipline (M35): NO git checkout/reset/revert, NO src edits, NO parallel build trees; stop and report if a foreign writer appears.
- Constraints: LOW blast radius; NORMAL human-release gate (owner user-test precedes any release decision).
- Requested At: 2026-07-15T17:17:00+09:00

---

- Request ID: REQ-M54-001
- From: MSX Master Agent (coordinator)
- To: MSX Planner Agent
- Milestone ID: M54 (DEC-0081)
- Type: Planning — THOROUGH milestone package (planner-first; PLANNING-ONLY cycle per owner directive)
- Scope: Produce `docs/m54-planner-package.md` for macOS support per DEC-0081. Owner hard requirements: single codebase, both toolchains (MSVC + AppleClang), build-time auto-detect, no repo split. Package must contain: (1) PORTABILITY AUDIT — enumerate with concrete file:line every platform-sensitive surface (CMakeLists.txt:29/161/188 guards; any std::filesystem/path handling; type-width or UB-sensitive constructs; text-vs-binary file I/O sites esp. fixture/golden readers in tests and disk-image/SRAM/config I/O; SDL3 usage — confirm no Windows-only subsystem calls); (2) CMAKE PLATFORM MATRIX — if(MSVC)/if(WIN32)/if(APPLE) branch design, AppleClang warning-flag set (mirror of MSVC intent), single-config (Ninja/Makefiles: CMAKE_BUILD_TYPE + ctest without -C) vs multi-config invocation table, SDL3 vendored build on macOS (bootstrap path incl. arm64; universal-binary = explicit NON-goal or goal — decide and justify), msx-disk POST_BUILD copy on macOS (no .exe suffix — verify .gitignore already covers /diskutils/msx-disk); (3) LINE-ENDING POLICY — the repo has mixed CRLF/LF (git warnings observed); decide a .gitattributes policy that keeps golden fixtures byte-stable on both platforms WITHOUT churning the existing Windows checkout (analyze which tests read files in text mode, if any); (4) FILENAME-CASE AUDIT obligation (bios/roms/disks/games asset names + #include casing); (5) TOOLING STRATEGY — tools/*.ps1 on macOS (pwsh via Homebrew vs documented manual equivalents; which gates are dev-only), openMSX A/B natively on macOS (no WSL; note the A/B harness path assumptions), bootstrap-build.ps1 disposition (port vs a parallel bootstrap-build.sh vs manual commands — recommend one); (6) MACOS RUNBOOK (the deliverable the owner executes on the Mac): prerequisites (Xcode CLT, CMake, optional pwsh), exact clone/configure/build/ctest commands for BOTH Ninja and Xcode generator options (recommend one), expected results (all 3 binaries; fast ctest 253/253 + which tests may legitimately skip-when-absent given untracked assets on a fresh clone — enumerate them honestly: roms/disks/games content is NOT in git, so asset-gated tests will skip/fail — list which, and what the owner must copy over), evidence to capture for the Mac-side developer gate, first-compile troubleshooting playbook (missing-include pattern, SDL3 discovery, execution-policy N/A); (7) ACCEPTANCE CRITERIA — per-slice + milestone (Windows byte-identity preserved: rebuilding on Windows after any portability edits stays 253/253 with zero behavior diff; macOS: clean build all 3 targets + ctest parity modulo the enumerated asset-gated skips + determinism byte-identity of golden-oracle tests); (8) EVIDENCE GATES + the DEFERRAL protocol (developer/QA gates execute Mac-side; this cycle ends with the package + a recorded deferral note); (9) RISKS with mitigations; (10) explicit Assumptions with verification actions. NO code, NO CMake edits this cycle.
- Constraints: planner produces specs only; ground every claim in concrete repo paths (read CMakeLists.txt, tests/CMakeLists.txt, tools/bootstrap-build.ps1, .gitignore, .gitattributes-if-exists, a sample of fixture-reading tests); cite references/sdl3/ for any SDL3-platform claim; zero device/timing behavior in scope.
- Requested At: 2026-07-15T18:40:00+09:00
