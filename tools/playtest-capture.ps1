...
.SYNOPSIS
    ...
    then converts each frame dump to PNG via tools/frame-to-png.py.
...
.DESCRIPTION
    ...
    This wrapper:
      ...
      2. Converts each frame_N.txt dump to frame_N.png via frame-to-png.py
      3. Returns the output directory (ready for agent vision processing)

    Example:
      $result = & tools/playtest-capture.ps1 \
        -Goal "boot_to_basic" \
        -BiosDir bios \
        -Frames 100 \
...
.PARAMETER Goal
    Human-readable goal name (e.g., "boot_to_basic", "ys2_title").
...
.EXAMPLE
    ...
    PS> & tools/playtest-capture.ps1 -Goal "boot_to_basic" -BiosDir bios -Frames 100

    # Boot YS II and run for 30 frames
    PS> & tools/playtest-capture.ps1 -Goal "ys2_boot" -BiosDir bios -Disk disks/games/ys2/disk1.dsk -Frames 30

    # Continue from an existing script
    PS> & tools/playtest-capture.ps1 -Goal "ys2_title" -BiosDir bios -Disk disks/games/ys2/disk1.dsk \
          -InputScript "debug/playtest_ys2_boot_*.script" -Frames 50
#>
...
function Invoke-PlaytestRun {
    ...
    # Build sony_msx_headless command line
    $cmdArgs = @(
        "--debug-session",
        $BiosDir,
...
function Convert-FramesToPng {
    ...
    $pythonScript = "tools/frame-to-png.py"
    if (-not (Test-Path $pythonScript)) {
        Write-Error "frame-to-png.py not found at $pythonScript"
        return $false
    }
...
function Main {
    ...
    if (-not (Invoke-PlaytestRun -Emulator $emulator -BiosDir $BiosDir -Frames $Frames `
              -Disk $Disk -InputScript $InputScript -OutputDir $OutputDir)) {
        exit 1
    }
