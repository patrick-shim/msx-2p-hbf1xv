...
#     architectural state (PC/opcode/registers/flags) via tools/trace-diff.py.
#
...
#     tools/gen-m16-fdc-probe.py into tests/parity/m16_fdc_probe.bin), run from
#     a flat-RAM base with the IDENTICAL tests/parity/m16_boot.dsk (tools/
#     gen-m16-boot-disk.py; byte-identical to this emulator's default
...
Require-Path -Path $FdcProbeBin -Label "FDC probe program (run tools/gen-m16-fdc-probe.py first)"
Require-Path -Path $FdcDisk -Label "M16 boot disk fixture (run tools/gen-m16-boot-disk.py first)"
...
python tools/trace-diff.py $fdcA $fdcBRaw $fdcDiff
