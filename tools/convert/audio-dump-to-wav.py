...
`tools/convert/mem-to-audio.py` (M10-S5) is explicitly insufficient for this: it
...
mirroring tools/convert/frame-to-png.py's exact "genuinely decoded, not raw bytes"
...
Usage:
  python tools/convert/audio-dump-to-wav.py <dump.dump> -o <out.wav>
  python tools/convert/audio-dump-to-wav.py --self-check
"""
import argparse
import hashlib
import struct
...
# here in Python, mirroring tools/convert/frame-to-png.py's own cross-language fixture
# precedent).
...
def encode_wav(pcm, sample_rate=44100, channels=2, bits=16):
    ...
    A small, already-proven encoder duplicated from tools/convert/mem-to-audio.py's
    ...
    "either is acceptable" framing -- keeps tools/convert/mem-to-audio.py's own
...
def parse_audio_dump(text):
    ...
    here in Python, mirroring tools/convert/frame-to-png.py's own
    parse_frame_dump()/tools/convert/mem-to-png.py's own parse_region_from_dump()
