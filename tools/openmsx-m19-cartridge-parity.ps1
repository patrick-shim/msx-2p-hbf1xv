...
# authored `Generic8kB` cartridge image (tools/gen-m19-cartridge-probe.py) and
...
# tools/trace-diff.py) captures the read-back byte VALUES in the AF field --
...
Require-Path -Path $ProgramBin -Label "M19 cartridge probe program (run tools/gen-m19-cartridge-probe.py first)"
Require-Path -Path $CartridgeRom -Label "M19 synthetic cartridge image (run tools/gen-m19-cartridge-probe.py first)"
...
$lines += "- Probe: ``$ProgramBin`` + synthetic cartridge ``$CartridgeRom`` (4 x 8 KB ``Generic8kB`` banks, bank N's every byte == N), generated by ``tools/gen-m19-cartridge-probe.py``."
$lines += ""
...
$lines += "python tools/gen-m19-cartridge-probe.py"
$lines += "tools/openmsx-m19-cartridge-parity.ps1"
$lines += '```'
$lines += ""
