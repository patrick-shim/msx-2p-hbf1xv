...
#     tools/trace-diff.py). Because the OPLL is write-only (A-M17-5), this is
...
Require-Path -Path $ProgramBin -Label "YM2413 probe program (run tools/gen-m17-ym2413-probe.py first)"
Require-Path -Path $RegsList -Label "YM2413 probe regs list (run tools/gen-m17-ym2413-probe.py first)"
...
$lines += "- Probe: ``$ProgramBin`` ($($regPairs.Count) two-port register writes, generated by ``tools/gen-m17-ym2413-probe.py``), covering `$00-`$07 (user patch), `$0E (rhythm control), `$10-`$18/`$20-`$28 (F-Num/Block/Key-on), `$30-`$38 (instrument/volume, incl. `$36-`$38 rhythm volumes)."
$lines += ""
...
$lines += "python tools/gen-m17-ym2413-probe.py"
