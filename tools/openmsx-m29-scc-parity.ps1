...
param(
    [string]$ProgramBin = "tests/parity/m29_scc_probe.bin",
    [string]$CartridgeRom = "tests/parity/m29_scc.rom",
    [string]$BaseHex = "C000",
...
# synthetic KonamiSCC image (tools/gen-m29-scc-probe.py) and run the
# IDENTICAL Z80 driver program, whose every readback lands in AF -- an empty
# per-instruction diff (tools/trace-diff.py) is simultaneously the
...
Require-Path -Path $ProgramBin -Label "M29 SCC probe program (run tools/gen-m29-scc-probe.py first)"
Require-Path -Path $CartridgeRom -Label "M29 synthetic KonamiSCC image (run tools/gen-m29-scc-probe.py first)"
...
python tools/trace-diff.py $traceA $traceBRaw $traceDiff
...
python tools/trace-diff.py $traceA $emptyPath $selfcheckBlockedOut
...
python tools/trace-diff.py $traceA $corruptedPath $selfcheckDivergedOut
...
$lines += "- Probe: ``$ProgramBin`` + synthetic KonamiSCC image ``$CartridgeRom`` (8 x 8 KB banks, bank N's every byte == N), generated by ``tools/gen-m29-scc-probe.py`` (annotated instruction listing + expected readbacks in its docstring)."
$lines += ""
...
$lines += "python tools/gen-m29-scc-probe.py"
$lines += "tools/openmsx-m29-scc-parity.ps1"
$lines += '```'
$lines += ""
