...
param(
    ...
    [string]$InputScript = "tools/input-scripts/cartridge-evidence.script",
    [int]$Frames = 1100,
    [string]$FrameName = "m31-rc-f1100.frame",
    [string]$PngOut = "debug/frames/m31-rc-f1100.png"
)

# ---------------------------------------------------------------------------
# M32-closure cartridge gameplay-evidence capture — THE RECORDED RECIPE
# (QA condition, docs/m32-qa-signoff.md; docs/m32-implementation-report.md
# "Evidence supersession (M32 closure)").
#
# Regenerates `debug/frames/m31-rc-f1100.png` deterministically:
#   * real Sony HB-F1XV BIOS cold boot from <BiosDir>;
#   * `roms/game.rom` loaded EXPLICITLY as mapper type `Konami`
...
#   * scripted input from `tools/input-scripts/cartridge-evidence.script`
...
#   * frame 1,100 (gameplay: player sprite in the opening scene, HUD visible)
#     dumped and converted to PNG via tools/convert/frame-to-png.py (deterministic
#     encoder — byte-identical PNG for identical pixels).
#
# HISTORY / SUPERSESSION NOTE: the ORIGINAL m31-rc-f1100.png was
...
& $Emulator --debug-session $BiosDir 0 --stock --frames $Frames `
...
python tools/convert/frame-to-png.py $FramePath -o $PngOut
