...
# ---------------------------------------------------------------------------
# M34 openMSX audio A/B harness -- the ultrasonic-silence property
...
#   game ROM with SPACE injected via the debugger `keymatrixdown`/`keymatrixup` surface
#   (MSX matrix row 8 bit 0; the m32 Tcl-injection precedent,
#   tools/openmsx/split-screen-ab.ps1) at emulated-time ~10s/25s/35s (~frames
...
#
#   Comparator: tools/analyze/audio-ab.py applies the IDENTICAL
...
Require-Path $GameRom "game ROM (local dev asset)"
...
Write-Host "== side A: the M34 ultrasonic regression oracle (ours) =="
...
after time $RecordStartSeconds { record start -audioonly "$WslWork/openmsx_game_transition" }
...
"@
Set-Content -Path "$WorkDir/side_b_game.tcl" -Value $tcl -Encoding ascii
Write-Host "== side B: openMSX Sony_HB-F1XV + game ROM via WSL (record sound) =="
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
cmd /c "wsl.exe -- openmsx -machine Sony_HB-F1XV -carta `"$WslRoot/roms/game.rom`" -script `"$WslWork/side_b_game.tcl`" 2>nul" | Out-Null
$ErrorActionPreference = $prevEap
Require-Path "$WorkDir/openmsx_game_transition.wav" "side-B recording (record sound failed?)"
...
python tools/analyze/audio-ab.py "$WorkDir/openmsx_game_transition.wav" |
