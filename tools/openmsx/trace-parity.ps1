...
param(
    ...
    [string]$TraceA = "build/parity_trace_A.txt",
    [string]$TraceBRaw = "build/parity_trace_B.txt",
    [string]$DiffOut = "docs/m10-parity-trace-diff.md",
...
# ---------------------------------------------------------------------------
# M10-S4 openMSX per-instruction opcode-trace parity harness (resolves R5).
#
# Produces a REAL captured architectural-state trace diff between:
#   A = this emulator (via the M10-S1 trace-export, --parity-trace mode), and
#   B = openMSX 19.1 on WSL, single-stepped over the SAME RAM-only program at
#       the SAME base address with the SAME forced initial CPU vector.
#
# It NEVER fabricates parity: the actual diff outcome (empty or the concrete
# field mismatches) is written to $DiffOut by tools/analyze/trace-diff.py. If openMSX
# cannot be driven to step the program, the captured openMSX output is left in
# $TraceBRaw and the diff tool reports BLOCKED (empty B side).
# ---------------------------------------------------------------------------
...
Require-Path -Path $ProgramBin -Label "parity program"
...
# --- Tcl driver: boot to a RAM-ready state, force the documented initial CPU
#     vector, write the program into mapped RAM, then single-step via chained
#     break-callbacks (the mechanism that makes `step` actually advance PC in a
#     headless script context — the documented old blocker was stepping in the
#     un-broken -script startup context). ---
$tcl = @"
set renderer none
set throttle off
set ::PROG { $byteList }
...
$runCmd = "echo $encoded | base64 -d > /tmp/omx_trace_parity.tcl && " +
          "timeout 120 openmsx -machine $ProbeMachine -command 'set renderer none' " +
          "-script /tmp/omx_trace_parity.tcl 2>&1 | grep -a -E 'OMTRACE|OMSETUP' || true"
...
Write-Output "Trace B (openMSX) instructions captured: $omtraceCount"
...
python tools/analyze/trace-diff.py $TraceA $TraceBRaw $DiffOut
$diffExit = $LASTEXITCODE

switch ($diffExit) {
    0 { Write-Output "R5 RESULT: ARCHITECTURAL PARITY (empty diff). See $DiffOut" }
    1 { Write-Output "R5 RESULT: ARCHITECTURAL DIVERGENCE captured. See $DiffOut" }
    default { Write-Output "R5 RESULT: BLOCKED / not comparable. See $DiffOut and $TraceBRaw" }
}
exit $diffExit
