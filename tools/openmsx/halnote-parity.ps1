...
# tools/gen/halnote-probe.py's own docstring): rather than swapping a
...
$lines += ""
$lines += "Separately confirmed by direct source read (not required for this run's methodology, but verifies the planner's own flagged open question): ``references/openmsx-21.0/src/memory/Rom.cc:202-208`` -- a machine-XML ``<sha1>`` mismatch on a ``<ROM>`` element prints a CliComm WARNING only (``motherBoard.getMSXCliComm().printWarning(...)``), it is never a hard rejection/throw. **The SHA1 tag is ADVISORY, not enforced** -- so a synthetic-image swap technique (tools/gen/halnote-probe.py) remains available for any FUTURE milestone that needs one; it was not required for THIS milestone's own evidence given the real-firmware-identity finding above."
$lines += ""
...
$lines += "python tools/gen/halnote-probe.py"
$lines += "tools/openmsx/halnote-parity.ps1"
$lines += '```'
$lines += ""
