...
param(
    ...
    [string]$ZexallCom = "src/external/zexall/zexall.com",
    [string]$ZexdocCom = "src/external/zexall/zexdoc.com",
...
if (-not (Test-Path -LiteralPath $ZexallCom)) {
    Write-Blocked -Reason "zexall.com not found: $ZexallCom (M24-S0 must commit src/external/zexall/ first)."
    exit 2
}
if (-not (Test-Path -LiteralPath $ZexdocCom)) {
    Write-Blocked -Reason "zexdoc.com not found: $ZexdocCom (M24-S0 must commit src/external/zexall/ first)."
    exit 2
}
