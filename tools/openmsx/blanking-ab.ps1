...
# ---------------------------------------------------------------------------
# M34 openMSX A/B harness -- cartridge-title R#1 BL (display-enable) transitions
...
# over tools/input-scripts/blanking-transition.script (SPACE 300/600 lineage
...
# dialogue; UP held from 4100 walks the player north off the opening
# screen). Artifacts: debug/logs/m34-bl-probe.txt (every R#1 bit6
# transition with frame + display line) and debug/frames/m34-bl-f*.png.
...
proc tick {} {
    ...
    incr FRAME
    # --- frame-indexed input (mirrors tools/input-scripts/blanking-transition.script) ---
...
Write-Host "== side B: openMSX Sony_HB-F1XV + game ROM via WSL (R#1 BL poll) =="
$prevEap = $ErrorActionPreference
$ErrorActionPreference = "Continue"
cmd /c "wsl.exe -- openmsx -machine Sony_HB-F1XV -carta `"$WslRoot/$GameRom`" -script `"$WslWork/side_b_mg_bl.tcl`" 2>nul" | Out-Null
...
Write-Host "== compare against side A: debug/logs/m34-bl-probe.txt =="
