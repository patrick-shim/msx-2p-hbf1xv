param(
    ...
    [string]$InputScript = "tools/aleste-play-evidence-input.script"
)

# ---------------------------------------------------------------------------
# M34-closure Aleste 2 gameplay-evidence capture — THE RECORDED RECIPE
# (QA condition, docs/m34-qa-signoff.md §4; the M32 AC-6 supersession pattern).
#
# Regenerates the four committed Aleste 2 gameplay frames deterministically:
#   * real Sony HB-F1XV BIOS cold boot from <BiosDir>;
#   * roms/aleste.rom loaded EXPLICITLY as mapper type KonamiSCC (auto-ID
...
#   * scripted input from tools/aleste-play-evidence-input.script
#     (HBF1XV-INPUT-SCRIPT v1): SPACE held ~15 frames at frames 600/1500/
#     2100/2700 (cycle stamps (F-1)x59,736, the committed metalgear-script
...
#
# HISTORY / SUPERSESSION NOTE: the ORIGINAL m32-aleste-play-*.png frames were
...
#       (Aleste drives R#1 bit6 low at display line 13 every gameplay frame —
...
Require-Path $Emulator "emulator (build first: tools/bootstrap-build.ps1)"
...
foreach ($f in $frames) {
    $name = "m32-aleste-play-f$f.frame"
    $verify = "m32-aleste-play-f$f-verify.frame"
    $png = "debug/frames/m32-aleste-play-f$f.png"

    Write-Host "== frame $f : capture run 1/2 =="
    & $Emulator --debug-session $BiosDir 0 --frames $f `
    ...
    python tools/frame-to-png.py "debug/frames/$name" -o $png
    if ($LASTEXITCODE -ne 0) { throw "frame $f PNG conversion failed" }
    Write-Host "== frame $f : $png regenerated (sha256 $($h1.Substring(0,16))...) =="
}

Write-Host "== done: four m32-aleste-play gameplay PNGs regenerated from the recorded recipe =="
