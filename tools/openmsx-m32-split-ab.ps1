...
# 512-byte Z80 program (tools/gen-m32-split-probe.py) injected into a live
...
#
# Comparison gate (REGION-STRUCTURAL, tools/m32-split-ab-compare.py --
...
python tools/gen-m32-split-probe.py -o $WorkDir
...
python tools/m32-split-ab-compare.py --ours "$WorkDir/ours_split.frame" --self-check
if ($LASTEXITCODE -ne 0) { throw "comparator self-check FAILED to flag a corrupted region" }

Write-Host "== structural A/B: split arm =="
python tools/m32-split-ab-compare.py --ours "$WorkDir/ours_split.frame" --openmsx "$WorkDir/openmsx_split_raw.png" |
...
python tools/m32-split-ab-compare.py --openmsx "$WorkDir/openmsx_noie1_raw.png" --expect-no-split
