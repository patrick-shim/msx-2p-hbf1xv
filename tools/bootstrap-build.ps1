# bootstrap-build.ps1 — THE single way to build and test this project (M33, DEC-0041).
#
...
# built ONCE from the vendored, zlib-licensed references/sdl3/ source into
...
#
# Usage:
#   powershell -ExecutionPolicy Bypass -File tools/bootstrap-build.ps1              # configure + build
#   powershell -ExecutionPolicy Bypass -File tools/bootstrap-build.ps1 -RunTests    # ... + ctest
#   powershell -ExecutionPolicy Bypass -File tools/bootstrap-build.ps1 -Config Release
#
...
if (-not (Test-Path (Join-Path $sdlInstall "cmake/SDL3Config.cmake")) -and
    ...
    Write-Host "[bootstrap] SDL3 dev install missing -> building once from references/sdl3 (zlib license, M26 recipe)"
