...
`tools/mem-to-png.py` (M10-S5) is explicitly insufficient for this: it
...
Determinism (required, guaranteed by construction -- mirrors mem-to-png.py's
...
Usage:
  python tools/frame-to-png.py <dump.frame> -o <out.png>
...
def _zlib_stored(data):
    ...
    Identical algorithm to tools/mem-to-png.py's own _zlib_stored (small,
...
def parse_frame_dump(text):
    ...
    precedent in tools/mem-to-png.py (never copied from any external source).
    """
    lines = text.split("\n")
