...
// Suite: Machine_Hbf1xvM29Scc_System (M29-S6, backlog G1)
//
...
// itself over the M11 bus) banks, enables the SCC (0x3F at 0x9000), and
// writes a waveform/period/volume/enable set; the frame loop runs via
// step_cpu_instruction() + on_vsync_boundary() (the DEC-0034-proven loop
// shape -- never run_frame(), the R-M25-5 double-count hazard); then the
// M29-S5 MachineAudioMixer pumps N samples and the PCM sequence matches a
...
RunResult run_session() {
    ...
    // The DEC-0034-proven frame loop shape: step_cpu_instruction() to each
    // frame boundary, then on_vsync_boundary() -- never run_frame().
    ...
    // Pump N samples through the REAL M29-S5 mixer (PSG left at its silent
...
int main() {
    ...
    std::cout << "All Machine_Hbf1xvM29Scc_System cases passed\n";
    return 0;
}
