...
// Suite: System_Hbf1xvRacingStability
//
// Permanent regression oracle for a racing title's racing-view flicker: the
// command-row sink (VdpRenderSyncAdapter::on_commit_up_to) used to seal rows
// far AHEAD of the render beam with COMMIT-TIME register state (frame-start
// R#26/R#27 horizontal scroll + the instantaneous SAT). The game bends its
// straight-road page-0 canvas by rewriting R#26/#27 per scanline band
// mid-frame; once per ~13-frame beat its dashboard blit landed while the
// beam was at raster ~2, sealing ~145 rows with the frame-start scroll -> a
// full-scene flash (straight road, shifted dashboard, car sprite absent).
// openMSX 21.0 over the identical window: 0/40 anomalous; ours pre-fix:
// 3/40 anomalous on an exact 13-frame period. The beam clamp
...
// effect (Yamaha V9958 VDP fact sheet per-scanline model; openMSX renders
// only the backlog per command write, VDPVRAM.hh -- effect only, never
// copied). (DEC-0091-AMENDMENT-A)
//
// RECIPE (the pinned repro, in-process equivalent of
//   sony_msx_headless --debug-session bios 0 --stock
//     --disk games/disks/racing/racing-d2.dsk --disk games/disks/racing/racing-d3.dsk
//     --swap-disk-frame 19000 --input-script <the script embedded below>):
...
//
// ORACLE (a self-referential rate metric -- no absolute pixel
...
// fails this exact test with 3/40 anomalous frames.
//
...
// skip-when-absent discipline as the other asset-gated tests.
...
// The pinned repro input script, embedded VERBATIM (parsed by the
// production parser, exactly as
...
// Dashboard band sampling (the anomaly discriminator): rows [150, height),
...
int main() {
    ...
    // test must print the identical value).
    ...
    // --- Per-frame classification (anomalous-frame rate metric). ---
    int anomalous = 0;
    int transitions = 0;
    ...
    std::cout << "All System_Hbf1xvRacingStability cases passed\n";
    return 0;
}
