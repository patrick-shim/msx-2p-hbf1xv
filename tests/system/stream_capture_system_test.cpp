...
// Suite: Hbf1xvStreamCapture_System (DEC-0052 live stream-capture).
//
...
// proven hbf1xv_fdc_integration_test so the FDC case drives a REAL sector
...
// Mirror of the read probe for a REAL Write Sector(LBA0) over the full M11 bus:
// Restore to track 0, then Write Sector (Type II, single) streaming 512 source
// bytes from kBuffer to the WD2793 data register (0x7FFB), polling the same
// active-low DRQ line (0x7FFF bit 7) the disk ROM does. Drives the machine-armed
// FDC WRITE observer end to end so the stream_<id>_fdcwrite.log is produced by
// exactly the path a live save takes (DEF-M47-DISKWRITE observer, DEC-0052 arm).
std::vector<std::uint8_t> build_restore_write_sector_probe() {
    Prog p;
    p.emit({0x3E, 0x08});        // LD A,0x08
    p.emit({0x32, 0xFF, 0xFF});  // LD (0xFFFF),A  ; page1->slot3-2(FDC)
    p.emit({0x3E, 0x00});        // LD A,0x00
    ...
    p.emit({0x3E, 0x01});        // LD A,0x01
    p.emit({0x32, 0xFA, 0x7F});  // LD (0x7FFA),A  ; SR = 1
    p.emit({0x3E, 0xA0});        // LD A,0xA0
    p.emit({0x32, 0xF8, 0x7F});  // LD (0x7FF8),A  ; Write Sector (Type II, single)
    p.emit({0x21, static_cast<std::uint8_t>(kBuffer & 0xFF),
            static_cast<std::uint8_t>(kBuffer >> 8)});  // LD HL,kBuffer (source data)
    p.emit({0x01, 0x00, 0x02});  // LD BC,0x0200
    const std::size_t write_wait = p.here();
    p.emit({0x3A, 0xFF, 0x7F});  // LD A,(0x7FFF)
    p.emit({0xE6, 0x80});        // AND 0x80 (!DRQ)
    p.emit_jr_back(0x20, write_wait);
    p.emit({0x7E});              // LD A,(HL)      ; next source byte
    p.emit({0x32, 0xFB, 0x7F});  // LD (0x7FFB),A  ; -> data register
    p.emit({0x23});              // INC HL
    p.emit({0x0B});              // DEC BC
    p.emit({0x78});              // LD A,B
    p.emit({0xB1});              // OR C
...
int main() {
    ...
    const fs::path root_d2 = base / "D2";  // DEF-M47-DISKWRITE FDC write log
    ...
    // --- Run D2 (DEF-M47-DISKWRITE + DEC-0052 arm): per-byte + per-sector FDC
    //     WRITE log while streaming. This is the WRITE counterpart of Run D and
    //     the END-TO-END proof that arming the machine-level stream capture (the
    //     same path SDL3 F10 and headless --snapshot take) installs BOTH the read
    //     AND the write FDC observer: arm, drive a REAL Restore + Write Sector(LBA0)
    //     over the full bus (streaming 512 known source bytes), and confirm
    //     <root>/traces/stream_<id>_fdcwrite.log is produced with per-byte lines
    //     (value / committed-vs-substituted / data_index / target LBA) AND a
    //     per-sector FINISH line (CHS/LBA/CRC) whose crc32 matches an independent
    //     CRC of the 512 bytes that actually landed on disk.
    {
        Hbf1xvMachine d2;
        d2.cold_boot();
        d2.set_debug_root(root_d2);
        d2.map_flat_ram();
        // Deterministic, non-constant source pattern the CPU streams to the drive.
        std::array<std::uint8_t, 512> src{};
        for (int i = 0; i < 512; ++i) {
            src[static_cast<std::size_t>(i)] = static_cast<std::uint8_t>((i * 7 + 0x11) & 0xFF);
        }
        d2.load_memory(kBuffer, src.data(), static_cast<std::uint32_t>(src.size()));
        const std::vector<std::uint8_t> probe = build_restore_write_sector_probe();
        d2.load_memory(kBase, probe.data(), static_cast<std::uint32_t>(probe.size()));
        d2.cpu().state().regs().pc = kBase;
        d2.set_stream_capture_enabled(true);  // id = f0_c0 (cold_boot -> cycle 0)
        const std::string sid = d2.stream_capture_id();

        int guard = 0;
        while (!d2.cpu().state().halted() && guard < 4'000'000) {
            d2.step_cpu_instruction();
            ++guard;
        }
        expect(d2.cpu().state().halted(), "FdcWrite_Probe_ReachesHalt");
        expect(!d2.stream_capture_active(), "FdcWrite_Stream_FinalizedOnHalt");

        const fs::path fdcwrite_log = root_d2 / "traces" / ("stream_" + sid + "_fdcwrite.log");
        const std::string wlog = read_file(fdcwrite_log);
        // (i) the WRITE observer WAS armed by the machine-level stream capture ->
        // the fdcwrite.log exists and is non-empty (the exact thing that was
        // reported missing from a live F10 capture).
        expect(!wlog.empty(), "FdcWrite_Log_NonEmpty");
        // (ii) per-byte trace lines: first data byte at index 0, a value field, and
        // the genuine-CPU marker (prompt-polled write -> committed, not substituted).
        expect(wlog.find("idx=0 val=") != std::string::npos, "FdcWrite_Log_HasFirstByte");
        expect(wlog.find(" cpu ") != std::string::npos, "FdcWrite_Log_HasCommittedByte");
        // The write targets track0/side0/sector1 == LBA 0, command 0xA0.
        expect(wlog.find("chs=0/0/1") != std::string::npos, "FdcWrite_Log_HasChs");
        expect(wlog.find("cmd=A0") != std::string::npos, "FdcWrite_Log_HasWriteCommand");
        expect(wlog.find("frame=0") != std::string::npos, "FdcWrite_Log_HasFrame");
        // (iii) exactly one per-sector FINISH line at LBA 0 with a crc32 that
        // matches an independent CRC of the 512 bytes now on disk.
        expect(wlog.find("FINISH cmd=A0 lba=0 chs=0/0/1") != std::string::npos,
               "FdcWrite_Log_HasFinishLine");
        std::array<std::uint8_t, 512> landed{};
        expect(d2.disk_image().read_chs(0, 0, 1, landed.data()), "FdcWrite_Image_ReadChs_Ok");
        const std::string want_crc32 = "crc32=" + hex8(crc32_ref(landed.data(), landed.size()));
        expect(wlog.find(want_crc32) != std::string::npos, "FdcWrite_Log_FinishCrcMatchesSector");
        // The bytes that landed are exactly the source pattern the CPU streamed
        // (proves the trace describes a real, faithful write, not an aborted one).
        expect(landed == src, "FdcWrite_LandedBytesMatchSource");
        std::cout << "fdcwrite log first line: " << wlog.substr(0, wlog.find('\n'))
                  << "\nfdcwrite log finish line: " << last_line(wlog) << "  (want " << want_crc32
                  << ")\n";
    }
    ...
    std::cout << "All Hbf1xvStreamCapture_System cases passed\n";
    return 0;
}
