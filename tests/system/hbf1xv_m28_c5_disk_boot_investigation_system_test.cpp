...
// (docs/m28-planner-package.md §2.1a). Genuine, open-ended investigation: a
// real, unmodified BIOS + a REAL MSX-DOS system disk (disks/msxdos22.dsk,
// unlike M16's investigation, which only had the FDC's own default
// synthesized medium) is cold-booted for a long, deterministic budget, with
// and without a scripted keypress sequence (M27's InputScriptPlayer), while
// tracking (a) max PC, (b) whether page 1's resolved sub-slot EVER becomes
// (primary=3, sub=2) -- i.e. whether the disk-ROM window is ever paged in at
// all -- and (c) the WD2793 diagnostic counters (Restore/Read-Sector
// engagement). This is diagnostic instrumentation, NOT a re-implementation of
// BIOS/disk-ROM logic (no GPL/proprietary disassembly reproduced anywhere in
// this repo -- only OBSERVED, empirical CPU/bus behaviour of the real,
// unmodified local bios/ + disks/ assets already used since M10-M27).
//
// Honest, dual-outcome acceptance (planner §2.1a): this test asserts EXACTLY
// what is genuinely, deterministically observed. See
// docs/m28-implementation-report.md / docs/m28-parity-trace-diff.md (if
// produced) for the full investigation writeup and the openMSX A/B
// comparison at the first-divergence point.
...
// Generous, deterministic diagnostic budget. Per M16's own diagnostic run
// (docs/m16-parity-trace-diff.md: "a few million instructions" reaches a
// steady idle prompt loop, confirmed stable through 20,000,000), this
// budget is chosen to comfortably span the RAM-sizing loop, any ROM
// init/cold-start scan, AND settle into whatever steady state the BIOS/
// BASIC/disk-ROM combination reaches -- while keeping the committed test's
// wall-clock cost small (empirically measured below in the implementation
// report).
...
DiagnosticResult run_diagnostic(const std::vector<std::uint8_t>& disk_bytes,
    ...
    for (int i = 0; i < kDiagnosticInstructions; ++i) {
        ...
        // debug_sub_slot_register(3) (the same field the M17 integration
        // test's FmMusicRomGuard already exercises for slot 3-3/page 1;
        // here we check for sub==2, the disk-ROM sub-slot, XML:161-176).
...
int main() {
    const std::vector<std::uint8_t> msxdos22 = read_file(std::string(SONY_MSX_DISKS_DIR) + "/msxdos22.dsk");
    expect(msxdos22.size() == 737280, "Msxdos22Dsk_RealAssetPresent_737280Bytes");
    if (msxdos22.empty()) {
        std::cerr << "FATAL: disks/msxdos22.dsk not found under " << SONY_MSX_DISKS_DIR
                  << " -- cannot run the C5 investigation without the real asset.\n";
        return 1;
    }

    // --- Case 1: real MSX-DOS disk mounted, NO scripted input (idle boot). ---
    const DiagnosticResult idle = run_diagnostic(msxdos22, {});
    std::cerr << "[C5] idle-boot (real msxdos22.dsk, no input): max_pc=0x" << std::hex << idle.max_pc
    ...
    const DiagnosticResult with_space = run_diagnostic(msxdos22, space_hold);
    std::cerr << "[C5] scripted-input boot (real msxdos22.dsk, SPACE held T=1e6..2e6): max_pc=0x"
    ...
    const DiagnosticResult idle_repeat = run_diagnostic(msxdos22, {});
    ...
    // --- Honest, dual-outcome recording (planner §2.1a) -----------------
    // NOTE: the specific assertions below are written to match whichever
    // outcome this investigation GENUINELY, empirically observes -- see
    // docs/m28-implementation-report.md for the full narrative and (if
    // outcome (a) is reached) docs/m28-parity-trace-diff.md for the openMSX
    // A/B evidence at the first-divergence point.
    ...
    std::cout << "All Hbf1xvM28C5DiskBootInvestigation_System cases passed\n";
    return 0;
}
