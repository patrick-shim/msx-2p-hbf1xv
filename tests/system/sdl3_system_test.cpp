...
// Suite: Hbf1xvSdl3_System
//
// The SDL3 frontend's dedicated system integration test: combines windowing/
// real-time-loop STRUCTURE (Sdl3App::init()/run_one_frame(), never
// run_interactive()), real video-texture blit, real audio-stream
// numeric pump, real `SDL_PushEvent`-injected input mapping (keyboard +
// PAUSE + joystick), the Speed-Controller/Ren-Sha-Turbo bindings, and
// cartridge/disk CLI loading -- ALL together in ONE deterministic,
// bounded-frame scenario, mirroring the
// speed_pause_rensha_system_test.cpp "everything together"
// structure. Runs entirely
...
int main() {
    ...
    // the CPU (the hardware PAUSE gate) for an ENTIRE subsequent frame --
    ...
    // --- Speed-Controller / Ren-Sha-Turbo bindings. ---
    ...
    std::cout << "All Hbf1xvSdl3_System cases passed\n";
    return 0;
}
