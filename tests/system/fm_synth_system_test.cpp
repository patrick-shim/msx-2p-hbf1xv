...
// Suite: Machine_Hbf1xvFmSynth_System
//
// End-to-end register-driven FM synthesis over the REAL machine bus: an
// in-RAM Z80 program (fetched and executed by the real CPU) programs the
// YM2413 through genuine OUT (#7C)/OUT (#7D) instructions; the
// MachineAudioMixer then produces the mixed PCM stream (silent PSG + no SCC
// + the machine's own OPLL as the third source).
//
// Stages: (A) key-on -> machine-level pitch measured against the YM2413
// pitch-formula oracle f = F-Num * 2^BLOCK * MUL * 49716 / 2^19, expressed
// in the output domain (native period x 72/81); (B) key-off via a second CPU
// program -> release-bounded decay to EXACT digital silence; (C) a rhythm
// burst (the Yamaha-recommended setup) -> non-silent output. Deterministic
// oracle: the ENTIRE session (all three stages' PCM) is byte-identical
...
SessionResult run_session() {
    ...
    // Stage C program: the Yamaha-recommended rhythm setup + all five
    // drums keyed.
...
int main() {
    ...
    // --- Stage A: audible, and pitched per the pitch-formula oracle. Native
    ...
    //     effective rate 34 with this note's Rks; envelope closed form
    //     (1<<(14-8))*85 = 5440 native ticks (~4836 output samples), well
    //     inside the bound asserted below: the final quarter of 16384
    //     samples must be exactly zero. ---
    {
    ...
    std::cout << "All Machine_Hbf1xvFmSynth_System cases passed\n";
    return 0;
}
