...
// Suite: Hbf1xvDebugTooling_System -- the dedicated "everything together"
// system test for the production debug/test tooling.
//
// Combines the debug-tooling features in ONE deterministic, bounded,
// real-BIOS-boot scenario -- the exact composition the headless
// --debug-session mode (and the SDL3-side wiring) drive in production:
//   1. State-dump + CPU-trace CLI wiring.
//   2. Event-log CLI wiring, with event logging enabled BEFORE cold_boot()
//      so the Reset event is captured (shares the state-dump mechanism).
//   3. Scripted-input driving through the SAME CPU sub-loop.
...
// Mirrors the --debug-session mode's FULL combination exactly: event
// logging enabled BEFORE cold_boot() (so the Reset event is recorded at
// sequence 0), real BIOS asset loading, CPU trace enabled, a real input
// script driven through the SAME step_cpu_instruction() sub-loop, then all
// three write_* calls at the end.
...
int main() {
    ...
    // --- Item 2: event-log is real, well-formed, with the Reset event
    // genuinely present at sequence 0 (enable-before-cold_boot ordering). ---
    ...
    std::cout << "All Hbf1xvDebugTooling_System cases passed\n";
    return 0;
}
