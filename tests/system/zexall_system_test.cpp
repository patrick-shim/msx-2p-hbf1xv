...
// Suite: Hbf1xvZexall_System (M24-S3, backlog C3 -- ZEXDOC/ZEXALL full
// parity sweep, THE first point any real GPL binary is loaded this cycle).
//
// Loads the REAL, committed src/external/zexall/zexall.com and (separately)
// src/external/zexall/zexdoc.com and runs each to genuine CP/M warm-boot
...
//      count (not the full per-group parser -- that is tools/analyze/zexall-report.py,
...
//      table is produced by tools/analyze/zexall-report.py from this test's/the
...
std::vector<std::uint8_t> read_file_or_die(const std::string& path) {
    ...
    if (!in) {
        std::cerr << "FATAL: cannot open required ZEXALL/ZEXDOC asset: " << path
                  << " (SONY_MSX_ZEXALL_DIR wiring or src/external/zexall/ contents missing)\n";
        std::exit(1);
    }
