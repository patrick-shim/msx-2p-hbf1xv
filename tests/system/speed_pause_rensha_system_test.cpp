...
// Suite: System_Hbf1xvSpeedPauseRensha_System
//
// The dedicated "everything together" system integration test: combines all
// three HB-F1XV-specific
...
//   VBlank windows (pause_controller().on_vsync() called directly --
//   run_frame() is never called in this test).
...
//   observable effect at any sampled cycle).
...
// The same 10-byte deterministic counter-increment loop used by the
// pause-controller integration tests: LD A,(0010) / INC A / LD (0010),A / JP 0000.
...
int main() {
    ...
    // ================= Phase 3: Ren-Sha Turbo (held + negative control) =======
    {
        ...
        //     sampled across many real CPU-driven cycles. ---
    ...
    std::cout << "All System_Hbf1xvSpeedPauseRensha_System cases passed\n";
    return 0;
}
