...
int main() {
    ...
    // removed). It is the only MSX-DOS system floppy present, so the two-disk
    // lists below intentionally use it for BOTH slots -- the multi-disk swap seam
    // (index rotation + media-change signalling) is content-agnostic, so a
    // same-file two-entry list exercises it identically. disks/ content is
    // untracked, so the disk-dependent cases are guarded on `have_disk`: when the
    // asset is absent they SKIP, while the pure no-disk cases still run and PASS.
    const bool have_disk =
        std::filesystem::exists(std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk");
    if (!have_disk) {
        std::cout << "SKIP (disk-dependent cases): disks/msxdos23.dsk not present -- "
                     "skip-when-absent asset guard (no-disk cases still run below)\n";
    }
    ...
    if (have_disk)
    {
        const std::vector<std::string> disk_paths{std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk"};
    ...
    if (have_disk)
    {
        const std::vector<std::string> disk_paths1{std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk",
                                                    std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk"};
        ...
        const std::vector<std::string> disk_paths2{std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk",
                                                    std::string(SONY_MSX_DISKS_DIR) + "/msxdos23.dsk"};
