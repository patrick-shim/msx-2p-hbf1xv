...
// R-M27-4: for every one of the 72 entries in
...
int main() {
    ...
    // The FULL 72-entry correspondence, independently re-derived from
    ...
    static constexpr std::array<Pair, 72> kPairs{{
    ...
    expect(kPairs.size() == Sdl3InputMapper::scancode_map().size(),
           "PairTable_SameSizeAs_Sdl3InputMapperScancodeMap_72Entries");
    ...
    expect(all_agree, "AllSeventyTwoEntries_ScancodeTable_AgreesWithKeyNameTable_ExactRowColumnMatch");
