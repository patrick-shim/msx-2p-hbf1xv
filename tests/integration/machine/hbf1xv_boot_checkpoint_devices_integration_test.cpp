...
// Suite: Machine_Hbf1xvBootCheckpointDevices_Integration
//
// With the core I/O devices real (PSG #A0-A2, RTC #B4/B5, PPI/keyboard #A8-AB,
// VDP), the BIOS boot advances past ~PC 0x043C -- the point where the BIOS
// performs its first device read, previously the limit of verified boot
// progress. This test boots the real BIOS, single-steps a
...
//   3. boot ADVANCES strictly beyond the first-device-read boundary 0x043C
//      (the max PC reached during the checkpoint window exceeds it).
//
// The exact reached PC is DERIVED here (self-consistent + deterministic), not
// guessed; the value is emitted for the QA / openMSX A/B cross-check.
