...
// Suite: Machine_Hbf1xvPerLineLatch_Integration (M32-S2, Defect A of
// DEC-0039, docs/m32-planner-package.md §3-S2 / test matrix row 3)
//
// Machine-level L+2 latch rule (§2.3, RE-DERIVED M39/DEC-0060): a hooked VDP
// write while the raster is on display line L takes effect from line L+2 --
// lines <= L+1 keep the pre-write state. This is the openMSX sync-before-change
// protocol at LINE granularity WITH openMSX's active-display LEFT-MARGIN
// rounding (references/openmsx-21.0/src/video/PixelRenderer.cc:566-567,
// `renderUntil` limitY = (limitTicks + TICKS_PER_LINE - 400)/TICKS_PER_LINE),
// which lands the commit one display line later than a raw scanline boundary.
// The prior L+1 oracle here encoded the pre-M39 raw-boundary convention; L+2 is
// the openMSX-grounded value, calibrated against openMSX 19.1 Sony_HB-F1XV raw
// scroll-shooter gameplay captures (docs/m39-fix-plans.md follow-up; the top-HUD raster split
// lands exactly on openMSX's scanline). Cases:
//   1. R#23 rewrite at display line 100 => sealed frame rows [0,102) at the
//      old scroll, rows [102,height) at the new scroll (EXACT split -- no
...
int main() {
    ...
    // --- 1. The L+2 latch rule: hooked write at display line 100 (in-line
    //     tstate 30). With openMSX's left-margin rounding the change applies
    //     from line 102: rows [0,102) keep the pre-write scroll, rows
    //     [102,height) use the new scroll. ---
    {
        const FrameBuffer fb = run_scenario(false);
        expect(fb.width == ref0.width && fb.height == ref0.height, "Split_Dimensions");
        bool top_old = true;
        for (int y = 0; y <= 101 && top_old; ++y) {
            top_old = row_equals(fb, ref0, y);
        }
        expect(top_old, "HookedWriteAtLine100_RowsThrough101_KeepPreWriteScroll");
        bool bottom_new = true;
        for (int y = 102; y < fb.height && bottom_new; ++y) {
            bottom_new = row_equals(fb, ref64, y);
        }
        expect(bottom_new, "HookedWriteAtLine100_RowsFrom102_UseNewScroll");
    }
    ...
    std::cout << "All Machine_Hbf1xvPerLineLatch_Integration cases passed\n";
    return 0;
}
