...
// Suite: Machine_Hbf1xvHalnote_Integration  (M20-S3, closes backlog B4 AND
// B6 together)
//
...
int main() {
    ...
    //     Hbf1xvMachine instances via a shared file path (A-M20-12). ---
    {
        const std::filesystem::path sram_path =
            std::filesystem::temp_directory_path() / "sony_msx_halnote_sram_test.bin";
        std::error_code ec;
    ...
    std::cout << "All Machine_Hbf1xvHalnote_Integration cases passed\n";
    return 0;
}
