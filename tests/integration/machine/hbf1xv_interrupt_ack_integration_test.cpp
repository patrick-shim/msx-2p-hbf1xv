...
int main() {
    // --- IM2: device-supplied vector low byte selects the ISR via the table. ---
    {
        ...
        // Datasheet: IM2 ack(19) + LD A,n(7) + RETI(14) + HALT(4) = 44. The S1985
        // M1 wait applies to opcode-fetch M1s only, NOT the interrupt-acknowledge
        // M1 (which asserts /M1 + /IORQ, already carries the Z80's 2 auto ack
        // waits): M1-wait cycles = LD A,n(1) + RETI ED-4D(2) + HALT(1) = 4 (the
        // ack contributes 0) -> 44 + 4 = 48. Grounded in openMSX Z80.hh
        // CC_IRQ2 = 7+3+3+3+3 = 19 (no M1 wait folded in) + a live openMSX A/B
        // that billed a 13T IM1 accept; corrects the prior 14T/20T guess.
        if (!expect_true(cycles == 48, "Im2Ack_Sequence_DeterministicCycleTotal")) {
            return 1;
        }
    }
    ...
    // --- IM0: device drives an RST 18h opcode onto the bus. -------------------
    {
        ...
        // Oracle, MSX-accurate (datasheet + S1985 +1-per-M1 wait, fact-sheet §8).
        // Datasheet: IM0 ack RST(13) + LD B,n(7) + RET(10) + HALT(4) = 34. The
        // S1985 M1 wait applies to opcode-fetch M1s only, NOT the interrupt-ack M1
        // (the bus-driven RST is not a memory M1 fetch either): M1-wait cycles =
        // LD B,n(1) + RET(1) + HALT(1) = 3 (the ack contributes 0) -> 34 + 3 = 37.
        // Grounded in openMSX Z80.hh CC_IRQ0 = 7+3+3 = 13 (no M1 wait folded in)
        // + a live openMSX A/B (13T IM1 accept); corrects the prior 14T guess.
        if (!expect_true(cycles == 37, "Im0Ack_Sequence_DeterministicCycleTotal")) {
            return 1;
        }
    }
