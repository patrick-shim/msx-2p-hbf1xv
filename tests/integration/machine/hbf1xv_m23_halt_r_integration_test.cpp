...
// Suite: Machine_Hbf1xvM23HaltR_Integration  (M23-S1, closes backlog C2 in
// full / DEC-0004)
//
...
// the machine level) followed by an interrupt wake at the SAME IM1-ack timing
// (14T) already established in M12 -- is proven here instead, leaving the
// named oracle file untouched.
...
int main() {
    ...
    // Interrupt wake: SAME IM1-ack timing (14T = 13 bare + 1 M1 wait) as
    // already established in M12 -- proving zero coupling between the HALT-R
    // fix and interrupt-accept timing.
    machine.cpu().request_maskable_interrupt();
    const std::uint32_t t_wake = machine.step_cpu_instruction();
    if (!expect_true(t_wake == 14 && !machine.cpu().state().halted() &&
                         machine.cpu().state().regs().pc == 0x0038,
            "HaltedIdle_InterruptWake_Im1AckFourteenTstatesUnaffected")) {
        return 1;
    }
