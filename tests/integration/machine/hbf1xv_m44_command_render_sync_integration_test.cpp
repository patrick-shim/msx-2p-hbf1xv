...
// Suite: Machine_Hbf1xvM44CommandRenderSync_Integration
// (M44, DEF-M44-CMDSYNC Phase 1, DEC-0065; docs/m44-planner-package.md §4.4;
// RE-DERIVED for M62, DEC-0091-AMENDMENT-A -- justification below)
//
// The V9958 command engine's ATOMIC bitmap writes are routed through the M32
// scanline accumulator at per-destination-row granularity (the openMSX
// cmdWrite -> notify -> renderUntil analog;
// references/openmsx-21.0/src/video/VDPVRAM.hh:575-593,309-322).
//
// M62 RE-DERIVATION (spec-grounded, no assertion weakened -- the contract
// FLIPPED to the hardware-true one and the pre-M62 behavior now FAILS this
...
// references/fact-sheets/Yamaha V9958 VDP.md §6 per-scanline model; tier 2:
...
// VDPVRAM.hh:575-593, PixelRenderer.cc:549-571). The early seal was the M62
// racing-title defect (frame-start R#26/#27 sealed into ~145 rows -> the
// racing-view flash; docs/m62-investigation-report.md). The sink is now
...
// hbf1xv_m62_command_beam_clamp_integration_test.cpp.
//
...
// (which fires NEITHER the render-sync seam NOR the M44 row sink -- §2.3
// exclusion). Rows committed on the beam path BEFORE the overwrite keep the
...
//
// Cases:
//   1. BEAM CLAMP (M62): a command whose unswept destination rows lie AHEAD
...
//      PRE-M62 sink (destination-keyed early seal) FAILS this case -- the
...
int main() {
    ...
    std::cout << "All Machine_Hbf1xvM44CommandRenderSync_Integration cases passed\n";
    return 0;
}
