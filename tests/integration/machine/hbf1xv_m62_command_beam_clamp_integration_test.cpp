...
// Suite: Machine_Hbf1xvM62CommandBeamClamp_Integration (M62,
// DEC-0091-AMENDMENT-A; docs/m62-investigation-report.md)
//
// THE M62 DEFECT SCENARIO, machine-level -- the racing-title mechanism in
// miniature. The M44 command-row sink (VdpRenderSyncAdapter::on_commit_up_to)
...
// LINE:
//   - tier 1: references/fact-sheets/Yamaha V9958 VDP.md §6 (per-scanline
...
//     state", references/openmsx-21.0/src/video/VDPVRAM.hh:575-593;
//     PixelRenderer.cc:549-571) -- EFFECT re-expressed, never copied.
// The M62 fix clamps the sink's sweep to the io-write-seam beam boundary
// (raster + 2, the M39-calibrated margin on_before_state_change uses): rows
...
//       the beam-time SAT, not the blit-time SAT (FAILS pre-fix: the M51
//       pacing sealed the band from the not-yet-written SAT). Whole sealed
...
constexpr int kScrollSplitLine = kScrollBeamLine + 2;  // seam boundary (M39 +2)
...
// the M44 per-destination-row sink (the M44/M51 tests' exact recipe).
...
// M32 in-test legacy-renderer oracle).
...
// no mid-frame hooked activity (the M51 reference pattern -- frame-atomic
// recompute carries the sprite on 100..107).
...
int main() {
    ...
    std::cout << "All Machine_Hbf1xvM62CommandBeamClamp_Integration cases passed\n";
    return 0;
}
