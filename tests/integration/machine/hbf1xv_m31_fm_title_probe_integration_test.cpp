...
// 2. A-M31-2 (the FM-activity probe on roms/aleste.rom): a scripted 900-
...
//    edges. HONEST PROBE, NON-GATING on the outcome: whether Aleste 2
//    exercises FM is an UNVERIFIED belief about third-party software; the
...
// different aleste.rom dump (A-M30-1 precedent).
//
...
int main() {
    ...
    // --- 2. A-M31-2: the FM-activity probe on Aleste 2 (skip-gated). ---
    ...
    const std::vector<std::uint8_t> image((std::istreambuf_iterator<char>(rom_in)),
                                          std::istreambuf_iterator<char>());
    if (sony_msx::machine::sha1_hex(image) != kAleste2Sha1) {
        std::cout << "SKIP(probe): aleste.rom is a different dump -- A-M31-1 result above stands\n";
        return g_failures == 0 ? 0 : 1;
    }

    const ProbeResult first = run_probe(image);
    expect(first.frames == 900, "Probe_Ran900Frames");
    std::cout << "ALESTE FM probe: reg_change_observations=" << first.reg_change_observations
              << " key_on_edges=" << first.key_on_edges << " frames=" << first.frames << "\n";
    if (first.key_on_edges > 0) {
        std::cout << "PROBE OUTCOME: Aleste 2 DID exercise the YM2413 (A-M31-2 confirmed; RC "
