...
// Suite: Machine_Hbf1xvM23VdpAccessTimingNonGating_Integration  (M23-S2,
...
// byte sequences (tests/system/hbf1xv_m21_vdp_render_system_test.cpp:53-64/
// 81-89/106-114/137-149 and
// tests/system/hbf1xv_m22_sprites_command_engine_system_test.cpp:49-82/
...
int main() {
    // --- M21 fixture 1/4: GRAPHIC1 (verbatim from hbf1xv_m21_vdp_render_ ------
    // --- system_test.cpp:53-64). -----------------------------------------------
    {
        ...
        expect(machine.cpu().state().halted(), "M21Graphic1_ReachesHalt");
        ...
        expect(total == 185, "M21Graphic1_CumulativeTstates_ByteIdenticalToPreM23");
    }
    ...
    // --- M21 fixture 2/4: GRAPHIC4 (verbatim from :81-89). --------------------
    {
        ...
        machine.load_memory(0x0000, program.data(), static_cast<std::uint32_t>(program.size()));
        const std::uint64_t total = run_to_halt_summing(machine, 64);
        expect(machine.cpu().state().halted(), "M21Graphic4_ReachesHalt");
        expect(total == 105, "M21Graphic4_CumulativeTstates_ByteIdenticalToPreM23");
    }
    ...
    // --- M21 fixture 3/4: GRAPHIC6 (verbatim from :106-114). ------------------
    {
        ...
        machine.load_memory(0x0000, program.data(), static_cast<std::uint32_t>(program.size()));
        const std::uint64_t total = run_to_halt_summing(machine, 64);
        expect(machine.cpu().state().halted(), "M21Graphic6_ReachesHalt");
        expect(total == 125, "M21Graphic6_CumulativeTstates_ByteIdenticalToPreM23");
    }
    ...
    // --- M21 fixture 4/4: YJK/SCREEN12 (verbatim from :137-149). --------------
    {
        ...
        machine.load_memory(0x0000, program.data(), static_cast<std::uint32_t>(program.size()));
        const std::uint64_t total = run_to_halt_summing(machine, 64);
        expect(machine.cpu().state().halted(), "M21Yjk_ReachesHalt");
        expect(total == 205, "M21Yjk_CumulativeTstates_ByteIdenticalToPreM23");
    }
    ...
    // --- M22 fixture 1/2: sprite setup + collision (verbatim from :49-82). ----
    {
        ...
        expect(machine.cpu().state().halted(), "M22Sprites_ReachesHalt");
        expect(total == 445, "M22Sprites_CumulativeTstates_ByteIdenticalToPreM23");
    }
    ...
    // --- M22 fixture 2/2: command engine HMMV+LMMC (verbatim from :134-172). --
    {
        ...
        machine.load_memory(0x0000, program.data(), static_cast<std::uint32_t>(program.size()));
        const std::uint64_t total = run_to_halt_summing(machine, 200);
        expect(machine.cpu().state().halted(), "M22CommandEngine_ReachesHalt");
        expect(total == 525, "M22CommandEngine_CumulativeTstates_ByteIdenticalToPreM23");
    }
