...
//
// Machine-level L+2 latch for the R#1 BL (display enable) bit (RE-DERIVED
// M39/DEC-0060: openMSX's LINE renderer rounds the render-sync point with the
// active-display left margin, PixelRenderer.cc:566-567, so a mid-frame write
...
// BL-band raster split lands exactly on openMSX's scanline). Driven through the
// REAL seam (V9958Vdp::io_write on #99 -- the exact entry point a CPU OUT
// reaches over the IoBus; the M32 hbf1xv_m32_per_line_latch precedent):
