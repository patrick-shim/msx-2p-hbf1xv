...
// Suite: Machine_Hbf1xvM19MetalgearSmoke_Integration (M19 follow-up, backlog B7)
//
// *** MECHANICAL/PLAUSIBILITY SMOKE CHECK -- NOT A BYTE-EXACT PROTOCOL CLAIM ***
//
// Unlike roms/aleste.rom (A-M19-10, genuinely unconfirmable identity),
// roms/metalgear.rom's real-world identity as a `Konami`-mapper "Metal Gear"
// (Konami, 1987, MSX2) dump IS directly confirmed by a concrete, citable
// source already in this repo: its SHA1
...
// entries under `<title>Metal Gear</title>` (`<genmsxid>1028</genmsxid>`,
// Konami, 1987, MSX2, JP) in
...
// 19.1 via `-carta roms/metalgear.rom -romtype Konami` succeeds (no
...
// This is a genuinely STRONGER real-world validation than aleste.rom's
...
// hbf1xv_m19_aleste_smoke_integration_test.cpp's own copy -- each test
...
int main() {
    ...
    const std::filesystem::path path = std::filesystem::path(SONY_MSX_ROMS_DIR) / "metalgear.rom";
    std::ifstream in(path, std::ios::binary);
    if (!in) {
        std::cerr << "Case failed: MetalgearRom_FileOpens (path: " << path << ")\n";
        return 1;
    }
    ...
    expect(image.size() == 139'264, "MetalgearRom_FileSize_Is139264Bytes");

    // --- SHA1 matches the softwaredb.xml-cited "Metal Gear" (Konami, 1987,
    //     MSX2, genmsxid 1028) `Konami`-typed dump entry EXACTLY. ---
    ...
    expect(sha1_digest == expected_sha1,
           "MetalgearRom_Sha1_MatchesSoftwareDbKonamiTypedDump");
    ...
    expect(machine.debug_bus_read(0x4000) == 0x41, "MetalgearRom_Page1_FirstByte_Is_A_0x41");
    expect(machine.debug_bus_read(0x4001) == 0x42, "MetalgearRom_Page1_SecondByte_Is_B_0x42");
