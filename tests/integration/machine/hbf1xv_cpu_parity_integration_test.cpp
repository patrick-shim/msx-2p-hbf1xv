...
//      order, and the ACK cycle == datasheet 13 with NO S1985 M1 wait (the ack
//      special M1 is /M1 + /IORQ, not the opcode-fetch /M1 + /MREQ the S1985
//      gates its wait on; openMSX CC_IRQ1 = 7+3+3 = 13, live A/B: 13T accept).
...
int main() {
    ...
    // --- 4 & 5. IM1 accept oracle, IFF clear, RETI restore, EI delay -------
    {
        ...
        const bool ack_ok = ack_cycles == 13 &&  // datasheet 13, NO S1985 M1 wait on the ack M1
