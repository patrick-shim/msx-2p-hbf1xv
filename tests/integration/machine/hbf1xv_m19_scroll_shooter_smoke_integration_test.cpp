...
// Suite: Machine_Hbf1xvM19ScrollShooterSmoke_Integration (M19-S4, backlog B7)
//
...
// roms/ holds ONLY the FM-PAC peripheral. This test reads a scrolling-shooter
// title's ROM at games/roms/scroll-shooter.rom, which is BYTE-IDENTICAL to the
// pre-reorg roms/ copy of the same dump (verified: same 2,097,152 bytes, same
// SHA256 4ea1e18...); every assertion below therefore
// holds unchanged. games/roms/ is untracked, so a machine without the asset
// SKIPS (returns 0) rather than FAILS -- the same skip-when-absent discipline
// the FM-PAC autoload / sdl3_cli_session Case-3 tests use.
//
// The scroll-shooter ROM's real-world mapper/game identity is NOT confirmable from
...
// milestone) that this proves the real game runs/plays correctly, nor
...
int main() {
    ...
    if (!in) {
        std::cout << "SKIP: games/roms/scroll-shooter.rom not present (path: " << path
    ...
    expect(image.size() == 2'097'152, "ScrollShooterRom_FileSize_Is2097152Bytes");

    // --- (b) SHA256 matches docs/asset-checksums.txt's recorded value. ---
    const std::string digest = sha256_hex(image);
    // This exact SHA256 was recorded for the pre-reorg roms/ copy of this dump
    // and holds byte-for-byte for the post-reorg games/roms/ copy; re-verified
    // independently here (an own from-spec SHA-256 implementation above), not
    // merely re-asserted.
    const std::string expected_digest =
        "4ea1e183467abe094df88901fc5dc70f0df5c1fc424de19f212692001bd5ad43";
    expect(digest == expected_digest, "ScrollShooterRom_Sha256_MatchesRecordedChecksum");

    Hbf1xvMachine machine;
    machine.cold_boot();
    const CartridgeLoadResult load_result = machine.load_cartridge(1, CartridgeMapperType::Generic8kB, image);
    expect(load_result == CartridgeLoadResult::Ok, "ScrollShooterRom_LoadsAsGeneric8kB_MechanicalOnly_NoError");
    ...
    expect(machine.debug_bus_read(0x4000) == 0x41, "ScrollShooterRom_Page1_FirstByte_Is_A_0x41");
    expect(machine.debug_bus_read(0x4001) == 0x42, "ScrollShooterRom_Page1_SecondByte_Is_B_0x42");
