...
// Suite: Machine_Hbf1xvBootCheckpointFdc_Integration
//
...
//      present, matching the earlier boot-checkpoint tests' pattern);
...
//   3. boot ADVANCES strictly past the devices-only boot checkpoint (max PC
//      0x488, that run's self-derived maximum) -- with the FDC present
//      (replacing the bare DISK-ROM attach), the CPU trajectory diverges from
//      the devices-only run almost immediately (the FDC decode intercepts what
//      used to be raw ROM reads at 0x7FF8-0x7FFF) and continues advancing far
//      beyond it.
//
// HONEST RESIDUAL (genuinely investigated, not fabricated):
//   Over this budget (and confirmed separately up to a 20,000,000-instruction
//   diagnostic run), the real BIOS/DISK-ROM auto-boot handshake
//   (DSKCHG -> drive/motor/side -> status/INTRQ poll ->
...
//   3000-instruction real-boot window -- so this is not a slot/decode defect
//   in THIS emulator's boot path;
//   the real MSX auto-disk-boot trigger condition requires something this
//   headless, no-keyboard-input run does not provide. The FDC DEVICE itself
//   (register decode, command semantics, DRQ streaming) is independently,
//   positively verified via hbf1xv_fdc_integration_test.cpp (a CPU program
//   that explicitly selects slot (3,2) and drives Restore + Read Sector
//   end-to-end) and by A/B-checking the same operations against genuine
//   openMSX with the IDENTICAL disk image, functionally
...
// Generous budget: comfortably exceeds the devices-only checkpoint window and
// (per the exploratory investigation) reaches the
...
// The devices-only boot checkpoint's self-derived maximum (that run reported
// "final PC=0x454 max PC=0x488 over 4096 instructions").
...
int main() {
    ...
    // Primary signal: PC advances strictly past the devices-only boot
    // checkpoint. Genuinely true and substantial here (max PC reaches well
    ...
    // Honest residual (see the file-header comment):
