...
// Suite: Machine_Hbf1xvCartridge_Integration
//
...
// with NO SlotBus::set_expanded call for either (confirmed via
// direct read of openMSX's Sony_HB-F1XV.xml: both are bare, unexpanded
...
// hbf1xv_scroll_shooter_smoke_integration_test.cpp).
//
// Cases:
//   1. A real CPU program running over the full machine bus (SlotBus/IoBus)
...
//      pre-cartridge open-bus default (regression guard) with the cartridge
//      code present but unused.
...
//      (regression guard).
//
// (Zero-regression across the full pre-existing suite, in particular the
// CPU-timing oracles, is confirmed separately by the full `ctest` run -- no
// new clock consumer is introduced by any cartridge device.)
//
...
// SlotBus bit layout (src/devices/chipset/slot_bus.cpp,
// `primary_for_page(page) = (primary_select_ >> (2*page)) & 0x03`, i.e. 2
// bits per page, page N at bits [2N, 2N+1]). A naive "#A8 = 0x04" (page 1 to
// slot 1, all other pages to slot 0) would leave pages
// 0/2/3 at primary slot 0 (BIOS ROM) rather than primary slot 3 (RAM), and so
// could not hold a running CPU program/stack. To route ONLY page 1 to primary
...
// Tiny self-contained Z80 assembler helper (mirrors the FDC integration
...
void run_all_mapper_types_probe(const int slot_number, const std::uint8_t a8_value,
    ...
    //     slot2 never moves (corrected finding per the unit test). ---
    {
        Hbf1xvMachine machine;
        machine.cold_boot();
        machine.map_flat_ram();
...
int main() {
    ...
    // --- 3. Unloaded/never-loaded slot: byte-for-byte the pre-cartridge
    //        open-bus default, with the cartridge code present but unused. ---
    {
        Hbf1xvMachine machine;
        machine.cold_boot();
    ...
    // --- 4. cold_boot() reinitializes bank state but NEVER unloads. ---
    {
        Hbf1xvMachine machine;
        machine.cold_boot();
    ...
    // --- 5. Regression guard: neither external primary slot is
