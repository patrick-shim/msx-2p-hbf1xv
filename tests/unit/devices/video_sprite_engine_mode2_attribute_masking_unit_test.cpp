...
// SCREEN5 R#5=0xEF, the stealth-action title R#5=0xE7): the resulting 1KB region holds the
...
//
// Case 1 reproduces the exact stealth-action-title register shape (R#5=0xE7, R#11=0x01,
...
// the stealth-action title's sprite tables live at 0xF000-0xF27F).
...
int main() {
    // --- Case 1: the exact stealth-action-title register shape. R#5=0xE7/R#11=0x01 ->
    ...
    //     (zeros) and the sprite never appeared anywhere in the frame. ---
    {
        ...
        set_register(vdp, 1, 0x42);  // display enable + 16x16 sprites (as the stealth-action title: R#1=0x62)
        set_register(vdp, 5, 0xE7);
        set_register(vdp, 11, 0x01);
        set_register(vdp, 6, 0x1F);  // pattern_base = 0xF800
        set_register(vdp, 9, 0x80);  // 212-line display (the title's R#9 LN=1)
        ...
        expect(visible.size() == 1, "StealthActionShape_SpriteVisibleOnLine177");
        if (visible.size() == 1) {
            expect(visible[0].x == 0x2A, "StealthActionShape_SpriteXFromMaskedTable");
            expect((visible[0].color_attrib & 0x0F) == 14, "StealthActionShape_ColorFromMaskedTable");
            expect(visible[0].pattern == 0xFFFF0000u, "StealthActionShape_PatternRowSolid16");
        }
        ...
        expect(fb.at(0x2A, 177) == color14, "StealthActionShape_FrameBufferShowsSpritePixel");
        expect(fb.at(0x2A + 15, 177) == color14, "StealthActionShape_FrameBufferShows16thColumn");
    }
