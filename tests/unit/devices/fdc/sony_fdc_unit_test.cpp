...
//     building-entry
    ...
    //     back high through the S1985 pull-ups). So a 0x7FFD read with
    //     drive-select bits 11 (NONE) or 01 (absent B) reads "not changed"
    //     (bit2 = 1) and MUST NOT consume drive A's changed one-shot; only a
    ...
    //     strategy-title scrambled campaign map. ---
    {
        ...
        // Drive-select NONE (bits 11): line pulled up = "not changed"; the
        // latch survives untouched.
        f.sony_fdc.mem_write(0x7FFD, 0x03);
        expect((f.sony_fdc.mem_read(0x7FFD) & 0x04) != 0,
               "Dskchg_DriveSelectNone_ReadsNotChanged");
        expect(f.drive.disk_changed(), "Dskchg_DriveSelectNone_LatchPreserved");

        // Drive-select B (bits 01): the second drive is absent on the HB-F1XV
        // (single built-in drive) -> same pulled-up, non-consuming read.
        f.sony_fdc.mem_write(0x7FFD, 0x01);
        expect((f.sony_fdc.mem_read(0x7FFD) & 0x04) != 0,
               "Dskchg_DriveSelectB_ReadsNotChanged");
        expect(f.drive.disk_changed(), "Dskchg_DriveSelectB_LatchPreserved");

        // Drive-select A (bits 00): NOW the read reports "changed" (bit2 = 0)
        // and consumes the one-shot -- the disk ROM's post-select probe sees
        // the swap exactly as on real hardware.
        f.sony_fdc.mem_write(0x7FFD, 0x00);
        expect((f.sony_fdc.mem_read(0x7FFD) & 0x04) == 0,
               "Dskchg_DriveSelectA_ReportsChanged");
        expect(!f.drive.disk_changed(), "Dskchg_DriveSelectA_ConsumedOneShot");
        expect((f.sony_fdc.mem_read(0x7FFD) & 0x04) != 0,
    ...
    //     reports + consumes (PhilipsFDC.cc:158-169: 00 and 10 both select A). ---
    {
        Fixture f;
        f.drive.set_disk_changed(true);
        f.sony_fdc.mem_write(0x7FFD, 0x02);  // bits 10 -> drive A
        expect((f.sony_fdc.mem_read(0x7FFD) & 0x04) == 0,
               "Dskchg_DriveSelectA_AltEncoding_ReportsChanged");
