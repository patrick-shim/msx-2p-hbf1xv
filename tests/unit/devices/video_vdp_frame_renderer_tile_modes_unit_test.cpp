...
int main() {
    // --- GRAPHIC2 (base 0x04): the V9958 AND-MASK table addressing
    //     (DEF-M43-FMPAC-SCREEN / DEC-0063). Uses the CANONICAL SCREEN 2
    //     registers R#4=0x03, R#3=0xFF (the registers the FM-PAC manager and
    //     real SCREEN-2 software program) -- registers where the ADDITIVE
    //     base+offset model diverges from the AND-mask model, so this test is
    //     NON-TAUTOLOGICAL: under the old additive model R#4=0x03 would place
    //     the pattern base at 0x1800 (the NAME table) and R#3=0xFF the colour
    //     base at 0x3FC0 (past the colour table), rendering the wrong bytes and
    //     failing the pixel asserts below. Mutating pattern_table_mask()/
    //     color_table_mask() away from the AND-mask likewise fails it (the
    //     adversarial-mutation non-tautology check, DEC-0063). Quarter-addressed
    //     pattern/color tables (CharacterConverter.cc:299-315), one 2048-byte
    //     quarter per 64 raster lines. ---
    {
        V9958Vdp vdp;
        ...
        set_register(vdp, 2, 0x06);  // name table @ 0x1800 (CANONICAL SCREEN 2)
        set_register(vdp, 4, 0x03);  // pattern generator @ 0x0000 (AND-mask: 0x03<<11|0x7FF=0x1FFF)
        set_register(vdp, 10, 0x00);
        set_register(vdp, 3, 0xFF);  // colour table @ 0x2000 (AND-mask: 0xFF<<6|0x3F=0x3FFF)
        set_palette(vdp, 5, 5, 0, 0);
        set_palette(vdp, 9, 0, 0, 9 & 0x7);

        // Same char code (5) at name row-group 0 (line 0-7) and name row-group
        // 8 (line 64-71); DIFFERENT pattern bytes per quarter prove the
        // 2048-byte quarter offset is genuinely applied. Tables at their
        // canonical AND-mask addresses (pattern 0x0000, colour 0x2000, name
        // 0x1800), all within the 14-bit CPU-port range (< 0x4000).
        write_vram(vdp, 0x1800, 0x05);                    // line0..7, col0: char 5
        write_vram(vdp, 0x1800 + 8 * 32, 0x05);           // line64..71, col0: char 5
        write_vram(vdp, 0x0000 + 5 * 8 + 0, 0xF0);        // quarter0 pattern
        write_vram(vdp, 0x0000 + 2048 + 5 * 8 + 0, 0x0F);  // quarter1 pattern (DIFFERENT)
        write_vram(vdp, 0x2000 + 5 * 8 + 0, 0x59);        // quarter0 color: fg=5,bg=9
        write_vram(vdp, 0x2000 + 2048 + 5 * 8 + 0, 0x59);  // quarter1 color: same fg/bg
        ...
        expect(row0[0] == fg && row0[3] == fg && row0[4] == bg && row0[7] == bg,
               "Graphic2_Canonical_Quarter0_PatternMatches");
        ...
        expect(row64[0] == bg && row64[3] == bg && row64[4] == fg && row64[7] == fg,
               "Graphic2_Canonical_Quarter1_UsesDifferentPatternByte");
    }

    // --- GRAPHIC3 (base 0x08) reuses the SAME renderer + AND-mask addressing as
    //     GRAPHIC2 (openMSX VDP.cc case 0x08 pattern/colour ~0u<<13). Same
    //     canonical registers (R#4=0x03/R#3=0xFF); render a real pixel scene to
    //     prove the mode-decode path, not just width(). ---
    {
        V9958Vdp vdp;
        set_register(vdp, 1, 0x40);  // M34: R#1 bit6 BL=1 (display enable)
        set_register(vdp, 0, 0x04);  // M4 -> GRAPHIC3 (base 0x08)
        set_register(vdp, 2, 0x06);  // name @ 0x1800
        set_register(vdp, 4, 0x03);  // pattern @ 0x0000 (AND-mask)
        set_register(vdp, 10, 0x00);
        set_register(vdp, 3, 0xFF);  // colour @ 0x2000 (AND-mask)
        set_palette(vdp, 6, 3, 3, 0);
        set_palette(vdp, 2, 0, 0, 2);

        write_vram(vdp, 0x1800, 0x07);              // line0 col0: char 7
        write_vram(vdp, 0x0000 + 7 * 8 + 0, 0xCC);  // pattern row0: 1100_1100
        write_vram(vdp, 0x2000 + 7 * 8 + 0, 0x62);  // colour: fg=6, bg=2
        ...
        std::uint16_t row0[256];
        renderer.render_line(0, Field::Progressive, row0);
        const std::uint16_t fg = pack_rgb555(expand3to5(3), expand3to5(3), expand3to5(0));
        const std::uint16_t bg = pack_rgb555(expand3to5(0), expand3to5(0), expand3to5(2));
        // Pattern 0xCC = 1100_1100 -> fg fg bg bg fg fg bg bg.
        expect(row0[0] == fg && row0[1] == fg && row0[2] == bg && row0[3] == bg &&
                   row0[4] == fg && row0[5] == fg && row0[6] == bg && row0[7] == bg,
               "Graphic3_Canonical_AndMask_PatternColorMatches");
    }
