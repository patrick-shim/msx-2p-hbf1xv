...
class Hbf1xvMachine {
public:
    // The main-RAM (DRAM) size is configurable. The DEFAULT is
    ...
    // MemoryMapperRam asserts the power-of-two invariant defensively. (DEC-0061)
    ...
    // records any missing-asset diagnostics. An absolute path keeps asset
    ...
    // 7 files as the built-in default filenames (byte-identical). Only the filename
    ...
    // cold_boot() so the (re)load picks them up. (DEC-0077)
    ...
    // cold_boot, since cold_boot resets #A8 to the authentic 0 (BIOS). Sets
    ...
    void run_frame();
    // Frame-boundary bookkeeping for a REAL-TIME driver (the SDL3
    ...
    // the same frame boundary would double-count elapsed cycles --
    ...
    // Non-perturbing debug seams over the full decode fabric. Unlike
    ...
    // VDP access-timing foundation accessors (additive-only,
    // NON-GATING -- nothing in step_cpu_instruction()/
    ...
    // call, or cycle 0 if run_frame() was never called (a tested boundary
    // condition; many system tests drive the CPU purely via
    ...
    // Deterministic CPU trace-export facility. Off by default. When
    ...
    // Minimum INERT memory region: a pure storage byte buffer sized to
    ...
    //
    // NO internal SRAM: the earlier speculative 8 KB `sram_`
    // region was removed. The HB-F1XV's built-in FM is MSX-MUSIC (OPLL +
    // APRLOPLL BIOS, NO SRAM) -- grounded in
    // openMSX 21.0: share/machines/Sony_HB-F1XV.xml (`<MSX-MUSIC>`
    ...
    // present, empty otherwise. (DEC-0050)
    //
    // The 128 KB VRAM lives in the V9958 VDP device (not an
    ...
    // size live in devices::video::VdpVram (VdpVram::kVramBytes); access
    // via vdp().vram().
    //
    // kDramBytes is the STOCK-spec DEFAULT DRAM size (the strict
    ...
    // RAM-search all follow the fitted size automatically. (DEC-0061)
    ...
    // The V9958 VDP device. Owns the 128 KB VRAM and answers ports
    ...
    // Decoded frame accessor. Field::Progressive (every
    // production call site) routes to
    ...
    // (a hard test oracle). Even/Odd (test/debug-only interlace fields; no
    // production caller) keep the legacy frozen-snapshot semantics via
    // VdpFrameRenderer::render_frame(). (DEC-0039)
    //
    // Logical constness: the accumulator member
    ...
    // Device integration accessors (PSG/RTC/PPI/keyboard/joystick). Tests
    // drive human input and inspect device state through these; the CPU reaches
    // them over the IoBus at #A0-A2 / #B4/B5 / #A8-AB / #F5.
    [[nodiscard]] const devices::audio::PsgYm2149& psg() const;
    devices::audio::PsgYm2149& psg();
    // MSX 1-bit key-click DAC on PPI port-C bit 7 (also used by software for
    // digitized-voice playback).
    // An ADDITIVE, band-limited mixer source fed cycle-stamped bit-7 edges from
    // the PPI seam. Capture is DISABLED by default (byte-identical
    ...
    // YM2413 (OPLL) register-accurate device, answering the real
    // MSX-MUSIC I/O ports #7C/#7D alongside the unmodified fmmusic_rom_
    // at slot 3-3 page 1 (no memory-space register overlay,
    ...
    // Peripheral I/O device integration accessors (Kanji font ROM,
    // printer port, cassette interface). The CPU reaches the
    // Kanji device over #D8-#DB and the printer over #90-#97; the cassette
    // interface has no dedicated CPU-visible port -- it is reached
    ...
    // Sony MB670836 hardware PAUSE + Speed Controller, and Ren-Sha Turbo
    // autofire. pause_controller() is the machine-level
    // CPU-execution gate consulted at the top of step_cpu_instruction() and
    // driven per-VBlank by
    // run_frame(); rensha_turbo() is the autofire signal generator wired
    // into keyboard_()/joystick_() via the OR-mask attach points.
    ...
    // FDC subsystem. The WD2793 core answers the Sony register window
    ...
    // External cartridge slots. Primary slots 1 and 2 are
    // bare, unexpanded XML `<primary external="true">` bays; each is
    ...
    // the empty-bay open-bus default (regression guard). load_cartridge/
    ...
    // Konami SCC wavetable chip accessor.
    // Returns the given bay's owned SccWavetable
    ...
    // psg()'s accessor shape with the default-nullptr precedent: with no
    // SCC cart loaded the frontend/mixer sees nullptr
    // (regression null). Both bays are queryable independently (real
    // hardware mixes both slots' sound-in lines). No new clock consumer and
    // no wire_bus() change: reached over the existing slot attachment;
    // its generator advances only via the frontend audio pump
    // (SccWavetable::advance_cycles).
    ...
    // FM-PAC peripheral cartridge accessor. Returns the given
    ...
    // .sram-persistence plumbing. (DEC-0050)
    ...
    // FM-PAC 8 KB battery-SRAM .sram persistence. Mirrors the
    ...
    // S1985 16-byte backup-RAM .sram persistence. Set the
    // file path BEFORE cold_boot to load it (absent -> deterministic zero state,
    // preserving the golden-locked state dump). flush_backup_ram writes the
    // current 16 bytes.
    ...
    // backup_ram_path/flush_backup_ram EXACTLY (no CLI flag): set the
    ...
    // Full-state debug dump + execution-event logging.
    //
    ...
    // Flush the collected per-instruction CPU trace to <root>/traces/<filename>.
    ...
    // Decoded-FrameBuffer -> dump capture.
    // Mirrors write_state_dump()/write_cpu_trace()/write_event_log()
    ...
    // on-demand VRAM/register snapshot.
    ...
    // --- Comprehensive debug SNAPSHOT. A SEPARATE artifact from the
    //     golden-locked serialize_state_dump(): captures the
    //     exact current state of EVERY machine component into
    //     a per-component typed dump, versioned "HBF1XV-SNAPSHOT v1" and
    //     designed restore-ready (restore itself is future work).
    ...
    // The #FC-#FF memory-mapper segment registers. const
    ...
    // boundary yields the identical id and byte-identical content.
    ...
    // frontend's multi-disk index -- a FRONTEND concept
    ...
    // --- Live STREAM-CAPTURE (crash-trajectory diagnostic; additive,
    //     default-off). Extends the component snapshot + the
    //     CPU-trace facility to record the trajectory INTO a control-flow crash
    //     (built to diagnose a multi-disk-RPG building-entry HALT). Armed at a chosen moment
    ...
    //     on_vsync_boundary) -- src/devices/cpu/ + src/core/ are
    //     untouched. (DEC-0052) ---
    //
    //     The finalize trigger ALSO fires on a
    ...
    //     (PC=0x0038, HALT=0), which a HALT-only trigger never caught. When
    ...
    //     finalize_stream_capture). (DEC-0052) ---
    static constexpr std::size_t kStreamTraceRingCapacity = 1u << 20;  // 1,048,576 records

    // Stack-runaway finalize floor. SP < this is an unambiguous
    ...
    // frames of the derail (frame ~2683), so it stays comfortably
    // retained. (DEC-0052)
    static constexpr std::uint16_t kStreamStackFloor = 0x4000;

    // --- "Stream-light" mode (for long-session upstream crash hunts). A
    ...
    //     The heavy per-frame mode remains the default when light is off;
    ...
    // above (default false = the heavy per-frame-snapshot mode, byte-for-byte
    // unchanged from before light mode existed). Finalizing dumps the ring + writes the final
    ...
    // --- PHYSICAL-DRAM memory-write WATCHPOINT (a save-corruption
    ...
    //     default behavior. Cap kMemWatchMaxRows bounds memory (records past
    //     the cap are dropped and a note is emitted). (DEC-0072) ---
...
private:
    ...
    // Like write_text_file but APPENDS (never truncates) -- the stream-capture
    // FDC sector-read log accumulates one line per read while streaming. Determinism
    ...
    // --- Stream-capture internals (all genuine no-ops unless armed). (DEC-0052) ---
    ...
    // Append deterministic per-byte + per-sector Write Sector
    ...
    // Stream-light WATCHLOG appenders (called via the mapper-RAM /
    ...
    // Physical-DRAM watchpoint recorder (self-guarded on
    ...
    // consumer (stream-light watchlog OR the physical-DRAM watchpoint) wants it.
    void refresh_ram_write_observer();
    // Build a PRE-execution trace record from the live CPU state, REUSING the
    ...
    // Cold-boot RAM power-on content: the XML `initialContent` alternating
    // 00/FF pattern (Sony_HB-F1XV.xml:129) decoded + repeat-filled across 64 KB,
    // matching openMSX Ram::clear (openMSX 21.0: src/memory/Ram.cc:37-78).
    ...
    // (openMSX 21.0: src/video/PixelRenderer.cc:549-571), erring by
    // at most one line vs the pixel-accurate model (a disclosed precision
    // simplification). `suspended` gates the machine's non-perturbing
    // debug_io_write() seam OUT of the hook (scenes built through
    ...
    private:
        ...
        // The last beam-derived commit target (raster line + 2) fed to the
        ...
        // the accumulator's wrap-safety finalize) from the case where the
    ...
    // Line-interrupt delivery. Consulted once per step_cpu_instruction, O(1):
    ...
    // semantic, VDP.cc:518-524) or after a crossing fired. (DEC-0039)
    void poll_line_interrupt();
    void recompute_line_interrupt_cache(std::uint8_t r19, std::uint8_t r23, std::uint8_t r9,
                                        std::uint64_t now);

    // Level-held /INT adapter: forwards the V9958's owned interrupt
    // line to the Z80A maskable-interrupt request/clear, REUSING the IM1
    ...
    // Key-click 1-bit-DAC seam adapters. PpiClickAdapter forwards each
    ...
    // Cycle-stamp source for the PSG sync-before-change seam
    ...
    // Stream-capture FDC observer adapter. Installed on fdc_ only
    ...
    // Non-perturbing (see FdcSectorReadObserver's contract). (DEC-0052)
    ...
    // Stream-capture Write Sector observer adapter, mirroring
    ...
    // Stream-light mapper-RAM write observer. Installed on ram_mapper_
    ...
    // Stream-light VDP control-register-write observer. Installed on
    ...
    // a watchlog line for R#1 only. Non-perturbing. (DEC-0052)
    ...
    // Deterministic emulated-cycle clock source for the RTC. Returns the
    ...
    // Deterministic emulated-cycle clock source for the FDC (X-pattern of
    ...
    // CPU T-state accounting (protecting the CPU-timing oracles).
    ...
    // (X-pattern of RtcClock/FdcClock). Returns scheduler total
    // cycles READ-ONLY; the synthetic-tape input model advances off this,
    // never the host wall clock or CPU T-state accounting.
    ...
    // Deterministic emulated-cycle clock source for Ren-Sha Turbo
    // (X-pattern of RtcClock/FdcClock/CassetteClock). Returns
    ...
    // the zero-tolerance CPU-timing oracles). Consulted
    ...
    // Deterministic emulated-cycle clock source for the YM2413
    // write-timing gate (X-pattern of RtcClock/FdcClock/
    // CassetteClock/RenshaTurboClock). Returns scheduler total
    ...
    // zero-tolerance CPU-timing oracles). Consulted pull-style
    ...
    // via set_write_timing_enforced(true).
    ...
    // status bits (a boot-hang fix: the real BIOS hangs forever polling
    ...
    public:
        ...
        // The ABSOLUTE monotonic cycle
        // count for the command-engine CE busy window (never reset at vsync).
        // Reads an EXISTING public core::Scheduler accessor -- no src/core
        // edit. (DEC-0069)
    ...
    // S1985 "MSX-ENGINE" chipset + full system bus. The machine owns the
    ...
    // Human-input peripherals. Declared before the PPI so the PPI can bind
    // the keyboard matrix by reference at construction.
    peripherals::KeyboardMatrix keyboard_;
    peripherals::JoystickPorts joystick_;
    // Centronics printer port peripheral,
    // answering #90-#97. Purely combinational
    // -- no clock dependency.
    peripherals::PrinterPort printer_;
    devices::chipset::Ppi8255 ppi_{slot_bus_, keyboard_};  // #A8-#AB (+#AC-AF mirror)
    // Cassette interface: motor/output derived
    // READ-ONLY from ppi_ (zero edit to Ppi8255); cassette_clock_
    // feeds its deterministic synthetic-tape input model, pull-style
    // only. Declared after ppi_ (binds a const reference to
    ...
    // Ren-Sha Turbo autofire: the signal
    // generator + its X-pattern clock adapter, attached into keyboard_/
    // joystick_ via wire_bus()'s additive OR-mask seams. Owned
    ...
    // PSG (YM2149) on #A0-A2 and RTC (RP5C01) on #B4/B5 — seam ports
    // answered by real devices. The #F5 system-control register gates the
    // RTC CLOCK-IC. rtc_clock_ feeds the RTC deterministic emulated time.
    devices::audio::PsgYm2149 psg_;
    // MSX 1-bit key-click DAC (additive band-limited mixer source). Fed
    ...
    PsgCycleClock psg_cycle_clock_{scheduler_};  // PSG sync-before-change stamps
    // YM2413 (OPLL) register-accurate device on #7C/#7D --
    // an IoDevice attached alongside the unmodified fmmusic_rom_ (below).
    // ym2413_clock_ feeds the write-timing gate READ-ONLY; the
    ...
    std::filesystem::path halnote_sram_path_;  // mirrors backup_ram_path_ exactly
    std::filesystem::path fmpac_sram_path_;    // mirrors halnote_sram_path_ exactly

    // CPU-addressable memory devices. The mapper RAM consumes mapper_io_'s
    // live segments; the ROM devices are read-only windows over the loaded images.
    // Window base/size come from the Sony_HB-F1XV.xml `<mem>` placements.
    ...
    // FM-MUSIC ROM presence, slot 3-3 page 1.
    // The real MSXMusic device has NO memory-space register
    ...
    // Halnote-mapped MSX-JE firmware ROM, slot 0-3, ALL 4 pages.
    // Composes the CartridgeRomWindow (main 8-slot
    // window, default block mask, NO Konami-style override) + the
    // BatteryBackedSram (real 16 KB SRAM store). Pure
    // combinational device -- no clock adapter needed.
    devices::halnote::HalnoteRom halnote_;

    // Kanji font ROM I/O device, answering #D8-#DB
    // directly (the MSXKanji device type, NOT the switched-I/O MSXKanji12). Reads
    // the real 256 KB bios/f1xvkfn.rom (JIS1+JIS2). Pure combinational device
    // -- no clock dependency, not slot-attached (I/O-only).
    devices::kanji::KanjiFontRom kanji_font_rom_;

    // FDC: WD2793 core + built-in 720 KB drive + deterministic image, and
    // the SonyFdc decode that wraps disk_rom_ and answers 0x7FF8-0x7FFF. Attached
    // at slot 3-2 page 1 in place of the bare disk_rom_.
    ...
    // External cartridge slots: primary slots 1 and 2,
    // each a bare, unexpanded external bay. Empty by default
    // (nullptr mapper inside CartridgeSlot) -- byte-for-byte the empty-machine
    // open-bus behavior until load_cartridge() is called.
    ...
    // Role-keyed BIOS ROM filenames (default = the strict
    // HB-F1XV spec set). Consumed by load_rom_assets(); overridable via
    // set_bios_filenames() before cold_boot (config-fed, CLI > XML >
    // default). (DEC-0077)
    ...
    // VDP access-timing foundation bookkeeping. Updated in exactly
    // ONE place, inside run_frame(), alongside the existing
    // vdp_.on_vsync() call. Defaults to
    ...
    // VDP raster-position clock. Declared AFTER
    ...
    // Line-interrupt delivery cache. line_int_next_cycle_ is
    ...
    // agree on (openMSX 21.0: src/video/VDP.cc:527-529:
    // `((controlRegs[19] - controlRegs[23]) & 0xFF)` display lines after
    // display start; fMSX 6.0: source/fMSX/MSX.c:2094-2100:
    ...
    // Sony MB670836 hardware PAUSE + Speed Controller. A
    // machine-level CPU-execution gate consulted at the very top of
    // step_cpu_instruction() -- NOT
    // part of Z80aCpu (zero edit to src/devices/cpu/*). Its VBlank-synced
    // duty-cycle window is advanced in exactly ONE place inside
    // run_frame(), alongside the existing vdp_.on_vsync() call.
    devices::chipset::Mb670836PauseController pause_controller_;

    // --- Live stream-capture state (default-off; see the public API; DEC-0052). ---
    // One ring entry = the reused per-instruction record + the #A8 primary-
    ...
    // Stream-light: whether the armed capture is the lightweight
    // variant (per-frame bundles suppressed -> coarse anchors + watchlog).
    bool stream_light_ = false;
    // Stream-light: the PC of the instruction currently executing,
    ...
    // Physical-DRAM watchpoint state (default-off; see set_mem_watch; DEC-0072).
    ...
    // Per-byte/per-sector Write Sector logger adapter (installed
    // on fdc_ only while armed). Only holds a reference to *this.
    FdcWriteStreamObserver fdc_write_stream_observer_{*this};
    // Stream-light watchlog observer adapters (installed on ram_mapper_
