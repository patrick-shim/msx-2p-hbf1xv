...
//
// Clean-room discipline: this file was written without opening
// fMSX 6.0's `source/EMULib/SHA1.c` (or any other emulator's SHA-1); the
// algorithm is a public NIST standard and this is an independent from-spec
// implementation, exactly like the from-spec SHA-256 precedent in the
// scroll-shooter smoke integration test under tests/integration/machine/.
//
// Placement note: src/machine/ -- the cartridge identifier
// (cartridge_identifier.h) needs it in-process, and tools/ scripts cannot be
// linked into the emulator.
class Sha1 {
public:
    Sha1();
