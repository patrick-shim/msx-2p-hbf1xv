...
std::vector<std::int16_t> MachineAudioMixer::mix_interleaved_stereo(
    ...
    // Byte-behaviour identical delegation (a nullptr fm contributes an
...
std::vector<std::int16_t> MachineAudioMixer::mix_interleaved_stereo(
    ...
    // Byte-behaviour identical delegation (an all-null FmSources
    // contributes an exact 0 fm_pac term to every sample -- the FM-PAC hard
...
std::vector<std::int16_t> MachineAudioMixer::mix_interleaved_stereo(
    ...
    // Byte-behaviour identical delegation (a nullptr click contributes
    // an exact 0 term to every sample -- the click hard regression oracle; the
...
std::vector<std::int16_t> MachineAudioMixer::mix_interleaved_stereo(
    ...
    for (std::size_t i = 0; i < sample_count; ++i) {
        // PSG: the genuine PsgAudioPump (advance + sample), unchanged.
        ...
        // box average over the window
        // (SccWavetable::take_integrated_sample(); the fixed-point
        // property keeps constant-output SCCs byte-identical to a
        // point sample; DEC-0043). A null entry contributes exactly 0, so the zero-SCC
        // path stays arithmetically identical to the SCC-less loop for ANY
        // input (hard regression oracle; both sides compute
        // through the same integrated pump).
        ...
        // FM-PAC: each inserted external FM-PAC cartridge owns
        ...
        // sums to exactly 0 -> byte-identical to the FM-PAC-less mix for ANY
        // input (the FM-PAC hard regression oracle). Output is mono, so like
        ...
        // Click: advance the 1-bit DAC by the SAME per-sample delta as
        // every other generator (pacer lockstep, DEC-0033 -- even under audio
        ...
        // 0 -- the click hard regression oracle. Mono, like SCC/FM: the same term
        ...
        // The clamp is REQUIRED: two SCCs
        // + a loud PSG exceed int16, and so can a loud built-in-FM + FM-PAC mix
        // (see header arithmetic); the click adds up to +-9,374 more. SCC
...
std::array<std::int16_t, 2> MachineAudioMixer::produce_synced_sample(
    ...
    // PSG: finalize the sub-frame-accurate integral to this
