...
devices::audio::PsgYm2149::StereoSample PsgAudioPump::pump_one_sample(devices::audio::PsgYm2149& psg) const {
    // Advance by exactly one sample window, then take the chip's exact
    // box-average over that window -- the integrated-sample production
    // switch. The pump guarantees the take-API's documented precondition by
    // construction (it advances exactly cycles_per_sample_ between takes).
    // cycles_per_sample_ == 0 (the idle case) advances nothing and takes
    // the guarded {0,0}. (DEC-0043)
