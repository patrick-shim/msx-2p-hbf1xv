...
// Deterministic, decoded-PSG-audio dump serializer (debug/test tooling).
//
// Reuses the real PSG sample-generation wiring
// (frontend::PsgAudioPump / devices::audio::PsgYm2149::advance_cycles()/
// take_integrated_sample(), the integrated box-average path -- see the note
...
// discipline (src/frontend/ MAY depend on src/machine/). Headless-
// buildable, no SDL3 dependency: added to the SAME unguarded
...
// Documented, linear, deterministic PCM-scaling formula, grounded
...
// so `raw_sum` is in [0, 62]. Linear map:
...
// PsgAudioPump(kSystemClockHz / sample_rate_hz) (genuine reuse of the
// existing pump wiring; the already-shipped Sdl3AudioPresenter::kSampleRateHz
...
//
// Inherits the pump's integrated-sample production -- each dumped sample is
// the exact box average over its cycle window; the degenerate
// cycles_per_sample == 1 case (sample_rate_hz >= the system clock) yields
// exact per-cycle levels. (DEC-0043)
[[nodiscard]] std::string serialize_psg_audio_dump(devices::audio::PsgYm2149& psg,
                                                    std::uint64_t sample_rate_hz,
                                                    std::size_t sample_count);

// Writes to <debug_root>/sounds/<filename> (a debug/ subfolder,
// matching the existing traces//logs//frames/ convention), creating the
...
// machine-level surface. Returns true on success.
...
// Serialize an ALREADY-MIXED interleaved signed-16-bit stereo PCM buffer
// (2 * sample_count int16 values, [left,right] pairs) into the SAME
// "HBF1XV-AUDIO-DUMP v1" text format as serialize_psg_audio_dump() -- reused by
// the headless --audio-dump voice-window capture, which mixes the FULL machine
// audio (PSG + SCC + built-in FM + FM-PAC + the 1-bit-DAC key click) per frame
// via MachineAudioMixer rather than pumping the PSG in isolation. Deterministic:
// pure serialization of the provided bytes (no gain/filter/resample). The dump
// text converts to a real WAV offline, exactly like the PSG dump.
...
bool write_pcm_audio_dump(const std::filesystem::path& debug_root, const std::string& filename,
                          const std::vector<std::int16_t>& pcm_stereo, std::uint64_t sample_rate_hz);
