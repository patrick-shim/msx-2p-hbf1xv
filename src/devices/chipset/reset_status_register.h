...
// bit 7 clear -> cold boot -> the BIOS runs the animated MSX logo sequence,
// then writes bit 7 set so the logo is skipped on subsequent warm restarts.
// (Empirically confirmed against openMSX Sony_HB-F1XV: #F4 reads 0x00 at
// power-on, the logo animation runs t=1.25s..4.0s, and #F4 reads 0x80
// afterwards. The local bios/f1xvbios.rom contains the IN A,(0F4h) /
// OUT (0F4h),A primitives at offsets 0x146A/0x146F.)
//
// Behaviour model (non-inverted variant), grounded at
// references/openmsx-21.0/src/MSXResetStatusRegister.cc:13-35 (read for
// understanding, never copied — GPL isolation):
//   - power-up: status = 0x00;
//   - read: returns the stored status byte;
//   - write: only bits 7 and 5 are storable — bit 5 is preserved from the
//     previous value while bit 7 (and bit 5 via the value mask) update,
//     i.e. status = (status & 0x20) | (value & 0xA0).
//
// Real hardware keeps this latch across a RESET-button warm reset (that is
// its entire purpose); this machine model only exposes cold power-up
