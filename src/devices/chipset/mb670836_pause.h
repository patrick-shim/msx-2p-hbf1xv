...
// Sony MB670836 residual hardware-PAUSE / Speed-Controller layer.
// Grounded in the `Yamaha S1985 MSX-ENGINE
// Chipset` fact sheet §9: a second Sony custom LSI (MB670836) beside the S1985 handles
...
// cannot be bypassed in software. Per the `Zilog Z80A
// CPU` fact sheet §6 item 3, the HB-F1XV has no CPU turbo mode -- the "Speed
...
// the manual PAUSE button (press_pause_button(), toggle semantics --
// a first-principles design choice; no datasheet documents the real
...
// DESIGN DEFAULT, not a measured hardware fact.
...
public:
    ...
    // Physical PAUSE button: one press toggles engaged/disengaged.
    ...
    // VBlank hook: advances the frame counter
    ...
    // same test, that double-counts elapsed cycles.
    void on_vsync();
    ...
    // BEFORE any opcode decode. True if EITHER source is active:
    // the manual button OR the Speed Controller's current duty-cycle window.
    [[nodiscard]] bool cpu_should_pause() const;

    // Debug-snapshot seam: the internal VBlank frame counter, for a
    // restore-ready snapshot. const, ZERO behavior change.
