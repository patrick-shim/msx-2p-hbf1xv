...
// --- Diagnostic sprite trace (DEC-0078) --------------------------------------
...
// call site short-circuits, and the emulated state is untouched (dual-run
// byte-identity; same pattern as the env-gated FDC write-logger). Owns the
// frame counter so all
// traced event classes land in one ordered stream with "F<n> " prefixes.
...
// NOTE: an earlier empirical per-mode command-throughput placeholder (a fixed
// 200% factor for GRAPHIC4/5, 300% for GRAPHIC6/7) was removed from here.
// The active-display command-throughput cap is now the principled,
...
// §8; the three per-line slot COUNTS live in vdp_access_timing.h). The
// old placeholder was a coarse per-mode scalar that was NOT sprite-aware and NOT
// grounded in the 154/88/31 slot counts. (DEC-0075)

}  // namespace

V9958Vdp::V9958Vdp() : cmd_engine_(vram_) {
    // Route the command engine's per-destination-
...
void V9958Vdp::reset() {
    ...
    // Reset the sprite/command engines before recompute_mode() below
    ...
    // No progressive sprite-split pass survives a reset.
    ...
    // No command busy window survives a reset (both the engine's
    ...
    // Color-burst registers default 00/3B/05 (fact sheet §4; VDP.cc
    // reset). Stored only (color burst has no visual consequence in this
    // device).
    ...
    // Blink state: stable/off at reset (VDP.cc:278-279).
    blink_countdown_ = 0;
    blink_state_ = false;
...
int V9958Vdp::effective_slots_per_line() const {
    // Select the live 3-way CPU/command VRAM access-slot
    // regime (fact sheet §8: 154 display-off / 88 sprites-off / 31
    // sprites-on) from live registers (DEC-0075).
    //
    // display ON  = R#1 bit6 (BL, display enable; fact sheet §4) AND the
    ...
    //               fact sheet §4) AND the current mode actually has
    ...
    // sprites are active on the line (fact sheet §6(a): the sprite
...
void V9958Vdp::arm_command_busy_window() {
    ...
    // Turn the command engine's pure, clock-free base duration into
    // an absolute expiry timestamp for the S#2-bit0 CE busy window.
    std::uint64_t duration = cmd_engine_.last_cmd_duration_tstates();
    if (duration != 0) {
        // Per-line slot-availability throughput cap (DEC-0075). The
        // engine's base duration corresponds to the MAX-availability 154-slot
        // regime; inflate it by slot_factor = 154 / S_effective when
        // fewer slots are free. From the three tier-1 counts (fact sheet §8)
        // this is a 3-valued, sprite-aware factor:
        //   S=154 -> 1.00x (display off/border/vblank -> no inflation;
        //            (d*154 + 153)/154 == d exactly),
        //   S= 88 -> 1.75x (display on, sprites off),
        //   S= 31 -> 4.97x (display on, sprites on).
        // Integer ceil of duration * 154 / S. This superseded an earlier
        // empirical per-mode slot-factor placeholder. It is
        // a pure CE busy-WINDOW / S#2 STATUS overlay ONLY: VRAM mutation stays
        // atomic and the engine's pure duration + per-command render-sync
    ...
    // Diagnostic bias (DEC-0072; default 0), applied AFTER the slot factor. Clamp
...
void V9958Vdp::steal_command_slot_for_cpu_vram_access() {
    // CPU-priority
    // VRAM access-slot STEAL (DEC-0075). On real hardware a CPU VRAM access
    // through #98 takes
    ...
    // serviced with its normal timing (fact sheet §8: "CPU VRAM access
    ...
    // V9958 /WAIT generator (fact sheet §1/§7), so the VDP can
    ...
    // no CPU T-state change. VRAM mutation stays atomic and
    ...
    // (S_effective above): slot_cost_tstates(S) = ceil(1368 / (6 * S)) ->
...
core::BusData V9958Vdp::io_read(const core::BusAddress port) {
    ...
    case 0:  // #98 VRAM data read
        // CPU-priority slot steal -- a concurrent CPU #98
...
void V9958Vdp::io_write(const core::BusAddress port, const core::BusData value) {
    // Render-sync seam: notify BEFORE any state mutates, for all four
    ...
    // writes into its own pixel store.
    // The adapter ALSO keeps the incremental sprite plane
    ...
    case 0:  // #98 VRAM data write
        // CPU-priority slot steal -- extends a BUSY
...
std::uint32_t V9958Vdp::effective_address() const {
    ...
    // CPU-port planar addressing: in G6/G7 (and the YJK overlay
    // modes sharing GRAPHIC7's base) planar VRAM is spread over two banks;
    // the STORAGE address is a 17-bit rotate-right-by-1 (derived
    ...
    // `vram_pointer_`/`control_regs_[14]`.
...
void V9958Vdp::advance_vram_pointer() {
    ...
    // TMS9918 modes (G1/G2/MC/T1) wrap at the 16 KB boundary (fact sheet §2).
...
std::uint8_t V9958Vdp::vram_data_read() {
    ...
    // scheduled) model is used; cycle-accurate access-slot timing is not modeled.
...
std::uint8_t V9958Vdp::read_status(const int reg) {
    ...
    if (reg == 0) {
        ...
        //
        // The collision-event queue is FRAME-ATOMIC (collected
        // by recompute_frame at on_vsync) -- the seam's render-
    ...
    case 0:
        ...
        // (SpriteChecker.hh:104-110's `& 0x1F`), leaving the
...
void V9958Vdp::indirect_write(const std::uint8_t value) {
    ...
    // even if R#17 is written indirectly (the documented openMSX edge case).
    data_latch_ = value;
...
void V9958Vdp::change_register(const std::uint8_t reg, const std::uint8_t value) {
    if (reg >= kNumControlRegs) {
        // R#32..R#46 = the command engine's own register file
    ...
    // Capture the OLD value before committing so the IE0/IE1
    ...
    // (openMSX 21.0: src/video/VDP.cc:1170-1198).
    ...
    // Diagnostic watchlog hook (default-off; DEC-0052): notify the diagnostic
    ...
    case 0:  // M3..M5 + IE1 (R#0 bit4, line-int enable, gates S#1 FH)
        // An R#0 write that TOGGLES IE1 re-evaluates the horizontal
    ...
    case 1:  // M1..M2 + IE0 (R#1 bit5, VBlank-int enable)
        ...
        // ISR's later EI re-fire forever -> nested-VBLANK stack overflow, seen
        // as a multi-disk RPG title's building-interior crash).
    ...
    case 16:
        ...
        break;
    case 13: {
        // Blink on/off timing. Writing R#13 resets blink
...
void V9958Vdp::recompute_mode() {
    ...
    // scrMode recompute: governs BOTH command legality and which
...
void V9958Vdp::command_row_sync(const unsigned dy) {
    ...
    // BEFORE committing the byte (openMSX 21.0: src/video/VDPVRAM.hh:
    // 575-593, 309-322), with the destination display line standing in for
    // openMSX's EmuTime. Behavior reference only -- never copied. (DEC-0065)
    if (render_sync_ == nullptr) {
    ...
    case VdpMode::ScreenYjkYae:
        ...
        break;
    default:
        return;  // non-bitmap / text / sprite-table destinations -> fall back
    }
    ...
    //     tripping the accumulator's mid-command finalize) and the BEAM
    //     CLAMP (never sealing rows AHEAD of the beam
    //     with commit-time register/SAT state; DEC-0091-AMENDMENT-A) are
    ...
    // Trace: the command-engine row sink's
...
// --- Progressive per-frame sprite-split lifecycle ----------------------------

int V9958Vdp::sprite_frame_height() const {
    // The sprite-buffer height formula, extracted so begin_sprite_frame()
...
void V9958Vdp::commit_sprite_split(const int target, const bool wrap) {
    ...
    // display write; a genuine frame wrap / geometry jump (`wrap`) re-opens it,
    ...
    // Trace: lazy-open / wrap-reopen with the
    ...
    // [wm, target) -- the SAME lines the background commits, split at the SAME
    // line. RENDERING-ONLY: it populates the visible-sprite buffers per-line-live
    ...
    // watermark) satisfies the "never move the watermark backwards" discipline.
...
void V9958Vdp::commit_sprite_rows(const int target) {
    // See the header contract (DEC-0078). The
...
void V9958Vdp::on_vsync() {
    ...
    // Blink countdown (VDP.cc:600-608). Decrements once
    ...
    // Sprite check/collision/5th-sprite pass. Once per frame boundary,
    // mirroring the blink-countdown
    ...
    // ALWAYS the frame-atomic recompute -- collision /
    // 5th-sprite / S#0 status stay frame-atomic for EVERY game (DEC-0031
    ...
    // Trace: the frame-atomic recompute boundary.
    if (m51_sprite_trace_enabled()) {
...
std::uint8_t V9958Vdp::peek_status_register(const int reg) const {
    ...
    case 2: {
        ...
        // bit4 BD, bit0 CE live from the command
        // engine. Bits5/6 (HR/VR) are LIVE, raster-position-derived (a bug
        // fix discovered via a real, CPU-driven BIOS boot: the BIOS
        ...
        // a vsync. Same pull-style clock read as the VR/HR bits above. (DEC-0069)
