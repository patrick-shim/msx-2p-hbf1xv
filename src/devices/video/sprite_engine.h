...
// V9958 sprite check / collision / 5th-sprite-detection engine.
// Owned INSIDE V9958Vdp as a private member (mirrors the
...
// (mirrors the blink-countdown precedent exactly).
//
...
// openMSX 21.0: src/video/SpriteChecker.{hh,cc} (check/collision/
...
public:
    ...
    // palette index, bit5 = IC (collision-exclusion only), bit6 =
    ...
    //
    // A thin single-shot convenience WRAPPER over the
    // stateful progressive engine below -- `begin_frame(); check_until(height-1)`.
    // Byte-identical to a single-sweep pass: the whole frame
    // is checked in one segment against one register/VRAM snapshot, so every
    // per-line visible-sprite buffer + S#0/S#3-S#6 status matches a one-shot
    // recompute. This is the entry `V9958Vdp::on_vsync()` calls.
    void recompute_frame(const VdpVram& vram, std::span<const std::uint8_t, 32> control_regs,
                         const VdpModeState& mode, int height);

    // --- Progressive per-line LIVE sprite check -------------------------------
    //
    ...
    // frame-boundary geometry (height, mode -> sprite-mode/planar;
    ...
    // (the point of the progressive engine; the per-line R#1 BL / R#8 SPD /
    // sprite-mode-0 gates are
    ...
    // exactly as a single full-frame sweep would. Grounding (behavior/model
    // only, never copied): Yamaha V9958 VDP fact sheet §6 (sprite
    // data for line N fetched during line N-1 -> per-scanline, not per-frame);
    // openMSX SpriteChecker::checkUntil() (the incremental sync-before-
    // change model the background render-sync seam already implements).
    //
    // The render-sync seam drives check_until_visible_only() below; this full
    // check_until() (with collision collection) is called only by
    // recompute_frame()'s single full-height sweep.
    void begin_frame(const VdpVram& vram, std::span<const std::uint8_t, 32> control_regs,
                     const VdpModeState& mode, int height);
    void check_until(int line);

    // RENDERING-ONLY progressive pass. Populates the per-line
    // VISIBLE-sprite buffers up to (and including) `line` from the bound LIVE state
    // AT THIS CALL -- exactly like check_until -- but does NOT touch the
    ...
    // mid-frame sprite-relevant change splits the RENDERED sprite plane
    // per-line-live, while the CPU-visible collision/status stays frame-atomic
    // (recompute_frame at on_vsync) -- no
    // game-behaviour change (migrating collision itself to per-line-live would
    // be a separate, separately-verified change). Advance-only; shares
    // check_until's range logic. (DEC-0031)
    ...
    // unconditionally sprite-free, so it is never in the checked range).
    [[nodiscard]] int watermark() const { return watermark_; }

    // Visible sprites for 0-based active-display output line `line`. Empty
    // for line 0 (the 1-pixel vertical shift means output line 0 is
    ...
    // S#0-read side effect (SpriteChecker.hh:104-110): clears ONLY
...
private:
    ...
    // Check + collect collision for display lines [lo, hi) (lo>=1,
    // hi<=height_) using the bound LIVE state, advancing fifth_num_/fifth_line_/
    // sprite_end_ and appending collision events in raster order. Uses the
    // same per-line Y/X/pattern/mode-2-base-mask/planar formulas as a
    // whole-frame sweep --
    ...
    // byte-identical to a full-frame sprite loop.
    // `collect_collision` (default true) gates ONLY the collision-event
    ...
    // Compose S#0 (5S/number) + the frame-boundary collision latch once
    // the whole frame has been checked. No-op if the frame never became active
    // (disabled / sprite-mode-0 / height<=0), leaving status + collision
    // untouched.
    void finalize_frame();
    ...
    VdpModeState mode_captured_{};  // sprite-mode/planar are a per-frame decision
    int height_ = 0;
