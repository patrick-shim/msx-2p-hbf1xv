...
// (dual-run byte-identity; same pattern as the env-gated FDC
// write-logger). Declared here (no new header) so V9958Vdp AND the machine
// render-sync seam (hbf1xv_machine.cpp) write the traced sprite event classes
// into ONE raster-ordered stream.
...
// status bits. X-pattern of rtc::RtcClockSource/
...
public:
    ...
    // accessor; does NOT edit src/core). (DEC-0069)
    [[nodiscard]] virtual std::uint64_t cpu_total_cycles() const {
        return cpu_tstates_since_vsync();
    }
};

// Render-sync seam. Nullable listener notified at the TOP of
// V9958Vdp::io_write(), for
...
// applies (openMSX 21.0: src/video/PixelRenderer.cc:253-394
...
// 13 commands execute inside register/data-port writes per the hybrid
// execution model, so their VRAM effects are bracketed by hooked writes).
// X-pattern of
...
public:
    ...
    // display line standing in for openMSX's EmuTime). (DEC-0065) BEAM CLAMP:
    // the implementation MUST NOT commit rows AHEAD of
    ...
    // DEFAULT EMPTY => every existing listener (and the render-sync seam
...
// Non-perturbing control-register-write observer for diagnostic capture.
...
// state or advance any clock. (DEC-0052)
...
// Yamaha V9958 VDP — register / VRAM / status / interrupt CONTRACT.
//
...
// VRAM store plus the sprite engine and VDP command engine that mutate
...
// VdpFrameRenderer. Not modeled here:
// fully cycle-accurate slot/command timing and the exact sub-frame IRQ raster
...
//
// Grounding: Yamaha V9958 VDP fact sheet (registers, ports,
// status, interrupts); behavior reference openMSX 21.0: src/video/
...
public:
    ...
    // the command engine's own register file (see cmd_engine()) and are
    ...
    // Wire the S#2 VR/HR raster-position clock source.
    ...
    // Wire the render-sync listener (see VdpRenderSyncListener
    // above). Nullptr (the default) detaches -- byte-identical no-op.
    void attach_render_sync(VdpRenderSyncListener* listener);

    // Wire the control-register-write observer (see
    // VdpRegisterWriteObserver above; DEC-0052). Nullptr (the default)
    // detaches -- byte-identical no-op. Not cleared by reset().
    void attach_register_write_observer(VdpRegisterWriteObserver* observer);

    // While set, an R#46 command write
    // does NOT arm the S#2-bit0 CE busy window. The machine brackets its
    ...
    // stay byte-identical (only real CPU-driven OUT writes pace CE). Default
    // false. (DEC-0069)
    void set_command_timing_suspended(bool suspended);

    // Diagnostic knob: add a signed
    ...
    // byte-for-byte unchanged. (DEC-0072)
    ...
    //     frame raster position is not modeled). ---
    ...
    // The machine's render-sync adapter calls this from
    ...
    // background split. Lazy-opens the per-frame progressive pass on the
    // first ACTIVE-display write (binding live VRAM/registers + capturing the frame
    // geometry, watermark reset); `wrap` (a genuine frame wrap / geometry jump,
    ...
    // (SpriteEngine::check_until never moves the watermark backwards -- the
    // advance-only direction discipline).
    void commit_sprite_split(int target, bool wrap);

    // CONSUMER-side sprite pacing for
    // every non-seam background commit -- the command-row sink
    // (VdpRenderSyncAdapter::on_commit_up_to) and the render_frame() mid-display
    // memoization syncs. Without this, the command sink advanced/committed
    ...
    // (a sprite-disappearance defect seen in scrolling-shooter / sprite-scroll
    // / split-screen titles). openMSX makes the CONSUMER
    ...
    // what triggered the render (openMSX 21.0: src/video/
    ...
    // (fMSX 6.0: source/fMSX/Common.h:99-155). ADVANCE-ONLY-WHEN-
    // ACTIVE: while the split is not open the buffers still hold the previous
    // on_vsync() recompute (fully populated -- the documented
    ...
    // a command commit never needs to open it (trace evidence confirmed every
    // failing command commit ran with the split active). `target` is the
    // EXCLUSIVE boundary of the rows about to be committed (same convention as
    // commit_sprite_split); RENDER-ONLY (frame-atomic collision/status
    // untouched, DEC-0031). (DEC-0078)
    ...
    // internal pending flag (the VDP owns the level; release only on the
    ...
    // --- Sprite engine + VDP command engine. Owned INSIDE V9958Vdp
    ...
    // re-latching (boot-logo fix) and -- public -- by the
    // machine's render-sync adapter (write at line L takes effect from line
    // L+1) and line-interrupt delivery.
    ...
    // Test/debug accessor: the ABSOLUTE CPU cycle at which
    ...
    // behavior change (same read-only-introspection idiom as the accessors
...
private:
    ...
    // --- The progressive per-frame sprite-split lifecycle. ---
    // The active-display pixel height for the CURRENT mode -- the on_vsync() sprite
    // buffer height (a per-frame decision). Extracted so begin_sprite_frame()
    ...
    // The live 3-way CPU/command VRAM access-slot regime
    ...
    // SpritesOn=31}, fact sheet §8) selected from live R#1 BL +
    // raster_display_line() + R#8 SPD + mode. Pure/const; consulted at
    // command-issue time by arm_command_busy_window() and per CPU #98
    // access by the io paths. NEVER gates CPU T-states. (DEC-0075)
    [[nodiscard]] int effective_slots_per_line() const;

    // Arm the S#2-bit0 CE busy window
    ...
    // suspended. Inflates the base (max-availability,
    // 154-slot) duration by slot_factor = 154 / effective_slots_per_line()
    // (~1.00 / 1.75 / 4.97 for the three tier-1 slot counts, fact sheet §8)
    // -- superseding an earlier empirical per-mode slot-factor
    ...
    // timing/STATUS overlay -- VRAM mutation stays atomic. (DEC-0069, DEC-0075)
    void arm_command_busy_window();

    // The CPU-priority VRAM access-slot steal. Called on
    ...
    // slot = slot_cost_tstates(effective_slots_per_line()) T-states (fact sheet
    // §8 "CPU VRAM access takes priority over the command engine and steals
    ...
    // exactly like arm_command_busy_window(). (DEC-0075)
    ...
    // destination-row render-sync (DEC-0065). Applies the strict-superset gates and, when
    ...
    // S#2 VR/HR raster-position clock source. Nullptr by
    // default; attached by the machine in wire_bus().
    VdpClockSource* clock_source_ = nullptr;

    // The ABSOLUTE CPU cycle at which
    ...
    // Diagnostic bias (see set_cmd_busy_bias; DEC-0072); default 0 = no change.
    ...
    // Render-sync listener. Nullptr by default (byte-identical
    // no-op); attached by the machine in wire_bus().
    VdpRenderSyncListener* render_sync_ = nullptr;

    // Diagnostic control-register-write observer (DEC-0052). Nullptr by default
    ...
    // Blink countdown state (VDP.cc:600-608/1040-1057).
    ...
    // Sprite engine + command engine. cmd_engine_ is constructed with
    ...
    // The RENDERING-ONLY progressive sprite pass is
    ...
    // on_vsync. Reset each frame at
    // on_vsync() and at reset().
    bool sprite_split_active_ = false;

    // The command-engine per-destination-row render-sync adapter, installed
