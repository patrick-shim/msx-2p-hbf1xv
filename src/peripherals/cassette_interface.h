...
// (mirrors devices::rtc::RtcClockSource,
...
// CPU T-state accounting is never touched (protects the CPU
...
// Cassette interface peripheral.
//
// NOT a core::IoDevice: the machine XML's `<CassettePort/>` is an
// empty element -- no dedicated CPU-visible I/O port exists for the cassette
// on the HB-F1XV (openMSX 21.0: share/machines/Sony_HB-F1XV.xml:21;
...
//
// Write path (motor/output) -- pure, on-demand derivation, no owned
// state: motor_on() = !ppi_.cassette_motor_off(); output_level() =
...
// the "MemoryMapperRam is a pure consumer" precedent. Two identical
...
// peripherals::JoystickPorts's own header -- joystick.h) --
// cycle-driven, not wall-clock: with no tape loaded,
...
// deterministically).
//
// Out of scope: real .CAS/.WAV/.TSX tape-file decode/
...
// contract only (mirrors the synthetic-disk-image precedent).
