...
| **SOUND**          | Built-in **MSX-MUSIC** — Yamaha **YM2413 (OPLL)** 9-channel FM synthesizer + MSX-MUSIC BIOS ("APRLOPLL"), alongside the S1985's PSG. **IMPORTANT (DEC-0050): the built-in FM is MSX-MUSIC (OPLL + BIOS only, NO SRAM) — NOT a Panasonic FM-PAC.** The FM-PAC (which adds 8 KB battery-backed SRAM for game saves) is an **optional EXTERNAL CARTRIDGE PERIPHERAL**, not an internal component — grounded in the openMSX `Sony_HB-F1XV.xml` which declares `<MSX-MUSIC>` with **no** `<sramname>`, and the real `f1xvmus.rom` being the APRLOPLL MSX-MUSIC BIOS. Consequently "NO S-RAM AVAILABLE" is CORRECT on a bare machine; SRAM game saves require inserting an FM-PAC cartridge (or use disk saves). |
...
- The standalone `msx-disk` host-side disk utility (DEC-0080/M53): C++ source in `src/diskutils/`, binary post-build-copied to `diskutils/msx-disk.exe`; creates/reads/formats 720 KB MSX-DOS FAT12 `.dsk` images byte-exact to the machine layout; build-isolated from the emulator both ways.
...
### Reference precedence (authority order — STRICT)

The mission is a machine **as genuine to real Sony HB-F1XV / MSX2+ hardware as possible.** When
grounding any hardware behavior or timing decision, weigh sources in this order and prefer the
...
1. **Real hardware specification (HIGHEST — final authority).** Authoritative datasheets and
   ...
   correct.
2. **openMSX + your own spec-disciplined reasoning.** openMSX is a highly valuable, years-refined
   reference and the basis for A/B parity; it is the strong default when the hardware spec is
   silent or ambiguous, and your own reasoning must be held to the spec sheets. **But** openMSX's
   long evolution may carry modeling choices/tweaks that do NOT match real hardware — so when
   openMSX conflicts with an authoritative hardware spec, **the spec wins**, and the divergence is
   documented (in the report/decision) rather than followed.
...
- `references/openmsx-21.0/` — openMSX 21.0 source tree. A **mature, high-value behavior reference
  ...
  where the real-hardware spec is silent, but **subordinate to the hardware spec (tier 1)** where
  they conflict (its multi-year evolution can include non-hardware-exact tweaks). Read it to
  understand behavior; **never copy its code into `src/`** (license isolation — openMSX is GPL).
  Cite it as `references/openmsx-21.0/<path>` when grounding a claim.
...
- `references/fact-sheets/` — curated hardware/device fact sheets (e.g.
  ...
  Supplement with web-researched primary hardware documentation as needed, and cite it. New sheets
  are added here as needed. (A fact sheet that merely *guessed* and deferred to openMSX is not yet
  tier-1 evidence — resolve it against real hardware documentation before relying on it.)
...
When behavior references disagree, do not silently pick one: apply the **Reference precedence**
above — real-hardware spec (tier 1) outranks openMSX (tier 2) outranks fMSX (tier 3). Check the
fact-sheets/primary hardware documentation (web research included), note the disagreement in the
relevant investigation/implementation report, and adopt the interpretation the higher-precedence
source supports. openMSX A/B remains the parity harness, but a confirmed conflict with real
hardware is resolved in favor of hardware — with the openMSX divergence documented, not followed.
...
- `src/` — emulator source PLUS the standalone host-side disk-utility source in `src/diskutils/`
  (DEC-0080). Folder responsibilities: [`src/CLAUDE.md`](src/CLAUDE.md).
...
- `diskutils/` — build-output home of the standalone **`msx-disk.exe`** disk utility (DEC-0080/M53):
  `--create` (new fully-formatted blank 720 KB MSX-DOS FAT12 `.dsk`), `--read` (xxd-style hex dump,
  `--sector`/`--range`), `--format` (re-format in place; 737,280-byte size guard, `--force` override).
  Source lives in `src/diskutils/` (owner structure directive); the binary is POST_BUILD-copied here
  from the ONE `build/` tree and is gitignored (`.gitkeep` keeps the folder). HARD BOUNDARY: the tool
  and the emulator are build-isolated both ways — the shipped tool links no emulator code, the
  emulator never depends on the tool; shared disk-layout facts are re-expressed from the documented
  spec (`tools/format-blank-disk.ps1` layout, golden blank SHA256 `6f1a7983…b0ce5188`), never via a
  code dependency. Created disks are empty, DOS-recognizable, deliberately NON-bootable media (no
  proprietary DOS system files are ever written).
- `bios/`, `roms/`, `disks/`, `games/` — local development assets (third-party IP; no
  redistribution rights asserted). Per **DEC-0047** the repo is hosted on a PUBLIC remote by the
  owner's decision: `bios/` is tracked/published; `roms/` + `disks/` content is untracked (only
  `.gitkeep` + `roms/README.md`), and the entire `games/` tree is untracked (gitignored); older
  ROM/disk binaries remain in pre-b5efd29 history.
...
- `references/` — read-only grounding sources, ranked by the **Reference precedence** above:
  ...
  (independent cross-reference, non-commercial freeware, **tier 3**), SDL3 source (API reference),
  and `zexall/` (CPU test fixtures).
