...
> **Reference precedence.** When judging whether observed behavior is correct vs. an anomaly/bug, the mission is a machine as genuine to real HB-F1XV hardware as possible: apply the STRICT reference-precedence rule in `CLAUDE.md` — real hardware spec (fact-sheets or primary hardware docs) > openMSX > fMSX, with the spec winning on a confirmed conflict. openMSX A/B stays the parity harness, but "differs from openMSX" is not automatically a defect if the real-hardware spec says otherwise; flag the divergence for developer/QA rather than resolving it in-agent.
...
- **Scratch disks on demand**: the standalone `diskutils\msx-disk.exe` tool (DEC-0080/M53,
  source `src/diskutils/`) creates deterministic blank 720 KB MSX-DOS FAT12 `.dsk` images
  (`--create`), hex-inspects them (`--read --sector N`), and re-formats scratch copies
  (`--format`) — use it to provision empty save/test disks for playtest scenarios instead of
  hand-crafting images (created disks are DOS-recognizable but non-bootable data media).
